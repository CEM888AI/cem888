...
~/.cem888/channel_directory.json.  The send_message tool reads this file for
...
DIRECTORY_PATH = get_cem888_home() / "channel_directory.json"
...
def _build_matrix(adapter) -> List[Dict[str, Any]]:
    """Enumerate joined Matrix rooms as channel directory targets."""
    channels: List[Dict[str, Any]] = []
    try:
        joined_rooms = getattr(adapter, "_joined_rooms", set())
        dm_rooms = getattr(adapter, "_dm_rooms", {})
        for room_id in joined_rooms:
            is_dm = dm_rooms.get(room_id, False)
            channels.append({
                "id": room_id,
                "name": room_id,
                "type": "dm" if is_dm else "room",
            })
    except Exception as e:
        logger.warning("Channel directory: failed to build matrix: %s", e)
    return channels


def _build_from_sessions(platform_name: str) -> List[Dict[str, str]]:
    """Pull known channels/contacts from sessions.json origin data."""
    sessions_path = get_cem888_home() / "sessions" / "sessions.json"
