"""
Email platform adapter for the CEM888 gateway.

Allows users to interact with CEM888 by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "cem888-agent" "version" "{_cem888_version}" '
            '"vendor" "CEM888.AI" '
            '"support-email" "noreply@cem888.ai")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "CEM888 Agent")
        ...
        msg_id = f"<cem888-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
