...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-CEM888-Session-Id header; opt-in long-term memory scoping via X-CEM888-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-CEM888-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists cem888-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible CEM888 sessions
- POST /api/sessions               — create an empty CEM888 session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to cem888-agent
...
def _normalize_multimodal_content(content: Any) -> Any:
    ...
    converts (``_preprocess_cem888ai_content`` for CEM888.AI).
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_cem888_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted CEM888_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # cem888_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same CEM888 session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through cem888-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-CEM888-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-CEM888-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-CEM888-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-CEM888-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-CEM888-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``cem888 sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the cem888-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-CEM888-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "cem888-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return cem888-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every CEM888 version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "cem888.api_server.capabilities",
            "platform": "cem888-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side CEM888 AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-CEM888-Session-Id",
                "session_key_header": "X-CEM888-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted CEM888 sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty CEM888 session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "cem888.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "cem888.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "cem888.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "cem888.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-CEM888-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-CEM888-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "cem888.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-CEM888-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-CEM888-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        for idx, msg in enumerate(messages):
            ...
            if role == "system":
                # System messages don't support images (CEM888.AI rejects, OpenAI
        ...
        # stable per-channel identifier via X-CEM888-Session-Key.  This
        # is independent of X-CEM888-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-CEM888-Session-Id.
        ...
        provided_session_id = request.headers.get("X-CEM888-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-CEM888-Session-Id rejected: "
            ...
            # conversation map to the same CEM888 session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``cem888.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-CEM888-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-CEM888-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["cem888"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-CEM888-Completed"] = "false"
            response_headers["X-CEM888-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + CEM888-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-CEM888-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-CEM888-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-CEM888-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-CEM888-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: cem888.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-CEM888-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-CEM888-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-CEM888-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-CEM888-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "cem888.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local CEM888-Relay
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from cem888_cli.auth import has_usable_secret
