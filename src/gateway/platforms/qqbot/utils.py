...
def _get_cem888_version() -> str:
    """Return the cem888-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; CEM888/<cem888_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; CEM888/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    cem888_version = _get_cem888_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; CEM888/{cem888_version})"
