...
from agent.turn_guard import should_process_message
...
# Uses get_cem888_dir() so each profile gets its own Matrix store.
from cem888_constants import get_cem888_dir as _get_cem888_dir

_STORE_DIR = _get_cem888_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("CEM888_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("CEM888_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "cem888"
    ...
    async def _on_room_message(self, event: Any) -> None:
        ...
        # Turn guard: prevent agent infinite loops in group chats.
        # Resets on human message, blocks agent responses when turns exhausted.
        if not should_process_message(sender, room_id):
            logger.debug(
                "Matrix: turn guard blocked message from %s in %s",
                sender, room_id,
            )
            return
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "CEM888 Agent" become "Agent".
        """
        if not body:
            return ""
