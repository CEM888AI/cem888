...
class GatewayStreamConsumer:
    ...
    def __init__(
        ...
        self._last_commentary_text = ""   # Last interim commentary delivered (echo dedup)
    ...
    async def run(self) -> None:
        ...
        try:
            while True:
                ...
                # Echo dedup (#double-response): interim commentary was already
                # delivered as its own message, but models commonly re-open the
                # final answer with that same line.  Strip the prefix from the
                # accumulated final text BEFORE it flushes, so the user never
                # sees the same preamble twice.  Only strip when the final is
                # longer than the commentary — never delete the whole answer.
                if (
                    got_done
                    and self._last_commentary_text
                    and self._accumulated.startswith(self._last_commentary_text)
                    and len(self._accumulated) > len(self._last_commentary_text)
                ):
                    self._accumulated = self._accumulated[len(self._last_commentary_text):].lstrip(" \n")
                    self._last_commentary_text = ""
                ...
                if commentary_text is not None:
                    ...
                    try:
                        ...
                        self._last_commentary_text = commentary_text
                    except Exception:
                        self._last_commentary_text = ""
                    self._last_edit_time = time.monotonic()
                    self._reset_segment_state()
    ...
    async def _send_commentary(self, text: str) -> bool:
        ...
        try:
            ...
            # multiple tool calls. See: https://github.com/CEM888AI/CEM888/issues/10454
            if result.success:
