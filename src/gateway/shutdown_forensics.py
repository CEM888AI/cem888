...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        cem888_home_str = os.environ.get("CEM888_HOME")
        if cem888_home_str:
            takeover_path = Path(cem888_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(cem888_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded cem888-agent but never re-ran ``cem888 setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../cem888-gateway.service"
    ...
    # that's the common case for cem888.
    ...
    # constant in cem888_cli/gateway.py.
    headroom = 30.0
