...
#   - cli.py, cem888_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from cem888_cli.config import get_config_path
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("CEM888_KANBAN_TASK")),
        )
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("CEM888_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by CEM888_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like cem888-cli)
...
def handle_function_call(
    ...
    try:
        ...
        # Inspired by CEM888 Code 2.1.119, which added ``duration_ms`` to
        ...
        # CEM-176: mint evidence authority at the trusted post-execution
        # boundary. ``result`` here is the exact object ``registry.dispatch``
        # returned -- nothing between execution and this point can have
        # rewritten it -- so this is the only place a tool result can be
        # attested. The token is handed to the hooks below; a verifier that
        # never receives one cannot claim a PASS. Fail-open: on any error
        # there is simply no authority, which fails closed at verification.
        _tool_evidence_attestation = None
        try:
            from cem888_cli.plugins import attest_tool_result

            _tool_evidence_attestation = attest_tool_result(
                function_name,
                tool_call_id=tool_call_id or "",
                session_id=session_id or "",
                task_id=task_id or "",
            ...
            logger.debug("tool evidence attestation error: %s", _attest_err)
        ...
        try:
            ...
            invoke_hook(
                ...
                session_id=session_id or "",
                tool_call_id=tool_call_id or "",
                evidence_attestation=_tool_evidence_attestation,
