...
Inspired by CEM888.AI's CEM888 Skills system with progressive disclosure architecture:
...
# All skills live in ~/.cem888/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
CEM888_HOME = get_cem888_home()
SKILLS_DIR = CEM888_HOME / "skills"

# CEM888.AI-recommended limits for progressive disclosure efficiency
MAX_NAME_LENGTH = 64
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from CEM888_HOME/.env."""
    env_path = get_cem888_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("CEM888_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("CEM888_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_cem888_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.cem888/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``CEM888_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("CEM888_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``CEM888_PLATFORM`` environment variable
    3. ``CEM888_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from cem888_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("CEM888_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.cem888/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``cem888 skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_cem888_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from cem888_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: cem888 plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.cem888/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `cem888 skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.cem888.* first (agentskills.io convention), fall back to top-level
        cem888_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            cem888_meta = metadata.get("cem888", {}) or {}

        tags = _parse_tags(cem888_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            cem888_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
