...
~/.cem888/skills/. Existing skills (bundled, hub-installed, or user-created)
can be modified or deleted wherever they live.

Skills are the agent's procedural memory: they capture *how to do a specific
type of task* based on proven experience. General memory (memories/*_MANIFEST.md + the durable typed store) is
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `cem888 config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.cem888/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`cem888 curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.cem888/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER CEM888 profiles.
    ...
    # Default profile (~/.cem888/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.cem888/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`cem888 -p "
            ...
            base += (
                ...
                f"Switch profiles (`cem888 -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_cem888_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `cem888 curator unpin <name>`. "
