"""
Lazy dependency installer for opt-in CEM888 Agent backends.

Many CEM888 features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``cem888-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``cem888 tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    # ─── Inference providers ───────────────────────────────────────────────
    # Native CEM888.AI SDK — needed when provider=cem888ai (not via
    # OpenRouter / aggregators which use the openai SDK).
    "provider.cem888ai": ("cem888ai==0.87.0",),  # CVE-2026-34450, CVE-2026-34452
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("CEM888_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``cem888 update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``cem888_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``cem888 update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``cem888 update``. Never raises; lazy-install failures
