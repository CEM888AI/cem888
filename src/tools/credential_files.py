...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.cem888",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``CEM888_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside CEM888_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    cem888_home = _resolve_cem888_home()

    # Reject absolute paths — they bypass the CEM888_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to CEM888_HOME)",
            relative_path,
        )
        return False

    host_path = cem888_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape CEM888_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, cem888_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.cem888/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="cem888-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching cem888_constants.get_cem888_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_cem888_dir()`` for backward compatibility with old directory layouts.
    """
    from cem888_constants import get_cem888_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_cem888_dir(new_subpath, old_name)
        if host_dir.is_dir():
