...
`mcp_tool.py`, and `cem888_cli/mcp_config.py`. This module is the ONLY place
that instantiates the MCP SDK's `OAuthClientProvider` — all other code paths
go through `get_manager()`.

Design reference:

- CEM888 Code's ``invalidateOAuthCacheIfDiskChanged``
  (``cem888-code/src/utils/auth.ts:1320``, CC-1096 / GH#24317). Identical
...
class _ProviderEntry:
    ...
    Fields:
        ...
        pending_401: In-flight 401-handler futures keyed by the failed
            ...
            CEM888 Code's ``pending401Handlers`` map.
    """
...
# ---------------------------------------------------------------------------
# CEM888MCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_cem888_provider_class() -> Optional[type]:
    ...
    class CEM888MCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`CEM888TokenStorage` persisting an absolute
            ...
            from tools.mcp_oauth import CEM888TokenStorage
            if (
                isinstance(storage, CEM888TokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._cem888_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import CEM888TokenStorage
                        if isinstance(storage, CEM888TokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._cem888_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import CEM888TokenStorage
            if not isinstance(storage, CEM888TokenStorage):
                return
    ...
    return CEM888MCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_CEM888_PROVIDER_CLS: Optional[type] = _make_cem888_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`CEM888MCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _CEM888_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = CEM888TokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``cem888 mcp remove <name>`` and (indirectly) by
        ``cem888 mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
