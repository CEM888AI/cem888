...
CHECKPOINT_PATH = get_cem888_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_local(
        ...
        Args:
            use_pty: If True, use a pseudo-terminal via ptyprocess for interactive
                     CLI tools (Codex, CEM888 Code, Python REPL). Falls back to
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/cem888_bg_{session.id}.log"
        pid_path = f"{temp_dir}/cem888_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/cem888_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `cem888 update` restarting the gateway) is still
