...
One ``CDPSupervisor`` runs per CEM888 ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "cem888-dialog-bridge.invalid"
...
(() => {
  if (window.__cem888DialogBridgeInstalled) return;
  window.__cem888DialogBridgeInstalled = true;
  const ENDPOINT = "http://cem888-dialog-bridge.invalid/";
