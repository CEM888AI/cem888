...
def managed_cem888_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user has paid CEM888.AI Portal service access.
...
def cem888_tool_gateway_unavailable_message(
    capability: str = "the CEM888.AI Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable CEM888.AI Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `cem888 model` to refresh your "
        "CEM888.AI Portal login and billing status."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.cem888/.env`` (via
    ``cem888_cli.config.get_env_value`` when available) so tool-side
