...
# Must match cem888_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
DELEGATE_BLOCKED_TOOLS = frozenset(
    [
        ...
        "memory",  # no writes to the shared memory store
...
# (cem888-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("cem888-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``cem888-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on CEM888.AI Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. cem888-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. CEM888.AI Portal).
    ...
    # (e.g. MiniMax uses cem888ai_messages, DeepSeek uses chat_completions).
    ...
    # parent-level OpenRouter filters (e.g. `only=["CEM888.AI"]`) would
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.cem888/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if configured_base_url:
        ...
        # runtime resolver uses) so CEM888.AI-compatible direct endpoints with a
        # /cem888ai suffix — Azure AI Foundry, MiniMax, Zhipu GLM, LiteLLM
        ...
        # that only speak the CEM888.AI Messages protocol. Fixes #10213.
        ...
        elif base_url_hostname(configured_base_url) == "api.cem888ai.com":
            provider = "cem888ai"
        ...
        if configured_api_mode in {"chat_completions", "codex_responses", "cem888ai_messages"}:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'cem888 auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (cem888_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (CEM888 subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
