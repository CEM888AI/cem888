"""Central registry for all cem888-agent tools.
...
# Policy metadata belongs beside the tool registry, not in a profile prompt.
# The mapping is intentionally about operation semantics, never personality or
# prose.  Plugins can ask this registry which primitive is sanctioned for a
# resource class without carrying agent-specific tool-choice rules.
_OPERATION_CAPABILITIES = {
    "read_file": "read", "search_files": "read", "skill_view": "read",
    "write_file": "mutate", "file_write": "mutate", "patch": "mutate",
    "skill_manage": "mutate",
    "terminal": "shell", "shell": "shell", "bash": "shell", "execute_command": "shell",
}
_SANCTIONED_RESOURCE_ROUTES = {
    "profile_skill": {
        "read": {"tool": "read_file", "reason_code": "SANCTIONED_SKILL_READ"},
        "mutate": {"tool": "skill_manage", "reason_code": "SANCTIONED_SKILL_MUTATION"},
    },
}


def operation_capability(tool_name: str, args: Optional[dict] = None) -> str:
    """Return a stable operation class for runtime policy decisions."""
    del args  # reserved for tools whose action determines their capability
    return _OPERATION_CAPABILITIES.get(tool_name, "other")


def sanctioned_resource_route(resource_class: str, operation: str) -> Optional[dict]:
    """Return a copy of the registry-declared sanctioned route, if any."""
    route = _SANCTIONED_RESOURCE_ROUTES.get(resource_class, {}).get(operation)
...
# timescales. Cache results for ~30 s so env-var flips via ``cem888 tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``cem888 tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``cem888 tools enable foo``)
