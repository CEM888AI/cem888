...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``cem888 tools`` repaint, tool-registration scans,
    ...
    2. ``~/.cem888/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_cem888_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.cem888/.env`` first, then ``os.environ``.

    Wraps :func:`cem888_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from cem888_cli.config import get_env_value as _cem888_get_env_value

        value = _cem888_get_env_value(name)
...
def cem888_xai_user_agent() -> str:
    """Return a stable CEM888-specific User-Agent for xAI HTTP calls."""
    try:
        from cem888_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"CEM888-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers CEM888-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``cem888_cli.config.get_env_value`` so keys
    stored in ``~/.cem888/.env`` (the standard CEM888 location) are honored —
