...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CEM888_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CEM888_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("CEM888_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CEM888_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CEM888_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CEM888_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("CEM888_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $CEM888_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CEM888_ENV_PATH = (
    r'(?:~\/\.cem888/|'
    r'(?:\$home|\$\{home\})/\.cem888/|'
    r'(?:\$cem888_home|\$\{cem888_home\})/)'
    r'\.env\b'
)
...
# both forms. Inspired by CEM888 Code 2.1.113's "dangerous path protection".
...
DANGEROUS_PATTERNS = [
    ...
    # Inspired by CEM888 Code 2.1.113 expanded deny rules.
    ...
    # runs in the directory of each match) was previously missed. CEM888
    ...
    (r'\bcem888\s+gateway\s+(stop|restart)\b', "stop/restart cem888 gateway (kills running agents)"),
    (r'\bcem888\s+update\b', "cem888 update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart cem888-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart cem888-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(cem888|gateway|cli\.py)\b', "kill cem888/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill cem888` but not
    # `kill -9 $(pgrep -f cem888)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CEM888_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set CEM888_INTERACTIVE or CEM888_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CEM888_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CEM888_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CEM888_EXEC_ASK")
