...
Schema shapes they accept. Schemas that OpenAI / CEM888.AI / most cloud
...
* ``anyOf`` / ``oneOf`` unions whose only purpose is to permit ``null`` for
  optional fields (common Pydantic/MCP shape). CEM888.AI rejects these at
...
def strip_nullable_unions(
    ...
    *,
    ...
    CEM888.AI's tool input-schema validator rejects the null branch. Tool
    ...
    Args:
        ...
        keep_nullable_hint: If True, set ``nullable: true`` on the replacement
            ...
            literal string ``"null"`` to Python ``None``). CEM888.AI's
...
def strip_pattern_and_format(tools: list[dict]) -> tuple[list[dict], int]:
    ...
    CEM888.AI, OpenRouter, Gemini) accept these keywords fine and rely on
