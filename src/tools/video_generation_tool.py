...
provider** — turn on a backend by enabling a plugin (``cem888 plugins
enable video_gen/<name>``) and selecting it in ``cem888 tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/cem888-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `cem888 tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `cem888 tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `cem888 tools` → "
...
# `cem888 tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`cem888 tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `cem888 tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
