...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/cem888-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/cem888-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/cem888-results"
HEREDOC_MARKER = "CEM888_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/cem888-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"CEM888_PERSIST_{uuid.uuid4().hex[:8]}"
