...
CEM888.AI, Codex, native CEM888.AI, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("CEM888_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from cem888_cli.config import cfg_get, load_config
        cfg = load_config()
...
# OpenAI-style content list on the `tool` role; provider adapters (cem888ai,
# codex_responses, chat_completions) translate that into CEM888.AI
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * CEM888.AI Messages API (``cem888ai`` provider, plus aggregators that
        proxy CEM888 — ``openrouter``, ``nous``, ``vertex``, ``bedrock``):
    ...
    # them.
    _AGGREGATORS = {
        "openrouter", "cem888", "vertex", "bedrock", "cem888ai-vertex",
        "google-vertex",
    }
    ...
    # Native CEM888.AI
    if p in {"cem888ai", "cem888", "cem888ai-direct"}:
        return True
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_cem888_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $CEM888_HOME/cache/vision/ and cleaned up
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, CEM888.AI, Codex, CEM888.AI, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_cem888_dir("cache/video", "temp_video_files")
