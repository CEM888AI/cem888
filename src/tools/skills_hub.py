#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the CEM888 Skills Hub.
...
Used by cem888_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class SkillMeta:
    ...
    name: str
    description: str
    source: str           # "official", "github", "clawhub", "cem888-marketplace", "lobehub"
...
class GitHubSource(SkillSource):
    ...
    DEFAULT_TAPS = [
        ...
        {"repo": "cem888ais/skills", "path": "skills/"},
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            cem888_meta = metadata.get("cem888", {})
            if isinstance(cem888_meta, dict):
                tags = cem888_meta.get("tags", [])
        if not tags:
...
class SkillsShSource(SkillSource):
    ...
    def _discover_identifier(self, identifier: str, detail: Optional[dict] = None) -> Optional[str]:
        ...
        base_paths = ["skills/", ".agents/skills/", ".cem888/skills/"]
        ...
        # repos like borghei/cem888-skills.
            ...
            if resp.status_code == 200:
                ...
                if isinstance(entries, list):
                    for entry in entries:
                        ...
                        if dir_name.startswith((".", "_")):
                            continue
                        if dir_name in {"skills", ".agents", ".cem888"}:
    ...
    def _candidate_identifiers(identifier: str) -> List[str]:
        ...
        candidates = [
            ...
            f"{repo}/.cem888/skills/{skill_path}",
        ]
...
# ---------------------------------------------------------------------------
# CEM888 Code marketplace source adapter
# ---------------------------------------------------------------------------

class CEM888MarketplaceSource(SkillSource):
    """
    Discover skills from CEM888 Code marketplace repos.
    Marketplace repos contain .cem888-plugin/marketplace.json with plugin listings.
    """
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if bundle:
            bundle.source = "cem888-marketplace"
        return bundle
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if meta:
            meta.source = "cem888-marketplace"
            meta.trust_level = self.trust_level_for(identifier)
        return meta

    def _fetch_marketplace_index(self, repo: str) -> List[dict]:
        """Fetch and parse .cem888-plugin/marketplace.json from a repo."""
        cache_key = f"cem888_marketplace_{repo.replace('/', '_')}"
        cached = _read_index_cache(cache_key)
        if cached is not None:
            return cached

        url = f"https://api.github.com/repos/{repo}/contents/.cem888-plugin/marketplace.json"
        try:
            resp = httpx.get(
                url,
                headers={**self.auth.get_headers(), "Accept": "application/vnd.github.v3.raw"},
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by CEM888.AI) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.cem888/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                cem888_meta = meta_block.get("cem888", {})
                if isinstance(cem888_meta, dict):
                    tags = cem888_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# CEM888 centralized index source
# ---------------------------------------------------------------------------

CEM888_INDEX_URL = "https://cem888.ai/docs/api/skills-index.json"
CEM888_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "cem888-index.json"
CEM888_INDEX_TTL = 6 * 3600  # 6 hours


def _load_cem888_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CEM888_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if CEM888_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - CEM888_INDEX_CACHE_FILE.stat().st_mtime
            if age < CEM888_INDEX_TTL:
                return json.loads(CEM888_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(CEM888_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("CEM888 index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("CEM888 index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        CEM888_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        CEM888_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class CEM888IndexSource(SkillSource):
    """Skill source backed by the centralized CEM888 Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_cem888_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "cem888-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "cem888-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        CEM888IndexSource(auth=auth), # Centralized index (search + resolved install paths)
        SkillsShSource(auth=auth),
        WellKnownSkillSource(),
        UrlSource(),                  # Direct HTTP(S) URL to a SKILL.md file
        GitHubSource(auth=auth, extra_taps=extra_taps),
        ClawHubSource(),
        CEM888MarketplaceSource(auth=auth),
        LobeHubSource(),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    _api_source_ids = frozenset({"github", "skills-sh", "clawhub",
                                  "cem888-marketplace", "lobehub", "well-known"})
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "cem888-index"
