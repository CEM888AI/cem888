...
_FORCE_SYNC_ENV = "CEM888_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.cem888") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.cem888 to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``CEM888_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, cem888_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.cem888/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="cem888-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.cem888/skills/a.md`` →
        ``~/.cem888/skills/a.md``, a new remote file at
        ``/root/.cem888/skills/b.md`` maps to ``~/.cem888/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
