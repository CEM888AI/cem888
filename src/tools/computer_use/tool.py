...
computer-use primitive. Replaces #4562's CEM888.AI-native `computer_20251124`
...
For captures / actions with `capture_after=True`:
  ...
  list-shaped `content` for OpenAI-compatible providers. The CEM888.AI
  adapter splices the base64 image into a `tool_result` block (see
  `agent/cem888ai_adapter.py`). Every provider that supports multi-part
...
# regardless of approval level (e.g. logout kills the session CEM888 runs in).
_BLOCKED_KEY_COMBOS = {
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("CEM888_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown CEM888_COMPUTER_USE_BACKEND={backend_name!r}")
            _backend.start()
        return _backend
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "Run `cem888 tools` and enable Computer Use to install cua-driver.",
        })
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$CEM888_HOME/cache/vision/``
    ...
    try:
        ...
        cache_dir = get_cem888_dir("cache/vision", "temp_vision_images")
