...
can break on OS updates. Pin the installed version via `CEM888_CUA_DRIVER_
...
PINNED_CUA_DRIVER_VERSION = os.environ.get("CEM888_CUA_DRIVER_VERSION", "0.5.0")

_CUA_DRIVER_CMD = os.environ.get("CEM888_CUA_DRIVER_CMD", "cua-driver")
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or CEM888_CUA_DRIVER_CMD resolves."""
...
def cua_driver_install_hint() -> str:
    return (
        ...
        "  cem888 computer-use install\n"
        "Or run the upstream installer directly:\n"
        '  /bin/bash -c "$(curl -fsSL '
        'https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh)"\n'
        "Or run `cem888 tools` and enable the Computer Use toolset to install it automatically."
    )
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps cem888 `capture(mode, app)` → cua-driver `list_windows` +
