...
This module provides the glue:
    - ``CEM888TokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "CEM888 Agent"
"""
...
from cem888_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__cem888_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses CEM888_HOME so each profile gets its own OAuth tokens.
    Layout: ``CEM888_HOME/mcp-tokens/``
    """
    try:
        from cem888_constants import get_cem888_home
        base = Path(get_cem888_home())
    except ImportError:
        base = Path(os.environ.get("CEM888_HOME", str(Path.home() / ".cem888")))
...
# ---------------------------------------------------------------------------
# CEM888TokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class CEM888TokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CEM888_HOME/mcp-tokens/<server_name>.json         -- tokens
        CEM888_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CEM888_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # CEM888 records an absolute wall-clock ``expires_at`` alongside the
    ...
    async def set_tokens(self, tokens: "OAuthToken") -> None:
        ...
        # ``mcp-oauth-token-diagnosis`` skill + CEM888 Code's
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to CEM888.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://cem888.ai/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `cem888 mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "CEM888 Agent")
    scope = cfg.get("scope")
