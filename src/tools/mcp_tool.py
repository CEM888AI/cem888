...
transport, discovers their tools, and registers them into the cem888-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.cem888/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.cem888/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            cem888_home = os.path.expanduser(
                os.getenv(
                    "CEM888_HOME", os.path.join(os.path.expanduser("~"), ".cem888")
                )
            )
            candidates = [
                os.path.join(cem888_home, "node", "bin", resolved_command),
                ...
                # image (which the CEM888 Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → CEM888 MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that CEM888 gateways know how to render.
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'cem888-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'cem888-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until CEM888 is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `cem888 mcp login {server_name}` (or delete the tokens "
            f"file under ~/.cem888/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the CEM888 config file.
    ...
    ``os.environ`` (which includes ``~/.cem888/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through CEM888' MEDIA: tag convention and out to messaging
            ...
            # CEM888' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * MCP/Pydantic optional fields commonly arrive as
      ``anyOf: [{...}, {"type": "null"}], default: null``.  CEM888.AI rejects
    ...
    OpenAI, CEM888.AI, Gemini, and Moonshot in one pass.
    """
    if not schema:
    ...
    def _strip_nullable_union(node):
        ...
        ingestion, the CEM888.AI guard, and the global sanitizer all share one
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves CEM888's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the CEM888 registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``cem888 tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the CEM888 registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple cem888
