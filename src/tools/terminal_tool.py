...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or CEM888 exit
...
# display_cem888_home imported lazily at call site (stale-module safety during cem888 update)
...
from tools.tool_backend_helpers import (
    ...
    cem888_tool_gateway_unavailable_message,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of cem888 directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "cem888-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("CEM888_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("CEM888_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("CEM888_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (CEM888_INTERACTIVE=1).
    ...
    try:
        os.environ["CEM888_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "CEM888_SPINNER_PAUSE" in os.environ:
            del os.environ["CEM888_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (CEM888_INTERACTIVE=1):
    ...
    # interactive CEM888 password prompt or the sudo -S password-pipe path.
    ...
    # silently without CEM888 prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("CEM888_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so CEM888 can track lifecycle and output.
After starting a server, verify readiness with a health check or log signal, then run tests in a separate terminal() call. Avoid blind sleep loops.
Use process(action="poll") for progress checks, process(action="wait") to block until done.
Working directory: Use 'workdir' for per-command cwd.
PTY mode: Set pty=true for interactive CLI tools (Codex, CEM888 Code, Python REPL).
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, CEM888SweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.cem888/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "CEM888.AI Tool Gateway access is not currently available and no direct "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so CEM888 can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Tier-0 authority cannot be bypassed with force=True. It runs before
        # the ordinary dangerous-command approval system and blocks terminal
        # equivalents of denied file-tool mutations.
        try:
            from agent.authority import load_agent_authority

            _authority = load_agent_authority()
            _authority_denial = (
                _authority.command_denial(command, workdir=cwd)
                if _authority is not None
                else None
            )
        except Exception as exc:
            _authority_denial = f"AUTHORITY_UNAVAILABLE: terminal authority could not be resolved: {exc}"
        if _authority_denial:
            return json.dumps({
                "output": "",
                "exit_code": -1,
                "error": _authority_denial,
                "status": "blocked",
                "reason_code": "TARGET_AUTHORITY_MISMATCH",
            }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # cem888-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in cem888-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("CEM888_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("CEM888_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("CEM888_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("CEM888_SESSION_USER_ID", "")
                        _gw_user_name = _gse("CEM888_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("CEM888_SESSION_MESSAGE_ID", "")
            ...
            try:
                from cem888_cli.plugins import invoke_hook
...
TERMINAL_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "pty": {
                "type": "boolean",
                "description": "Run in pseudo-terminal (PTY) mode for interactive CLI tools like Codex, CEM888 Code, or Python REPL. Only works with local and SSH backends. Default: false.",
                "default": False
            },
            "notify_on_complete": {
                "type": "boolean",
