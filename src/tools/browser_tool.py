...
supports multiple backends — **Browser Use** (cloud, default for CEM888.AI
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    cem888_node_bin = str(cem888_home / "node" / "bin")
    cem888_node_root = str(cem888_home / "node")
    cem888_nm_bin = str(cem888_home / "node_modules" / ".bin")
    return [cem888_node_bin, cem888_node_root, cem888_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.cem888/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed CEM888.AI gateway or
    ...
    browser plugins (``~/.cem888/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed CEM888.AI gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when CEM888 silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the CEM888 CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-cem888_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed cem888 processes — this way every clean cem888 exit sweeps
    ...
    # Sweep orphans from other crashed cem888 processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live cem888 processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current cem888 PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live cem888 process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning cem888 process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced cem888 PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent cem888 instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-cem888_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live cem888 process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, CEM888-managed
    ...
    # Build an extended search PATH including CEM888-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                cem888_nm = str(get_cem888_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=cem888_nm)
            if not recheck:
                cem888_node_bin = str(get_cem888_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=cem888_node_bin)
            if not recheck:
                cem888_node_root = str(get_cem888_home() / "node")
                recheck = shutil.which("agent-browser", path=cem888_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this cem888 PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = cem888_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from cem888_constants import get_cem888_dir
    screenshots_dir = get_cem888_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/cem888ai/cem888-agent:latest")
                else:
