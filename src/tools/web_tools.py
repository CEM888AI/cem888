...
Backend is selected during ``cem888 tools`` setup (web.backend in config.yaml).
When available, CEM888 can route Firecrawl calls through a CEM888.AI-hosted tool-gateway
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for CEM888.AI Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    read_cem888_access_token as _read_cem888_access_token,
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.cem888/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``cem888 tools``).
    ...
    # tool gateway is configured for CEM888.AI subscribers.
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `cem888 tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_cem888_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the CEM888.AI portal on every CLI startup (invoked at
...
def _is_cem888_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is CEM888.AI Portal."""
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_cem888_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        from agent.portal_tags import cem888_portal_tags
        extra_body = get_auxiliary_extra_body() or {"tags": cem888_portal_tags()}
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `cem888 tools` to set one up."
                ),
            }
        else:
            logger.info(
...
if __name__ == "__main__":
    ...
    cem888_available = check_auxiliary_model()
    ...
    if not cem888_available:
        print("❌ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure CEM888.AI Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
