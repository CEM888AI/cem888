...
it is automatically downloaded from GitHub releases to $CEM888_HOME/bin/tirith.
...
def _get_cem888_home() -> str:
    """Return the CEM888 home directory, respecting CEM888_HOME env var."""
    return str(get_cem888_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_cem888_home(), ".tirith-install-failed")
...
def _cem888_bin_dir() -> str:
    """Return $CEM888_HOME/bin, creating it if needed."""
    d = os.path.join(_get_cem888_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CEM888_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_cem888_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CEM888_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CEM888_HOME/bin/tirith
    ...
    cem888_bin = os.path.join(_cem888_bin_dir(), "tirith")
    if os.path.isfile(cem888_bin) and os.access(cem888_bin, os.X_OK):
        _resolved_path = cem888_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return cem888_bin
