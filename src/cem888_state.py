#!/usr/bin/env python3
"""
SQLite State Store for CEM888 Agent.
...
def get_default_db_path() -> Path:
    ...
    The module historically captured ``DEFAULT_DB_PATH = get_cem888_home() /
    "state.db"`` once, at import.  That froze the profile selection for the
    life of the process: a later ``CEM888_HOME`` change — or a context-local
    ``set_cem888_home_override()`` — could not move ``SessionDB()``, so two
    agents in one process shared a single state.db and a single FTS5 index.
    Resolve on every call instead.
    """
    return get_cem888_home() / "state.db"


def __getattr__(name: str):
    """PEP 562: keep the historical ``DEFAULT_DB_PATH`` name resolvable.

    ``from cem888_state import DEFAULT_DB_PATH`` keeps working, but each access
    returns the CURRENT profile's path instead of a value frozen at import.
    """
...
# cem888_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``cem888_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see cem888_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple cem888 processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        # Resolve at construction.  An explicit db_path always wins; otherwise
        # resolve the profile home NOW so a profile/home switch is honoured
        # instead of reading a value frozen at import time.
        ...
        try:
            ...
            # ``cem888_state._set_last_init_error(None)`` explicitly.
    ...
    def _init_schema(self):
        ...
        # Trigram FTS5 for CJK/substring search. Left unbuilt here: the
        # trigram table + triggers + backfill cost ~106MB and 2x write
        # amplification per message for a corpus that is overwhelmingly
        # non-CJK. It is materialized lazily by `_ensure_trigram()` on the
        # first actual CJK query, so non-CJK profiles never pay for it.
        # (See LazyTrigram in search_messages — feature-behavior preserved:
        # any profile that searches CJK still builds the trigram index on
        # demand, exactly once.)
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, CEM888.AI) receive
    ...
    @staticmethod
    def _fts_table_exists(cursor, name: str) -> bool:
        """Return True if an FTS5 virtual table already exists in the DB."""
        try:
    ...
    @staticmethod
    def _backfill_trigram(cursor) -> None:
        """One-time backfill of existing messages rows into the trigram index."""
        cursor.execute(
            "INSERT INTO messages_fts_trigram(rowid, content) "
    ...
    def _ensure_trigram(self) -> None:
        """Lazily materialize the CJK trigram FTS5 index (table + triggers).

        Called only when an actual CJK query reaches the trigram path, so a
        non-CJK profile never builds or maintains the ~106MB index (and never
        pays the per-write 2x amplification). Idempotent and safe under
        concurrency: the existence probe runs inside the write lock, so two
        threads that hit CJK simultaneously converge on one build.
        """
        with self._lock:
            if self._fts_table_exists(self._conn, "messages_fts_trigram"):
                return
            self._conn.executescript(FTS_TRIGRAM_SQL)
            self._backfill_trigram(self._conn)
            self._conn.commit()
    ...
    def search_messages(
        ...
        if is_cjk:
            ...
            if cjk_count >= 3 and not _any_short_cjk:
                ...
                # Build the trigram index on demand (first CJK query only).
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade CEM888, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one CEM888 session.

        A CEM888 session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a CEM888 session is already bound to any Telegram DM topic.
