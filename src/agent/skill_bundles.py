...
Storage
-------
Bundles live in ``~/.cem888/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CEM888_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``cem888 bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CEM888_HOME.

    Honors ``CEM888_BUNDLES_DIR`` for tests; falls back to
    ``<CEM888_HOME>/skill-bundles``.
    """
    override = os.environ.get("CEM888_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_cem888_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `cem888 bundles` CLI subcommand.
# ---------------------------------------------------------------------------
