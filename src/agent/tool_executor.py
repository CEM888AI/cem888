...
def _notify_post_tool_call(tool_name: str, args: dict, result: Any, *, task_id: str,
                           session_id: str, tool_call_id: str, duration_ms: int) -> None:
    """Deliver the authoritative result to observer/enforcement plugins.

    Pre-tool hooks have always run in this executor.  Keeping the matching
    post hook here makes result-backed controls possible without teaching
    individual tools about CEM888 policy.
    """
    try:
        from cem888_cli.plugins import invoke_hook
        invoke_hook(
            "post_tool_call", tool_name=tool_name, args=args, result=result,
            task_id=task_id, session_id=session_id, tool_call_id=tool_call_id,
            duration_ms=duration_ms,
        )
    except Exception:
        logger.debug("post_tool_call hook failed for %s", tool_name, exc_info=True)
...
def _emit_tool_telemetry(
    ...
    *,
    ...
    """Emit outcome metadata only; never accepts tool arguments or results."""
    try:
        from agent.inference_telemetry import emit, opaque_id

        emit(
            "tool_completed",
            call_id=opaque_id(tool_call_id),
            tool_name=tool_name,
            outcome=outcome,
        ...
        logger.debug("tool telemetry failed", exc_info=True)
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, (tc, name, args, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
        ...
        if r is None:
            ...
            tool_duration = 0.0
            _telemetry_error = True
            _telemetry_outcome = "interrupted" if agent._interrupt_requested else "error"
        else:
            function_name, function_args, function_result, tool_duration, is_error, blocked = r
            _telemetry_error = bool(is_error)
            _telemetry_outcome = "blocked" if blocked else ("error" if is_error else "success")
            ...
            if not blocked:
                ...
                _notify_post_tool_call(
                    function_name, function_args, function_result,
                    task_id=effective_task_id or "", session_id=agent.session_id or "",
                    tool_call_id=getattr(tc, "id", ""), duration_ms=int(tool_duration * 1000),
                )
        ...
        # rather than a raw Python dict.  The CEM888.AI adapter already
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if not _execution_blocked:
            ...
            _notify_post_tool_call(
                function_name, function_args, function_result,
                task_id=effective_task_id or "", session_id=agent.session_id or "",
                tool_call_id=getattr(tool_call, "id", ""), duration_ms=int(tool_duration * 1000),
            )
