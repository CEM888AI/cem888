...
A pre-run snapshot of ``~/.cem888/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.cem888/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.cem888/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_cem888_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.cem888/cron/jobs.json``)."""
    return get_cem888_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.cem888/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.cem888/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.cem888/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.cem888/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `cem888 curator rollback --list` to see available snapshots)",
            None,
        )
