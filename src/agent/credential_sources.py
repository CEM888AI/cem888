"""Unified removal contract for every credential source CEM888 reads from.

CEM888 seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.cem888/.env
    cem888_code   — ~/.cem888/.credentials.json
    cem888_pkce   — ~/.cem888/.cem888ai_oauth.json
    device_code   — auth.json providers.<provider> (cem888, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `cem888 auth add`
...
unify here is **removal**:
    ...
    ``cem888 auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, cem888 device_code (partial), cem888_pkce, copilot gh_cli, and
...
class RemovalResult:
    ...
    Attributes:
        ...
        hints: Diagnostic lines ABOUT state the user may need to clean up
            ...
            var, CEM888 Code credential file we don't delete, etc.).
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"cem888ai"``, ``"cem888"``, ...).
        ...
        source_id: Source identifier as it appears in
            ``PooledCredential.source``.  May be a literal (``"cem888_code"``)
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.cem888/.env  → clear it
    ...
    """
    from cem888_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to CEM888.",
            f"  The pool entry is now suppressed — CEM888 will ignore "
            f"{env_var} until you run `cem888 auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_cem888_code(provider: str, removed) -> RemovalResult:
    """~/.cem888/.credentials.json is owned by CEM888 Code itself.

    We don't delete it — the user's CEM888 Code install still needs to
    work.  We just suppress it so CEM888 stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed cem888_code credential — it will not be re-seeded.",
        "Note: CEM888 Code credentials still live in ~/.cem888/.credentials.json",
        "Run `cem888 auth add cem888ai` to re-enable if needed.",
    ])


def _remove_cem888_pkce(provider: str, removed) -> RemovalResult:
    """~/.cem888/.cem888ai_oauth.json is ours — delete it outright."""
    from cem888_constants import get_cem888_home

    result = RemovalResult()
    oauth_file = get_cem888_home() / ".cem888ai_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared CEM888 CEM888.AI OAuth credentials")
...
def _remove_cem888_device_code(provider: str, removed) -> RemovalResult:
    """CEM888.AI OAuth lives in auth.json providers.cem888 — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `cem888 auth add cem888` (or any other path that writes providers.cem888)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `cem888 auth add cem888` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as CEM888.AI: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``cem888 auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``cem888 auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `cem888 model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the CEM888 auth store is not enough — _seed_from_singletons() would
    ...
    via ``cem888 auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from cem888_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `cem888 auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as cem888_code — suppress, don't delete.  The user's
    ...
    return RemovalResult(hints=[
        ...
        "Run `cem888 auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    CEM888 stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `cem888 auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="cem888ai", source_id="cem888_code",
        remove_fn=_remove_cem888_code,
        description="~/.cem888/.credentials.json",
    ))
    register(RemovalStep(
        provider="cem888ai", source_id="cem888_pkce",
        remove_fn=_remove_cem888_pkce,
        description="~/.cem888/.cem888ai_oauth.json",
    ))
    register(RemovalStep(
        provider="cem888", source_id="device_code",
        remove_fn=_remove_cem888_device_code,
        description="auth.json providers.cem888",
    ))
    register(RemovalStep(
