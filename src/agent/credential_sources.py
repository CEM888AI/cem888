...
qwen-cli, nous device_code (partial), cem888_pkce, copilot gh_cli, and
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    """CEM888.AI OAuth lives in auth.json providers.nous — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `cem888 auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `cem888 auth add nous` to re-engage, which is the
