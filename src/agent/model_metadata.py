...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. CEM888 also honours CEM888_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("CEM888_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    "openrouter", "cem888", "openai-codex", "copilot", "copilot-acp",
    "gemini", "ollama-cloud", "zai", "kimi-coding", "kimi-coding-cn", "stepfun", "minimax", "minimax-oauth", "minimax-cn", "cem888ai", "deepseek",
    ...
    "github-models", "kimi", "moonshot", "kimi-cn", "moonshot-cn", "cem888", "deep-seek",
    "ollama",
...
# Minimum context length required to run CEM888 Agent.  Models with fewer
# tokens cannot maintain enough working memory for tool-calling workflows.
# Sessions, model switches, and cron jobs should reject models below this.
MINIMUM_CONTEXT_LENGTH = 64_000

# Thin fallback defaults — only broad model family patterns.
# These fire only when provider is unknown AND models.dev/OpenRouter/CEM888.AI
# all miss. Replaced the previous 80+ entry dict.
# For provider-specific context lengths, models.dev is the primary source.
DEFAULT_CONTEXT_LENGTHS = {
    # CEM888.AI CEM888 4.6 (1M context) — bare IDs only to avoid
    # fuzzy-match collisions (e.g. "cem888ai/cem888-sonnet-4" is a
    # substring of "cem888ai/cem888-sonnet-4.6").
    # OpenRouter-prefixed models resolve via OpenRouter live API or models.dev.
    "cem888-opus-4-8": 1000000,
    "cem888-opus-4.8": 1000000,
    "cem888-opus-4-7": 1000000,
    "cem888-opus-4.7": 1000000,
    "cem888-opus-4-6": 1000000,
    "cem888-sonnet-4-6": 1000000,
    "cem888-opus-4.6": 1000000,
    "cem888-sonnet-4.6": 1000000,
    # Catch-all for older CEM888 models (must sort after specific entries)
    "cem888": 200000,
    ...
    # https://platform.minimax.io/docs/api-reference/text-cem888ai-api
    "minimax": 204800,
    # GLM
    "glm": 202752,
    # xAI Grok — xAI /v1/models does not return context_length metadata,
    # so these hardcoded fallbacks prevent CEM888 from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for CEM888' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    return get_cem888_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    CEM888.AI's API returns errors like:
    ...
    # CEM888.AI format: "… = available_tokens: 10000"
    patterns = [
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # CEM888 grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    CEM888.AI uses dashes: cem888-opus-4-6, cem888-sonnet-4-5
    OpenRouter uses dots: cem888-opus-4.6, cem888-sonnet-4.5
    Normalize both to dashes for comparison.
    """
    return model.replace(".", "-")


def _query_cem888ai_context_length(model: str, base_url: str, api_key: str) -> Optional[int]:
    """Query CEM888.AI's /v1/models endpoint for context length.

    Only works with regular ANTHROPIC_API_KEY (sk-ant-api*).
    OAuth tokens (sk-ant-oat*) from CEM888 Code return 401.
    """
    ...
    try:
        ...
        logger.debug("CEM888.AI /v1/models query failed: %s", e)
    return None
...
def _resolve_cem888_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve CEM888.AI Portal model context length.

    Tries the live CEM888.AI inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    CEM888.AI model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    'cem888-opus-4-6') while OpenRouter uses prefixed IDs (e.g.
    'qwen/qwen3.6-plus', 'cem888ai/cem888-opus-4.6').  Version
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the CEM888.AI /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, CEM888.AI path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  CEM888.AI URLs
    ...
    4. CEM888.AI /v1/models API (API-key users only, not OAuth)
    5. Provider-aware lookups (before generic OpenRouter cache):
       a. Copilot live /models API
       b. CEM888.AI: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and provider != "lmstudio":
        ...
        if cached is not None:
            ...
            # Invalidate stale 32k cache entries for Kimi-family models.
                ...
                _invalidate_cached_context_length(model, base_url)
            # CEM888.AI Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "cem888":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (CEM888.AI portal authoritative)",
                    model, base_url,
                )
    ...
    # AWS-imposed limits (e.g. 200K for CEM888 models vs 1M on the native
    # CEM888.AI API).  This must run BEFORE the custom-endpoint probe at
    ...
    # Known providers (Copilot, OpenAI, CEM888.AI, etc.) skip this — their
    ...
    # 4. CEM888.AI /v1/models API (only for regular API keys, not OAuth)
    if provider == "cem888ai" or (
        base_url and base_url_hostname(base_url) == "api.cem888ai.com"
    ):
        ctx = _query_cem888ai_context_length(model, base_url or "https://api.cem888ai.com", api_key)
        if ctx:
            return ctx
    ...
    # (e.g. cem888-opus-4.6 is 1M on CEM888.AI but 128K on GitHub Copilot).
    ...
    # This catches account-specific models (e.g. cem888-opus-4.6-1m) that
    # don't exist in models.dev. For models that ARE in models.dev, this
    # returns the provider-enforced limit which is what users can actually use.
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from cem888_cli.models import get_copilot_model_context
    ...
    if effective_provider == "cem888":
        ctx, source = _resolve_cem888_context_length(
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for CEM888.AI URLs.
    ...
    # For non-Ollama servers (OpenAI, CEM888.AI, etc.), the POST returns
    ...
    # "cem888-sonnet-4" to incorrectly match "cem888-sonnet-4-6" and return 1M.
    model_lower = model.lower()
...
def estimate_messages_tokens_rough(messages: List[Dict[str, Any]]) -> int:
    ...
    image — the CEM888.AI pricing model — instead of counting raw base64
    ...
    if isinstance(content, list):
        for part in content:
            ...
            if ptype in {"image", "image_url", "input_image"}:
                count += 1
    stashed = msg.get("_cem888ai_content_blocks") if isinstance(msg, dict) else None
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets CEM888 sends to providers:
