...
# ``_nous_extra_body()`` or import ``cem888_portal_tags`` directly.
...
def _nous_base_url() -> str:
    ...
    return os.getenv("NOUS_INFERENCE_BASE_URL", _CEM888_DEFAULT_BASE_URL)
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        creds = resolve_nous_runtime_credentials(
            min_key_ttl_seconds=max(60, int(os.getenv("CEM888_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
            timeout_seconds=float(os.getenv("CEM888_NOUS_TIMEOUT_SECONDS", "15")),
            inference_auth_mode=(
                NOUS_INFERENCE_AUTH_MODE_LEGACY
                if force_refresh
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from cem888_cli.models import get_nous_recommended_aux_model
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_cem888ai`` / ``_try_nous`` shape but delegates to
...
def _build_call_kwargs(
    ...
    if provider == "cem888" or auxiliary_is_nous:
