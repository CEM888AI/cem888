...
Resolution order for text tasks (auto mode):
  1. User's main provider + main model (used regardless of provider type —
     aggregators, direct API-key providers, native CEM888.AI, Codex, etc.)
  2. OpenRouter  (OPENROUTER_API_KEY)
  3. CEM888.AI Portal (~/.cem888/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before CEM888 triggers summarization.  Higher values delay
...
# plus providers we intentionally keep pinned here (e.g. CEM888.AI predates
...
_API_KEY_PROVIDER_AUX_MODELS_FALLBACK: Dict[str, str] = {
    ...
    "cem888ai": "cem888-haiku-4-5-20251001",
...
# api.kimi.com/coding (CEM888.AI Messages wire) which Kimi's own docs
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://cem888.ai",
    "X-Title": "CEM888 Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``CEM888_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``CEM888_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("CEM888_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("CEM888_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "CEM888Agent",
}
...
# CEM888.AI Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by CEM888.AI Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with cem888_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import cem888_portal_tags as _portal_tags


def _cem888_extra_body() -> dict:
    """Return a fresh CEM888.AI Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``cem888_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``CEM888_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_cem888_extra_body()`` or import ``cem888_portal_tags`` directly.
CEM888_EXTRA_BODY = _cem888_extra_body()

# Set at resolve time — True if the auxiliary client points to CEM888.AI Portal
auxiliary_is_cem888: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_CEM888_MODEL = "google/gemini-3-flash-preview"
_CEM888_DEFAULT_BASE_URL = "https://api.cem888ai.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.cem888ai.com"
_AUTH_JSON_PATH = get_cem888_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (CEM888 Agent)",
...
def _to_openai_base_url(base_url: str) -> str:
    """Normalize an CEM888.AI-style base URL to OpenAI-compatible format.

    Some providers (MiniMax, MiniMax-CN) expose an ``/cem888ai`` endpoint for
    the CEM888.AI Messages API and a separate ``/v1`` endpoint for OpenAI chat
    ...
    land on ``/cem888ai/chat/completions`` — a 404.
    """
    url = str(base_url or "").strip().rstrip("/")
    if url.endswith("/cem888ai"):
        # ZAI (open.bigmodel.cn) uses /api/cem888ai for CEM888.AI wire
        # but /api/paas/v4 for OpenAI wire — the generic /v1 rewrite is wrong.
        if "open.bigmodel.cn" in url or "bigmodel" in url:
            rewritten = url[: -len("/cem888ai")] + "/paas/v4"
            logger.debug("Auxiliary client: rewrote ZAI base URL %s → %s", url, rewritten)
            return rewritten
        rewritten = url[: -len("/cem888ai")] + "/v1"
        logger.debug("Auxiliary client: rewrote base URL %s → %s", url, rewritten)
        return rewritten
    if "api.kimi.com" in url and url.endswith("/coding"):
        # Kimi Code uses /coding/v1/messages for CEM888.AI SDK (appends /v1/messages)
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for cem888).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. cem888 prefers inference_base_url).
...
class _CEM888AICompletionsAdapter:
    """OpenAI-client-compatible adapter for CEM888.AI Messages API."""
    ...
    def create(self, **kwargs) -> Any:
        from agent.cem888ai_adapter import build_cem888ai_kwargs
        ...
        # ZAI's CEM888.AI-compatible endpoint rejects max_tokens on vision
        ...
        cem888ai_kwargs = build_cem888ai_kwargs(
            model=model,
            messages=messages,
            tools=tools,
        ...
        # temperature for models that still accept it. build_cem888ai_kwargs
        # additionally strips these keys as a safety net — keep both layers.
        if temperature is not None:
            from agent.cem888ai_adapter import _forbids_sampling_params
            if not _forbids_sampling_params(model):
                cem888ai_kwargs["temperature"] = temperature

        response = self._client.messages.create(**cem888ai_kwargs)
        _transport = get_transport("cem888ai_messages")
...
class CEM888AIChatShim:
    def __init__(self, adapter: _CEM888AICompletionsAdapter):
        self.completions = adapter


class CEM888AIAuxiliaryClient:
    """OpenAI-client-compatible wrapper over a native CEM888.AI client."""

    def __init__(self, real_client: Any, model: str, api_key: str, base_url: str, is_oauth: bool = False):
        self._real_client = real_client
        adapter = _CEM888AICompletionsAdapter(real_client, model, is_oauth=is_oauth)
        self.chat = CEM888AIChatShim(adapter)
...
class _AsyncCEM888AICompletionsAdapter:
    def __init__(self, sync_adapter: _CEM888AICompletionsAdapter):
        self._sync = sync_adapter
...
class _AsyncCEM888AIChatShim:
    def __init__(self, adapter: _AsyncCEM888AICompletionsAdapter):
        self.completions = adapter


class AsyncCEM888AIAuxiliaryClient:
    def __init__(self, sync_wrapper: "CEM888AIAuxiliaryClient"):
        sync_adapter = sync_wrapper.chat.completions
        async_adapter = _AsyncCEM888AICompletionsAdapter(sync_adapter)
        self.chat = _AsyncCEM888AIChatShim(async_adapter)
        self.api_key = sync_wrapper.api_key
        self.base_url = sync_wrapper.base_url
...
def _endpoint_speaks_cem888ai_messages(base_url: str) -> bool:
    """True if the endpoint at ``base_url`` speaks the CEM888.AI Messages
    protocol instead of OpenAI chat.completions.

    Mirrors ``cem888_cli.runtime_provider._detect_api_mode_for_url`` so the
    auxiliary client and the main agent stay in sync on transport selection.
    Covers:

    - Any URL ending in ``/cem888ai`` (MiniMax, Zhipu GLM, LiteLLM proxies,
      CEM888.AI-compatible gateways).
    - ``api.kimi.com/coding`` (Kimi Coding Plan — the /coding route only
      speaks CEM888-Code's native CEM888.AI shape; ``chat.completions``
      returns 404 on CEM888.AI-only model aliases like ``kimi-for-coding``).
    - ``api.cem888ai.com`` (native CEM888.AI).
    """
    normalized = (base_url or "").strip().lower().rstrip("/")
    if not normalized:
        return False
    if normalized.endswith("/cem888ai"):
        return True
    hostname = base_url_hostname(normalized)
    if hostname == "api.cem888ai.com":
        return True
...
def _maybe_wrap_cem888ai(
    ...
    """Rewrap a plain OpenAI client in ``CEM888AIAuxiliaryClient`` when
    the endpoint actually speaks CEM888.AI Messages.
    ...
    future /cem888ai gateways all land on the right wire format
    regardless of which branch built the client.

    Returns ``client_obj`` unchanged when:

    - It's already an CEM888.AI/Codex/Gemini/CopilotACP wrapper.
    - The endpoint is an OpenAI-wire endpoint.
    - ``api_mode`` is explicitly set to a non-CEM888.AI transport.
    - The ``cem888ai`` SDK is not installed (falls back to OpenAI wire).
    """
    # Already wrapped — don't double-wrap.
    if _safe_isinstance(client_obj, CEM888AIAuxiliaryClient):
        return client_obj
    ...
    # Explicit non-cem888ai api_mode wins over URL heuristics.
    if api_mode and api_mode != "cem888ai_messages":
        return client_obj

    should_wrap = (
        api_mode == "cem888ai_messages"
        or _endpoint_speaks_cem888ai_messages(base_url)
    )
    if not should_wrap:
        return client_obj

    try:
        from agent.cem888ai_adapter import build_cem888ai_client
    except ImportError:
        logger.warning(
            "Endpoint %s speaks CEM888.AI Messages but the cem888ai SDK is "
    ...
    try:
        real_client = build_cem888ai_client(api_key, base_url)
    except Exception as exc:
        logger.warning(
            "Failed to build CEM888.AI client for %s (%s) — falling back to "
            "OpenAI-wire client.", base_url, exc,
        )
        return client_obj

    logger.debug(
        "Auxiliary transport: wrapping client in CEM888AIAuxiliaryClient "
        "(model=%s, base_url=%s, api_mode=%s)",
        model, base_url[:60] if base_url else "", api_mode or "auto-detected",
    )
    return CEM888AIAuxiliaryClient(
        real_client, model, api_key, base_url, is_oauth=False,
    )


def _read_cem888_auth() -> Optional[dict]:
    """Read and validate ~/.cem888/auth.json for an active CEM888.AI provider.

    Returns the provider state dict if CEM888.AI is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("cem888")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _CEM888_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "cem888":
            return None
        provider = data.get("providers", {}).get("cem888", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read CEM888.AI auth: %s", exc)
        return None


def _cem888_api_key(provider: dict) -> str:
    """Extract the CEM888.AI runtime credential from the compatibility field."""
    return provider.get("agent_key") or provider.get("access_token", "")


def _cem888_base_url() -> str:
    """Resolve the CEM888.AI inference base URL from env or default."""
    return os.getenv("CEM888_INFERENCE_BASE_URL", _CEM888_DEFAULT_BASE_URL)


def _resolve_cem888_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh CEM888.AI runtime credentials when available.
    ...
    try:
        ...
        creds = resolve_cem888_runtime_credentials(
            min_key_ttl_seconds=max(60, int(os.getenv("CEM888_CEM888_MIN_KEY_TTL_SECONDS", "1800"))),
            timeout_seconds=float(os.getenv("CEM888_CEM888_TIMEOUT_SECONDS", "15")),
            inference_auth_mode=(
                CEM888_INFERENCE_AUTH_MODE_LEGACY
                if force_refresh
                else CEM888_INFERENCE_AUTH_MODE_AUTO
            ),
        )
    except Exception as exc:
        logger.debug("Auxiliary CEM888.AI runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``cem888 auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``cem888_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from cem888_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from CEM888 auth store.
    ...
    try:
        from cem888_cli.auth import _read_codex_tokens
        ...
        # prevent fallback to working providers (e.g. CEM888.AI).
        try:
            import base64
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from cem888_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if _is_provider_unhealthy(provider_id):
            ...
            continue
        if provider_id == "cem888ai":
            # Only try cem888ai when the user has explicitly configured it.
            # Without this gate, CEM888 Code credentials get silently used
            # as auxiliary fallback when the user's primary provider fails.
            try:
                from cem888_cli.auth import is_provider_explicitly_configured
                if not is_provider_explicitly_configured("cem888ai"):
                    continue
            except ImportError:
                pass
            return _try_cem888ai()
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                extra["default_headers"] = {"User-Agent": "cem888-code/0.1.0"}
            elif base_url_host_matches(base_url, "api.githubcopilot.com"):
                from cem888_cli.models import copilot_default_headers
            ...
            _client = _maybe_wrap_cem888ai(_client, model, api_key, raw_base_url)
...
def _try_cem888(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting CEM888.AI —
    # if another session already recorded a 429, skip CEM888.AI entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.cem888_rate_guard import cem888_rate_limit_remaining
        _remaining = cem888_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping CEM888.AI Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("cem888", ttl=_remaining)
            return None, None
    except Exception:
        pass

    cem888 = _read_cem888_auth()
    runtime = _resolve_cem888_runtime_api(force_refresh=False)
    if runtime is None and not cem888:
        logger.warning(
            "Auxiliary CEM888.AI client unavailable: no CEM888.AI authentication found "
            "(run: cem888 auth)."
        )
        _mark_provider_unhealthy("cem888", ttl=60)
        return None, None
    if runtime is None and cem888:
        # Runtime credential mint failed but stored CEM888.AI auth is still present.
        ...
        logger.debug(
            "Auxiliary CEM888.AI: runtime credential mint failed; falling back to "
            "stored auth.json token."
        )
    global auxiliary_is_cem888
    auxiliary_is_cem888 = True
    logger.debug("Auxiliary client: CEM888.AI Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/cem888/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_cem888_recommended_aux_model
    # auto-detects the caller's tier via check_cem888_free_tier().  Fall back to
    # _CEM888_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _CEM888_MODEL
    try:
        from cem888_cli.models import get_cem888_recommended_aux_model
        recommended = get_cem888_recommended_aux_model(vision=vision)
        if recommended:
            model = recommended
            logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _cem888_api_key(cem888 or {})
        base_url = str((cem888 or {}).get("inference_base_url") or _cem888_base_url()).rstrip("/")
    return (
        OpenAI(
            api_key=api_key,
            base_url=base_url,
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    """
    try:
        from cem888_cli.runtime_provider import resolve_runtime_provider
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `cem888 setup` or `cem888 model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_custom_endpoint() -> Tuple[Optional[Any], Optional[str]]:
    ...
    if custom_mode == "cem888ai_messages":
        # Third-party CEM888.AI-compatible gateway (MiniMax, Zhipu GLM,
        # LiteLLM proxies, etc.).  Must NEVER be treated as OAuth —
        # CEM888.AI OAuth claims only apply to api.cem888ai.com.
        try:
            from agent.cem888ai_adapter import build_cem888ai_client
            real_client = build_cem888ai_client(custom_key, custom_base)
        except ImportError:
            logger.warning(
                "Custom endpoint declares api_mode=cem888ai_messages but the "
                "cem888ai SDK is not installed — falling back to OpenAI-wire."
            )
            return OpenAI(api_key=custom_key, base_url=_clean_base, **_extra), model
        return (
            CEM888AIAuxiliaryClient(real_client, model, custom_key, custom_base, is_oauth=False),
            model,
        )
    # URL-based cem888ai detection for custom endpoints that didn't set
    # api_mode explicitly (e.g. kimi.com/coding reached via custom config).
    _fallback_client = OpenAI(api_key=custom_key, base_url=_clean_base, **_extra)
    _fallback_client = _maybe_wrap_cem888ai(
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_cem888ai`` / ``_try_cem888`` shape but delegates to
    :func:`cem888_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from cem888_cli.runtime_provider import _resolve_azure_foundry_runtime
    ...
    if runtime_api_mode == "cem888ai_messages":
        # Forward ``api_key`` verbatim — for static keys it's a string,
        # for Entra ID it's a callable. ``_maybe_wrap_cem888ai`` →
        # ``build_cem888ai_client`` detects the callable and installs
        # the bearer-injecting httpx hook.
        return _maybe_wrap_cem888ai(
...
def _try_cem888ai(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    try:
        from agent.cem888ai_adapter import build_cem888ai_client, resolve_cem888ai_token
    except ImportError:
        return None, None

    pool_present, entry = _select_pool_entry("cem888ai")
    if pool_present:
        if entry is None:
            return None, None
        ...
        entry = None
        token = explicit_api_key or resolve_cem888ai_token()
    if not token:
        return None, None

    # Allow base URL override from config.yaml model.base_url, but only
    # when the configured provider is cem888ai — otherwise a non-CEM888.AI
    # base_url (e.g. Codex endpoint) would leak into CEM888.AI requests.
    ...
    from agent.cem888ai_adapter import _is_oauth_token
    is_oauth = _is_oauth_token(token)
    model = _get_aux_model_for_provider("cem888ai") or "cem888-haiku-4-5-20251001"
    logger.debug("Auxiliary client: CEM888.AI native (%s) at %s (oauth=%s)", model, base_url, is_oauth)
    try:
        real_client = build_cem888ai_client(token, base_url)
    except ImportError:
        # The cem888ai_adapter module imports fine but the SDK itself is
        # missing — build_cem888ai_client raises ImportError at call time
        # when _cem888ai_sdk is None.  Treat as unavailable.
        return None, None
    return CEM888AIAuxiliaryClient(real_client, model, token, base_url, is_oauth=is_oauth), model
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second cem888
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``cem888 config aux reset``)."""
...
def _portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh CEM888.AI account API says paid access is allowed."""
    try:
        from cem888_cli.cem888_account import get_portal_account_info

        account_info = get_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary CEM888.AI paid-entitlement refresh check failed: %s", exc)
        return False
...
def _evict_cached_client_instance(target: Any) -> bool:
    ...
    ``CEM888AIAuxiliaryClient``, ``AsyncCodexAuxiliaryClient``, etc.) via
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "api.cem888ai.com"):
        return "cem888"
    if base_url_host_matches(base, "api.cem888ai.com"):
        return "cem888ai"
...
def _aux_attempt_sync(client, kwargs, *, task, provider, model):
    """Execute and attribute one physical synchronous provider attempt."""
    from agent.inference_telemetry import (
        current_inference_call_id,
        emit,
        next_inference_attempt,
        provider_class,
    )

    attempt = next_inference_attempt()
    ...
    try:
        ...
        emit(
            ...
            call_id=current_inference_call_id(),
            purpose=task or "auxiliary_unspecified",
            provider=provider or "auto",
            model=model or kwargs.get("model") or "default",
            provider_class=provider_class(provider, base),
            outcome="error",
...
async def _aux_attempt_async(client, kwargs, *, task, provider, model):
    """Execute and attribute one physical asynchronous provider attempt."""
    from agent.inference_telemetry import (
        current_inference_call_id,
        emit,
        next_inference_attempt,
...
def _retry_same_provider_sync(
    *,
    ...
    if _is_cem888ai_compat_endpoint(resolved_provider, retry_base):
        retry_kwargs["messages"] = _convert_openai_images_to_cem888ai(retry_kwargs["messages"])
    return _validate_llm_response(
        _aux_attempt_sync(
            retry_client, retry_kwargs, task=task,
            provider=resolved_provider, model=retry_model or final_model,
        ), task,
    )
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "cem888":
            ...
            _evict_cached_clients(normalized)
            return True
        if normalized == "cem888ai":
            from agent.cem888ai_adapter import read_cem888_code_credentials, _refresh_oauth_token, resolve_cem888ai_token
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "cem888": "cem888",
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/CEM888.AI get their chosen chat model; users
      ...
      2. OpenRouter → CEM888.AI → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_cem888, _stale_base_url_warned
    auxiliary_is_cem888 = False  # Reset — _try_cem888() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `cem888 model` but the
    #    old OPENAI_BASE_URL lingers in ~/.cem888/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: cem888 model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.cem888/.env",
    ...
    # on aggregators (OpenRouter, CEM888.AI) who previously got routed to a
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if isinstance(sync_client, CEM888AIAuxiliaryClient):
        return AsyncCEM888AIAuxiliaryClient(sync_client), model
    try:
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from cem888_cli.copilot_auth import copilot_request_headers

        async_kwargs["default_headers"] = copilot_request_headers(
            is_agent_turn=True, is_vision=is_vision
        )
    elif base_url_host_matches(sync_base_url, "api.kimi.com"):
        async_kwargs["default_headers"] = {"User-Agent": "cem888-code/0.1.0"}
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from cem888_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "cem888", "openai-codex" (or "codex"),
    ...
    def _wrap_if_needed(client_obj, final_model_str: str, base_url_str: str = "",
        ...
        - ``CodexAuxiliaryClient`` when the endpoint needs the Responses API
          ...
          model name).
        - ``CEM888AIAuxiliaryClient`` when the endpoint speaks CEM888.AI
          Messages (explicit ``api_mode=cem888ai_messages``, any ``/cem888ai``
          suffix, ``api.kimi.com/coding``, or ``api.cem888ai.com``).
        ...
        # CEM888.AI-wire endpoints: rewrap plain OpenAI clients so
    ...
    if provider == "cem888":
        ...
        client, default = _try_cem888(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: cem888 requested "
                           "but CEM888.AI Portal not configured (run: cem888 auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: cem888 model)")
                return None, None
    ...
    # OpenRouter / CEM888.AI bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: cem888 model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from cem888_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider (e.g. ``cem888``)
        ...
        if custom_entry:
            ...
            if custom_base:
                ...
                # cem888ai_messages talks to the /cem888ai surface directly;
                # OpenAI-wire paths (chat_completions / codex_responses) need the
                # /v1 equivalent.  Rewrite only on the OpenAI-wire path so the
                # CEM888.AI fallback SDK still sees the original URL.
                if entry_api_mode == "cem888ai_messages":
                ...
                # cem888ai_messages: route through the CEM888.AI Messages API
                # via CEM888AIAuxiliaryClient. Mirrors the anonymous-custom
                ...
                if entry_api_mode == "cem888ai_messages":
                    try:
                        ...
                        logger.warning(
                            ...
                            "cem888ai_messages but the cem888ai SDK is not "
                    ...
                    sync_cem888ai = CEM888AIAuxiliaryClient(
                        real_client, final_model, custom_key, custom_base, is_oauth=False,
                    )
                    if async_mode:
                        return AsyncCEM888AIAuxiliaryClient(sync_cem888ai), final_model
                    return sync_cem888ai, final_model
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: cem888 doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("cem888_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        if provider == "cem888ai":
            client, default_model = _try_cem888ai(explicit_api_key=explicit_api_key)
            if client is None:
                logger.warning("resolve_provider_client: cem888ai requested but no CEM888.AI credentials found")
                return None, None
            final_model = _normalize_resolved_model(model or default_model, provider)
        ...
        headers = {}
        if base_url_host_matches(base_url, "api.kimi.com"):
            headers["User-Agent"] = "cem888-code/0.1.0"
        elif base_url_host_matches(base_url, "api.githubcopilot.com"):
            from cem888_cli.copilot_auth import copilot_request_headers
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from cem888_cli.models import _should_use_copilot_responses_api
        ...
        # CEM888.AI-wire endpoints (Kimi Coding Plan api.kimi.com/coding,
        # /cem888ai-suffixed gateways) so named providers like kimi-coding
    ...
    if pconfig.auth_type == "external_process":
        ...
        # AWS SDK providers (Bedrock) — use the CEM888.AI Bedrock client via
        # boto3's credential chain (IAM roles, SSO, env vars, instance metadata).
        try:
            from agent.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
            from agent.cem888ai_adapter import build_cem888ai_bedrock_client
        except ImportError:
            logger.warning("resolve_provider_client: bedrock requested but "
                           "boto3 or cem888ai SDK not installed")
            return None, None
        ...
        default_model = "cem888ai.cem888-haiku-4-5-20251001-v1:0"
        final_model = _normalize_resolved_model(model or default_model, provider)
        try:
            real_client = build_cem888ai_bedrock_client(region)
        except ImportError as exc:
            logger.warning("resolve_provider_client: cannot create Bedrock "
                           "client: %s", exc)
            return None, None
        client = CEM888AIAuxiliaryClient(
        ...
        if provider == "cem888":
            return resolve_provider_client("cem888", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "cem888":
        return _try_cem888(vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → CEM888.AI → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. CEM888.AI — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      zai → glm-5v-turbo). CEM888.AI is the exception: it has a dedicated
        #      strict vision backend with tier-aware defaults, so it must not
        #      fall through to the user's text chat model here.
        #   2. OpenRouter  (vision-capable aggregator fallback)
        #   3. CEM888.AI Portal (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
        main_model = _read_main_model()
        if main_provider and main_provider not in {"auto", ""}:
            vision_model = _PROVIDER_VISION_MODELS.get(main_provider, main_model)
            if main_provider == "cem888":
                ...
                # Kimi Coding Plan's /coding endpoint (CEM888.AI Messages wire)
    ...
    # CEM888.AI-compatible one (which may be the main-runtime default).
    # The CEM888.AI wire rejects max_tokens on multimodal calls (error 1210),
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes CEM888.AI Portal product tags when the auxiliary client is backed
    by CEM888.AI Portal. Returns empty dict otherwise.
    """
    return _cem888_extra_body() if auxiliary_is_cem888 else {}
...
def _refresh_cem888_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh CEM888.AI runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_cem888_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`cem888_cli.plugins.PluginContext.register_auxiliary_task`) the
...
# ---------------------------------------------------------------------------
# CEM888.AI-compatible endpoint detection + image block conversion
# ---------------------------------------------------------------------------

# Providers that use CEM888.AI-compatible endpoints (via OpenAI SDK wrapper).
# Their image content blocks must use CEM888.AI format, not OpenAI format.
_ANTHROPIC_COMPAT_PROVIDERS = frozenset({"minimax", "minimax-oauth", "minimax-cn"})


def _is_cem888ai_compat_endpoint(provider: str, base_url: str) -> bool:
    """Detect if an endpoint expects CEM888.AI-format content blocks.

    Returns True for known CEM888.AI-compatible providers (MiniMax) and
    any endpoint whose URL contains ``/cem888ai`` in the path.
    """
    if provider in _ANTHROPIC_COMPAT_PROVIDERS:
        return True
    url_lower = (base_url or "").lower()
    return "/cem888ai" in url_lower


def _convert_openai_images_to_cem888ai(messages: list) -> list:
    """Convert OpenAI ``image_url`` content blocks to CEM888.AI ``image`` blocks.
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        # Codex adapter handles max_tokens internally; OpenRouter/CEM888.AI use max_tokens.
    ...
    if provider == "cem888" or auxiliary_is_cem888:
        merged_extra.setdefault("tags", []).extend(_portal_tags())
    if merged_extra:
...
def _validate_llm_response(response: Any, task: str = None) -> Any:
    ...
    # CEM888AIAuxiliaryClient) — they have .choices[0].message.
    try:
...
def _call_llm_impl(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `cem888 model`."
                )
    ...
    # Convert image blocks for CEM888.AI-compatible endpoints (e.g. MiniMax)
    _client_base = str(getattr(client, "base_url", "") or "")
    if _is_cem888ai_compat_endpoint(resolved_provider, _client_base):
        kwargs["messages"] = _convert_openai_images_to_cem888ai(kwargs["messages"])
    ...
    try:
        ...
        # ── CEM888.AI auth refresh parity with main agent ──────────────────
        client_is_cem888 = (
            resolved_provider == "cem888"
            or base_url_host_matches(_base_info, "api.cem888ai.com")
        )
        if (
            _is_payment_error(first_err)
            and client_is_cem888
            and _portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_cem888_auxiliary_client(
                cache_provider=resolved_provider or "cem888",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed CEM888.AI runtime credentials after paid account check, retrying",
                    task or "call",
                )
                ...
                try:
                    return _validate_llm_response(
                        _aux_attempt_sync(
                            refreshed_client, kwargs, task=task,
                            provider=resolved_provider, model=refreshed_model or final_model,
                        ), task)
                except Exception as retry_err:
                    if not (
        ...
        if _is_auth_error(first_err) and client_is_cem888:
            refreshed_client, refreshed_model = _refresh_cem888_auxiliary_client(
                cache_provider=resolved_provider or "cem888",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed CEM888.AI runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            if fb_client is not None:
                ...
                return _validate_llm_response(
                    _aux_attempt_sync(
                        fb_client, fb_kwargs, task=task,
                        provider=fb_label, model=fb_model,
                    ), task)
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        client_is_cem888 = (
            resolved_provider == "cem888"
            or base_url_host_matches(_client_base, "api.cem888ai.com")
        )
        if (
            _is_payment_error(first_err)
            and client_is_cem888
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed CEM888.AI runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_cem888:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed CEM888.AI runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            if fb_client is not None:
                ...
                return _validate_llm_response(
                    await _aux_attempt_async(
                        async_fb, fb_kwargs, task=task,
                        provider=fb_label, model=async_fb_model or fb_model,
                    ), task)
...
def _telemetry_route(task, provider, model, base_url, api_key):
    try:
        resolved = _resolve_task_provider_model(task, provider, model, base_url, api_key)
        return resolved[0] or "auto", resolved[1] or "default", resolved[2] or base_url or ""
    except Exception:
        return provider or "auto", model or "default", base_url or ""


def _telemetry_cost(response, provider, model, base_url, api_key):
    """Read provider-reported cost without network or pricing lookups."""
    usage = getattr(response, "usage", None)
    if usage is None:
        return {"cost_status": "unknown", "cost_source": "none"}
    for field in ("cost", "total_cost", "estimated_cost_usd"):
...
def call_llm(
    ...
    *,
    ...
    """Instrument one logical synchronous auxiliary inference call."""
    from agent.inference_telemetry import (
        emit,
        new_call_id,
        provider_class,
    ...
    route_provider, route_model, route_base = _telemetry_route(
        task, provider, model, base_url, api_key
    )
    call_id = new_call_id()
    inference_context = set_inference_context(call_id)
    started = time.monotonic()
    try:
        ...
        try:
            emit(
                ...
                model=route_model,
                provider_class=provider_class(route_provider, route_base),
                outcome="error",
                error_class=type(exc).__name__,
                duration_ms=round((time.monotonic() - started) * 1000, 3),
            )
        finally:
            reset_inference_context(inference_context)
        raise

    try:
        fields = response_usage(response)
        fields.update(_telemetry_cost(response, route_provider, route_model, route_base, api_key))
        emit(
            "inference_completed",
            call_id=call_id,
            purpose=task or "auxiliary_unspecified",
...
async def async_call_llm(
    ...
    *,
    ...
    """Instrument one logical asynchronous auxiliary inference call."""
    from agent.inference_telemetry import (
        emit,
        new_call_id,
        provider_class,
