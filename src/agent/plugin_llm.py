...
Plugins built on CEM888 Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing CEM888
...
:class:`~cem888_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors CEM888' main config
      ...
      entries:
        my-plugin:
          llm:
            ...
            allowed_providers: [openrouter, cem888ai]   # optional
...
handles every provider, fallback chain, and per-task override CEM888
supports.
"""
...
def _extract_usage(response: Any) -> PluginLlmUsage:
    ...
    Tolerant of provider differences — CEM888.AI via the auxiliary
...
class PluginLlm:
    ...
    Instances are constructed by :class:`cem888_cli.plugins.PluginContext`
