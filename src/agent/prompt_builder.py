...
from cem888_constants import get_cem888_home, get_skills_dir, is_wsl
...
_CEM888_MD_NAMES = (".cem888.md", "CEM888.md")


def _find_cem888_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.cem888.md`` or ``CEM888.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are CEM888 Agent, an intelligent AI assistant created by CEM888.AI. "
...
CEM888_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using CEM888 Agent "
    "itself, load the `cem888-agent` skill with skill_view(name='cem888-agent') "
    "before answering. Docs: https://cem888.ai/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.cem888/kanban.db`. Your task id is in "
    "`$CEM888_KANBAN_TASK`; your workspace is `$CEM888_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CEM888_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `cem888 kanban <verb>` for board operations. Use "
...
# is active. Universal — works for any model (CEM888, GPT, open models).
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the CEM888 WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where CEM888 itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host CEM888 runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where CEM888 itself is running. The host OS, home, and cwd "
                f"of the CEM888 process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where CEM888 "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_cem888_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.cem888/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("CEM888_PLATFORM")
        or get_session_env("CEM888_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot CEM888 Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `cem888-agent` skill "
            "first. It has the actual commands (e.g. `cem888 config set …`, `cem888 tools`, "
            "`cem888 setup`) so you don't have to guess or invent workarounds.\n"
...
def build_cem888_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact CEM888.AI subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import CEM888.AI subscription helper: %s", exc)
        return ""

    if not managed_cem888_tools_enabled():
        return ""
    ...
    features = get_cem888_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via CEM888.AI subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.cem888_auth_present:
            return f"- {feature.label}: included with CEM888.AI subscription, not currently selected"
        if feature.key == "modal" and features.cem888_auth_present:
            return f"- {feature.label}: optional via CEM888.AI subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# CEM888.AI Subscription",
        "CEM888.AI subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a CEM888.AI-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that CEM888.AI subscription would unlock or simplify, suggest CEM888.AI subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: cem888 setup, cem888 setup tools, cem888 setup terminal, cem888 status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from CEM888_HOME and return its content, or None.
    ...
    """
    try:
        from cem888_cli.config import ensure_cem888_home
        ensure_cem888_home()
    except Exception as e:
        logger.debug("Could not ensure CEM888_HOME before loading SOUL.md: %s", e)

    soul_path = get_cem888_home() / "SOUL.md"
...
def _load_cem888_md(cwd_path: Path) -> str:
    """.cem888.md / CEM888.md — walk to git root."""
    cem888_md_path = _find_cem888_md(cwd_path)
    if not cem888_md_path:
        return ""
    try:
        content = cem888_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = cem888_md_path.name
        try:
            rel = str(cem888_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".cem888.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", cem888_md_path, e)
        return ""
...
def _load_cem888_md(cwd_path: Path) -> str:
    """CEM888.md / cem888.md — cwd only."""
    for name in ["CEM888.md", "cem888.md"]:
        candidate = cwd_path / name
        if candidate.exists():
            try:
                content = candidate.read_text(encoding="utf-8").strip()
...
def _load_profile_context() -> str:
    """Context files from CEM888_HOME — the last-resort fallback.

    The cwd-scoped lookups resolve against the PROCESS WORKING DIRECTORY. The
    gateway does not run from the profile directory (it runs from $HOME), so
    without this fallback a profile's own AGENTS.md is unreachable and any rule
    written there never reaches the model — a silent dead channel.

    Called last on purpose: a project's own AGENTS.md must still win over the
    agent's profile.
    """
    try:
        home = Path(get_cem888_home())
    except Exception as e:
        logger.debug("Could not resolve CEM888_HOME for context files: %s", e)
        return ""
    if not home.is_dir():
        return ""
    return (
        _load_cem888_md(home)
        or _load_agents_md(home)
        or _load_cursorrules(home)
    )
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .cem888.md / CEM888.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CEM888.md / cem888.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)
      5. the same three, from CEM888_HOME (last-resort fallback)

    SOUL.md from CEM888_HOME is independent and always included when present.
    Each context source is capped at 20,000 chars.

    Step 5 exists because steps 1-4 resolve against the process working
    directory — and the gateway does not run from the profile directory, so a
    profile's own AGENTS.md was previously unreachable and any rule written
    there never reached the model. cwd deliberately wins, so a project's own
    AGENTS.md still takes precedence.
    ...
    # NOTE: the third term previously repeated _load_cem888_md(cwd_path)
    # verbatim — a dead duplicate of the first. Removed; the CEM888_HOME
    # fallback now occupies that slot.
    project_context = (
        _load_cem888_md(cwd_path)
        or _load_agents_md(cwd_path)
        or _load_cursorrules(cwd_path)
        or _load_profile_context()
    )
    if project_context:
        sections.append(project_context)

    # SOUL.md from CEM888_HOME only — skip when already loaded as identity
    if not skip_soul:
