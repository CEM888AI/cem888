...
as ``kind: backend``) or ``~/.cem888/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``cem888 tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``cem888 tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``cem888 tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$CEM888_HOME/cache/images/``, creating parents as needed."""
    from cem888_constants import get_cem888_home

    path = get_cem888_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$CEM888_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$CEM888_HOME/cache/images/``.
