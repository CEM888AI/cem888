...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.cem888/models_dev_cache.json)
...
class ModelInfo:
    ...
    name: str
    family: str
    provider_id: str        # models.dev provider ID (e.g. "cem888ai")
...
# ---------------------------------------------------------------------------
# Provider ID mapping: CEM888 ↔ models.dev
# ---------------------------------------------------------------------------

# CEM888 provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → CEM888 (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_cem888_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``cem888 config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a CEM888 provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal CEM888 agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a CEM888 provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve CEM888 ID → models.dev ID
...
def get_model_info(
    ...
    Accepts CEM888 or models.dev provider ID.  Tries exact match then
