...
from cem888_cli.config import get_env_value, load_env
from agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import cem888_cli.auth as auth_mod
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "cem888":
            # CEM888.AI stores the runtime inference credential in agent_key for
...
class CredentialPool:
    ...
    def _sync_cem888ai_entry_from_credentials_file(self, entry: PooledCredential) -> PooledCredential:
        """Sync a cem888_code pool entry from ~/.cem888/.credentials.json if tokens differ.

        OAuth refresh tokens are single-use. When something external (e.g.
        CEM888 Code CLI, or another profile's pool) refreshes the token, it
        writes the new pair to ~/.cem888/.credentials.json. The pool entry's
        refresh token becomes stale. This method detects that and syncs.
        """
        if self.provider != "cem888ai" or entry.source != "cem888_code":
            return entry
        try:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``cem888 model`` / ``cem888 auth`` which
        ...
        Mirrors the CEM888.AI/CEM888.AI resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another CEM888 process
    ...
    def _sync_cem888_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a CEM888.AI pool entry from auth.json if tokens differ.

        CEM888.AI OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_cem888_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the CEM888.AI Portal.
        """
        if self.provider != "cem888" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing CEM888.AI state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync CEM888.AI entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently CEM888.AI, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every CEM888.AI/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``cem888`` invocation that defaults to the active provider
        (e.g. setup wizard, ``cem888 auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                if self.provider == "cem888":
                    ...
                    _store_provider_state(auth_store, "cem888", state, set_active=False)
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "cem888ai":
                from agent.cem888ai_adapter import refresh_cem888ai_oauth_pure

                refreshed = refresh_cem888ai_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("cem888_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # Keep ~/.cem888/.credentials.json in sync so that the
                # fallback path (resolve_cem888ai_token) and other profiles
                # see the latest tokens.
                if entry.source == "cem888_code":
                    try:
                        from agent.cem888ai_adapter import _write_cem888_code_credentials
                ...
                # refresh_token — single-use tokens consumed by another CEM888
            ...
            elif self.provider == "cem888":
                synced = self._sync_cem888_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_cem888_runtime_credentials(
                    min_key_ttl_seconds=DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
                    inference_auth_mode=(
                        auth_mod.CEM888_INFERENCE_AUTH_MODE_LEGACY
                        if force
                        else auth_mod.CEM888_INFERENCE_AUTH_MODE_AUTO
                    ),
                )
                updated = self._sync_cem888_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
            logger.debug("Credential refresh failed for %s/%s: %s", self.provider, entry.id, exc)
            # For cem888ai cem888_code entries: the refresh token may have been
            # consumed by another process. Check if ~/.cem888/.credentials.json
            # has a newer token pair and retry once.
            if self.provider == "cem888ai" and entry.source == "cem888_code":
                synced = self._sync_cem888ai_entry_from_credentials_file(entry)
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_cem888ai_oauth_pure(
                            ...
                            use_json=synced.source.endswith("cem888_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as cem888 — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # in-memory pool.  Mirrors the CEM888.AI quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/cem888 — another CEM888 process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and CEM888.AI quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For cem888: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "cem888":
                synced = self._sync_cem888_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("CEM888.AI refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_cem888_refresh_error(exc):
                    logger.debug("CEM888.AI refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "cem888") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_cem888_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_cem888_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal CEM888.AI OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.CEM888_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.CEM888_DEVICE_CODE_SOURCE}",
                    }
                    self._entries = [
                        item for item in self._entries
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "cem888":
            # CEM888.AI refresh/mint can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            # For cem888ai cem888_code entries, sync from the credentials file
            # before any status/refresh checks. This picks up tokens refreshed
            # by other processes (CEM888 Code CLI, other CEM888 profiles).
            if (self.provider == "cem888ai" and entry.source == "cem888_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For cem888 entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_cem888_runtime_credentials(), making this entry's
            ...
            # re-authed via `cem888 model` / `cem888 auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `cem888 model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `cem888 auth remove <provider> <N>` is stable across all source types.
    try:
        from cem888_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "cem888ai":
        # Only auto-discover external credentials (CEM888 Code, CEM888 PKCE)
        # when the user has explicitly configured cem888ai as their provider.
        # Without this gate, auxiliary client fallback chains silently read
        # ~/.cem888/.credentials.json without user consent.  See PR #4210.
        try:
        ...
        # API-key vs OAuth is a user-visible choice at `cem888 setup` ("CEM888
        # Pro/Max subscription" vs "CEM888.AI API key").  The signal that the
        # user picked the API-key path is: ANTHROPIC_API_KEY set in the env,
        # AND no OAuth env vars set — `save_cem888ai_api_key()` writes the
        # API key and zeros ANTHROPIC_TOKEN; `save_cem888ai_oauth_token()`
        # does the inverse.  When that signal is present we MUST NOT seed
        # autodiscovered OAuth tokens (~/.cem888/.credentials.json from the
        # CEM888 Code CLI, cem888_pkce creds from a previous OAuth login)
        # into the cem888ai pool — otherwise rotation on a 401/429 silently
        # flips the session onto an OAuth credential, which forces the CEM888
        # Code identity injection, `mcp_` tool-name rewrite, and cem888-cli
        # User-Agent header (`agent/cem888ai_adapter.py:2128`).  Users who
        # explicitly opted into the API-key path are explicitly opting OUT of
        # that masquerade.  Prefer ~/.cem888/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `cem888 setup` writes.
        _env_file = load_env()

        def _env_val(key: str) -> str:
            return (_env_file.get(key) or os.environ.get(key) or "").strip()

        cem888ai_api_key = _env_val("ANTHROPIC_API_KEY")
        cem888ai_oauth_env = (
            _env_val("ANTHROPIC_TOKEN") or _env_val("CEM888_OAUTH_TOKEN")
        )
        api_key_path_explicit = bool(cem888ai_api_key and not cem888ai_oauth_env)
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"cem888_pkce", "cem888_code"}
            ]
        ...
        from agent.cem888ai_adapter import read_cem888_code_credentials, read_cem888_oauth_credentials

        for source_name, creds in (
            ("cem888_pkce", read_cem888_oauth_credentials()),
            ("cem888_code", read_cem888_code_credentials()),
        ):
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_cem888_credentials(label=...) when the user ran
            # `cem888 auth add cem888 --label <name>`).  Fall back to the
        ...
        # CEM888 auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from cem888_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.cem888/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `cem888 auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `cem888 auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the CEM888 auth store.  Without this gate the removal is instantly
        ...
        # CEM888 owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``cem888 model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``cem888 auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from cem888_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.cem888/.env over os.environ — the user's config file is the
    # authoritative source for CEM888 credentials. Stale env vars from parent
    ...
    # Honour user suppression — `cem888 auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.cem888/.env.
    ...
    try:
        from cem888_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from cem888_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.cem888/.env over os.environ
    ...
    for env_var in env_vars:
        ...
        if _is_source_suppressed(provider, source):
            continue
        active_sources.add(source)
        auth_type = AUTH_TYPE_OAUTH if provider == "cem888ai" and not token.startswith("sk-ant-api") else AUTH_TYPE_API_KEY
