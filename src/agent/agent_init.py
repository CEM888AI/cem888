...
from cem888_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        model (str): Model name to use (default: "cem888ai/cem888-opus-4.6")
        ...
        prefill_messages (List[Dict]): Messages to prepend to conversation history as prefilled context.
            ...
            NOTE: CEM888.AI Sonnet 4.6+ and Opus 4.6+ reject a conversation that ends on an
        ...
        load_soul_identity (bool): If True, still use ~/.cem888/SOUL.md as the primary
            identity even when skip_context_files=True. Project context files from the cwd
            remain skipped.
    """
    _install_safe_stdio()

    # Immutable process/profile binding. This was validated on the CLI path
    # before plugins/config loaded; keep the object on the agent so prompt
    # assembly and tool execution consume the exact same authority instance.
    from agent.authority import load_agent_authority
    # A running agent must carry a verified Tier-0 authority object. If the
    # manifest is required and unresolvable this raises AuthorityError instead
    # of silently running with no write/terminal/prompt guards.
    agent.authority = load_agent_authority(required=True)
    ...
    if api_mode in {"chat_completions", "codex_responses", "cem888ai_messages", "bedrock_converse", "codex_app_server"}:
        ...
        agent.api_mode = "codex_responses"
        agent.provider = "xai"
    elif agent.provider == "cem888ai" or (provider_name is None and agent._base_url_hostname == "api.cem888ai.com"):
        agent.api_mode = "cem888ai_messages"
        agent.provider = "cem888ai"
    elif agent._base_url_lower.rstrip("/").endswith("/cem888ai"):
        # Third-party CEM888.AI-compatible endpoints (e.g. MiniMax, DashScope)
        # use a URL convention ending in /cem888ai. Auto-detect these so the
        # CEM888.AI Messages API adapter is used instead of chat completions.
        agent.api_mode = "cem888ai_messages"
    ...
    # CEM888.AI prompt caching: auto-enabled for CEM888 models on native
    # CEM888.AI, OpenRouter, and third-party gateways that speak the
    # CEM888.AI protocol (``api_mode == 'cem888ai_messages'``). Reduces
    # input costs by ~75% on multi-turn conversations. Uses system_and_3
    # strategy (4 breakpoints). See ``_cem888ai_prompt_cache_policy``
    # for the layout-vs-transport decision.
    agent._use_prompt_caching, agent._use_native_cache_layout = (
        agent._cem888ai_prompt_cache_policy()
    )
    # CEM888.AI supports "5m" (default) and "1h" cache TTL tiers. Read from
    ...
    agent._cache_ttl = "5m"
    try:
        from cem888_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.cem888/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from cem888_logging import setup_logging, setup_verbose_logging
    setup_logging(cem888_home=_ra()._cem888_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that cem888_logging.setup_logging() does not
        ...
        # handler level inside cem888_logging.py, not here.
        pass
    ...
    # Cache cem888ai image-to-text fallbacks per image payload/URL so a
    # single tool loop does not repeatedly re-run auxiliary vision on the
    # same image history.
    agent._cem888ai_image_fallback_cache: Dict[str, str] = {}

    # Initialize LLM client via centralized provider router.
    # The router handles auth resolution, base URL, headers, and
    # Codex/CEM888.AI wrapping for all known providers.
    # raw_codex=True because the main agent needs direct responses.stream()
    # access for Codex Responses API streaming.
    agent._cem888ai_client = None
    agent._is_cem888ai_oauth = False

    # Resolve per-provider / per-model request timeout once up front so
    # every client construction path below (CEM888.AI native, OpenAI-wire,
    # router-based implicit auth) can apply it consistently.  Bedrock
    # CEM888 uses its own timeout path and is not covered here.
    ...
    if agent.api_mode == "cem888ai_messages":
        ...
        # Bedrock + CEM888 → use CEM888.AIBedrock SDK for full feature parity
        # (prompt caching, thinking budgets, adaptive thinking).
        _is_bedrock_cem888ai = agent.provider == "bedrock"
        if _is_bedrock_cem888ai:
            from agent.cem888ai_adapter import build_cem888ai_bedrock_client
            _region_match = re.search(r"bedrock-runtime\.([a-z0-9-]+)\.", base_url or "")
            _br_region = _region_match.group(1) if _region_match else "us-east-1"
            agent._bedrock_region = _br_region
            agent._cem888ai_client = build_cem888ai_bedrock_client(_br_region)
            agent._cem888ai_api_key = "aws-sdk"
            agent._cem888ai_base_url = base_url
            agent._is_cem888ai_oauth = False
            agent.api_key = "aws-sdk"
            agent.client = None
            agent._client_kwargs = {}
            if not agent.quiet_mode:
                print(f"🤖 AI Agent initialized with model: {agent.model} (AWS Bedrock + CEM888.AIBedrock SDK, {_br_region})")
        else:
            # Only fall back to ANTHROPIC_TOKEN when the provider is actually CEM888.AI.
            # Other cem888ai_messages providers (MiniMax, Alibaba, etc.) must use their own API key.
            # Falling back would send CEM888.AI credentials to third-party endpoints (Fixes #1739, #minimax-401).
            _is_native_cem888ai = agent.provider == "cem888ai"
            effective_key = (api_key or resolve_cem888ai_token() or "") if _is_native_cem888ai else (api_key or "")

            # MiniMax OAuth issues short-lived (~15-min) access tokens. The
            # CEM888.AI SDK caches ``api_key`` as a static string at client
            ...
            # token provider — ``build_cem888ai_client`` recognizes the
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from cem888_cli.auth import build_minimax_oauth_token_provider
            ...
            agent._cem888ai_api_key = effective_key
            agent._cem888ai_base_url = base_url
            # Only mark the session as OAuth-authenticated when the token
            # genuinely belongs to native CEM888.AI.  Third-party providers
            # (MiniMax, Kimi, GLM, LiteLLM proxies) that accept the
            # CEM888.AI protocol must never trip OAuth code paths — doing
            # so injects CEM888-Code identity headers and system prompts
            # that cause 401/403 on their endpoints.  Guards #1739 and
            # the third-party identity-injection bug.
            from agent.cem888ai_adapter import _is_oauth_token as _is_oat
            agent._is_cem888ai_oauth = _is_oat(effective_key) if (_is_native_cem888ai and isinstance(effective_key, str)) else False
            agent._cem888ai_client = build_cem888ai_client(effective_key, base_url, timeout=_provider_timeout)
            # No OpenAI client needed for CEM888.AI mode
            agent.client = None
            agent._client_kwargs = {}
            if not agent.quiet_mode:
                print(f"🤖 AI Agent initialized with model: {agent.model} (CEM888.AI native)")
                # ``effective_key`` may be a callable Entra ID bearer
                # provider for Azure Foundry cem888ai_messages mode.
                # The CEM888.AI adapter installs an httpx event hook
        ...
        try:
            from cem888_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `cem888 model` to "
                        "select a provider, or run `cem888 setup` for first-time "
                        "configuration."
                    )
        
        agent._client_kwargs = client_kwargs  # stored for rebuilding after interrupt

        # Enable fine-grained tool streaming for CEM888 on OpenRouter.
        # Without this, CEM888.AI buffers the entire tool call and goes
        # silent for minutes while thinking — OpenRouter's upstream proxy
        # times out during the silence.  The beta header makes CEM888.AI
        # stream tool call arguments token-by-token, keeping the
        # connection alive.
        _effective_base = str(client_kwargs.get("base_url", "")).lower()
        if base_url_host_matches(_effective_base, "openrouter.ai") and "cem888" in (agent.model or "").lower():
            headers = client_kwargs.get("default_headers") or {}
            existing_beta = headers.get("x-cem888ai-beta", "")
            _FINE_GRAINED = "fine-grained-tool-streaming-2025-05-14"
            if _FINE_GRAINED not in existing_beta:
                if existing_beta:
                    headers["x-cem888ai-beta"] = f"{existing_beta},{_FINE_GRAINED}"
                else:
                    headers["x-cem888ai-beta"] = _FINE_GRAINED
    ...
    # worker (kanban_show tool is present iff CEM888_KANBAN_TASK is set).
    ...
    if agent._use_prompt_caching and not agent.quiet_mode:
        if agent._use_native_cache_layout and agent.provider == "cem888ai":
            source = "native CEM888.AI"
        elif agent._use_native_cache_layout:
            source = "CEM888.AI-compatible endpoint"
        else:
            source = "CEM888 via OpenRouter"
    ...
    try:
        ...
        os.environ["CEM888_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.cem888/sessions/ alongside gateway sessions
    cem888_home = get_cem888_home()
    agent.logs_dir = cem888_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.cem888/sessions/session_{sid}.json)
    ...
    try:
        from cem888_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from cem888_cli.config import load_config as _load_agent_config
    ...
    # Persistent memory (memories/*_MANIFEST.md + durable typed store) -- loaded from disk
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "cem888"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from cem888_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from cem888_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by CEM888 Agent.  Choose a model with at least "
    ...
    if agent.api_mode == "cem888ai_messages":
        agent._primary_runtime.update({
            "cem888ai_api_key": agent._cem888ai_api_key,
            "cem888ai_base_url": agent._cem888ai_base_url,
            "is_cem888ai_oauth": agent._is_cem888ai_oauth,
        })
