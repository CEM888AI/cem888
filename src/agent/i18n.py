"""Lightweight internationalization (i18n) for CEM888 static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by CEM888 itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``CEM888_LANGUAGE`` environment variable (for tests / quick override)
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a cem888 dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`cem888_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("CEM888_LANGUAGE")
    if env_lang:
