...
Two modes:
  ...
  native  — attach images as OpenAI-style ``image_url`` content parts on the
            user turn. Provider adapters (CEM888.AI, Gemini, Bedrock, Codex,
...
def _supports_vision_override(
    ...
    # (cem888_cli/runtime_provider.py:_resolve_named_custom_runtime), so the
...
def decide_image_input_mode(
    ...
    Args:
      provider: active inference provider ID (e.g. ``"cem888ai"``, ``"openrouter"``).
...
# the provider rejects the request (e.g. CEM888.AI's hard 5 MB per-image
...
# (OpenAI accepts 49 MB+, CEM888.AI 5 MB, Gemini 100 MB, others unknown).
...
def _sniff_mime_from_bytes(raw: bytes) -> Optional[str]:
    ...
    CEM888.AI strictly validates that declared media_type matches the
...
def build_native_content_parts(
    ...
    request because an image is too large (e.g. CEM888.AI's 5 MB per-image
