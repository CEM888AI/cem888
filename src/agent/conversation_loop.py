...
from agent.prompt_caching import apply_cem888ai_cache_control
from agent.retry_utils import jittered_backoff
from agent.trajectory import has_incomplete_scratchpad
from agent.usage_pricing import estimate_usage_cost, normalize_usage
from cem888_constants import display_cem888_home as _dhh_fn, PARTIAL_STREAM_STUB_ID
from cem888_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for CEM888 tool use: "
    ...
    return (
        ...
        f"context, but CEM888 needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In CEM888 config, set `model.ollama_num_ctx: 65536` "
...
def _try_refresh_cem888_paid_entitlement_credentials(agent) -> bool:
    """Refresh CEM888.AI runtime credentials after a fresh paid-entitlement check."""
    try:
        from cem888_cli.auth import NOUS_INFERENCE_AUTH_MODE_LEGACY
        from cem888_cli.cem888_account import get_portal_account_info

        account_info = get_portal_account_info(force_fresh=True)
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_cem888_client_credentials(
            force=False,
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt:
        ...
        # previous turn so the CEM888.AI cache prefix matches.
...
def _run_conversation_impl(
    ...
    # ``cem888 logs --session <id>`` can filter a single conversation.
    from cem888_logging import set_session_context
    ...
    agent._recent_tool_call_history = []
    agent._consecutive_tool_repeat_count = 0
    ...
    if agent.api_mode != "cem888ai_messages":
        try:
    ...
    # ── Verifier feedback loop: inject unresolved file mutations from ──
    # previous turn.  When the agent succeeds, the state file stays
    # empty → silence = resolved.  When it fails, the verifier footer
    # re-appears next turn.
    _verifier_state_file = os.path.join(
        os.environ.get("CEM888_HOME", os.path.expanduser("~/.cem888")),
        "profiles",
        os.environ.get("CEM888_PROFILE", "default"),
        "state",
        "verifier_unsolved.json",
    )
    try:
        if os.path.exists(_verifier_state_file):
            with open(_verifier_state_file, "r") as _vf:
                _prev_unresolved = json.load(_vf)
            if _prev_unresolved and isinstance(_prev_unresolved, dict):
                _paths = list(_prev_unresolved.keys())
                _lines = [
                    "⚠️ UNRESOLVED FROM PREVIOUS TURN — The file-mutation verifier "
                    "detected these files were NOT modified despite previous claims. "
                    "Fix them NOW:"
                ]
                for _p in _paths[:10]:
                    _info = _prev_unresolved[_p]
                    _preview = (_info.get("error_preview") or "").strip()
                    _tool = _info.get("tool") or "patch"
                    _lines.append(f"  • {_p} — [{_tool}] {_preview}")
                if len(_paths) > 10:
                    _lines.append(f"  • … and {len(_paths) - 10} more")
                _verifier_prefix = "\n".join(_lines)
                if _verifier_prefix:
                    user_message = _verifier_prefix + "\n\n" + user_message
            os.remove(_verifier_state_file)
    except Exception:
        pass
    ...
    # producing a different system prompt and breaking the CEM888.AI
    # prefix cache.
    ...
    # are reused.  The system prompt is CEM888's territory; plugins
    ...
    try:
        ...
        for r in _pre_results:
            if isinstance(r, dict) and r.get("action") == "block" and isinstance(r.get("message"), str):
                _pre_llm_block = r["message"]
                break
    ...
    if _pre_llm_block:
        ...
        return {
            "final_response": _pre_llm_block,
            "messages": messages + [{"role": "assistant", "content": _pre_llm_block}],
            "api_calls": 0,
            "completed": False,
            "partial": True,
            "error": "TASK_LIFECYCLE_CONTAINED",
        }

    # ── CEM888 Gateway Allowance Gate ──────────────────────────────────────
    # Blocks the turn when user is out of both free messages AND credits.
    try:
        from agent.gateway_allowance import check_allowance as _check_allowance
        _allowance = _check_allowance()
        if not _allowance["can_proceed"] and _allowance.get("action") == "top_up":
            from agent.gateway_allowance import format_block_message
            _block_msg = format_block_message(_allowance)
            logger.warning(
                "Gateway allowance blocked: free=%s credits=%s",
                _allowance.get("free_messages_remaining"),
                _allowance.get("credit_balance"),
            )
            return {
                ...
                "api_calls": 0,
                "completed": False,
                "turn_exit_reason": "allowance_exhausted",
                "failed": False,
                "partial": False,
            }
    except Exception as _allow_exc:
        logger.warning("Gateway allowance check failed, allowing through: %s", _allow_exc)
    ...
    # all run inside Codex). Default CEM888 path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        from agent.inference_telemetry import emit as _emit_inference, new_call_id as _new_call_id, provider_class as _provider_class
        _main_call_id = _new_call_id()
        _emit_inference(
            "inference_started",
            call_id=_main_call_id,
            purpose="main_conversation",
            provider=agent.provider or "unknown",
            model=agent.model or "unknown",
            provider_class=_provider_class(agent.provider, agent.base_url),
            attempt=1,
        )
        ...
        _request_current_user_idx = current_turn_user_idx
        _compiled_packet = ""
        try:
            from agent.context_compiler import compile_context as _compile_context, enabled as _context_compiler_enabled
            if _context_compiler_enabled():
                _compiled = _compile_context(
                    messages,
                    user_query=original_user_message if isinstance(original_user_message, str) else "",
                    # NOTE: pre_llm_call output is deliberately NOT handed to the
                    # compiler any more.  The compiler scored it as an optional
                    # "continuity_retrieval" candidate (authority 78) and then
                    # dropped it whenever the user's message shared no terms with
                    # it -- silently discarding context a plugin had declared
                    # mandatory, for every plugin, fleet-wide.  It is appended to
                    # the turn's user message below instead.
                    memory_context=_ext_prefetch_cache or "",
                    provider=agent.provider or "",
                    model=agent.model or "",
                )
                _request_messages = _compiled.messages
                _request_current_user_idx = _compiled.current_user_index
                _compiled_packet = _compiled.packet
                _emit_inference(
                    "context_compiled",
                    call_id=_main_call_id,
                    purpose="main_conversation",
                    context_chars=len(_compiled_packet),
                    input_tokens=_compiled.receipt.get("packet_tokens", 0),
                )
        except Exception:
            request_logger.debug("minimal context compiler failed open", exc_info=True)

        api_messages = []
        for idx, msg in enumerate(_request_messages):
            api_msg = msg.copy()
            ...
            if idx == _request_current_user_idx and msg.get("role") == "user":
                _injections = []
                if _compiled_packet:
                    _injections.append(_compiled_packet)
                elif _ext_prefetch_cache:
                    _fenced = build_memory_context_block(_ext_prefetch_cache)
                    if _fenced:
                        _injections.append(_fenced)
                # pre_llm_call output is MANDATORY context: a plugin that
                # returns a string from pre_llm_call is explicitly asking for it
                # to be in front of the model this turn.  It is appended
                # unconditionally -- in ADDITION to the compiled packet.  The
                # previous `and not _compiled_packet` guard meant that turning
                # the context compiler ON silently deleted every plugin
                # injection (scratch-pad checkpoints, unresolved-work warnings,
                # work-mode anchors), because the compiler had also been fed the
                # same text as a droppable, relevance-gated candidate.
        ...
        # cache prefix.  The system prompt is reserved for CEM888 internals.
        #
        # CEM888 invariant: the system prompt is built ONCE per session
        ...
        # Apply CEM888.AI prompt caching for CEM888 models on native
        # CEM888.AI, OpenRouter, and third-party CEM888.AI-compatible
        # gateways. Auto-detected: if ``_use_prompt_caching`` is set,
        # inject cache_control breakpoints (system + last 3 messages)
        # to reduce input token costs by ~75% on multi-turn
        # conversations.
        if agent._use_prompt_caching:
            api_messages = apply_cem888ai_cache_control(
                api_messages,
                cache_ttl=agent._cache_ttl,
                native_cem888ai=agent._use_native_cache_layout,
            )
        ...
        # left behind. Prevents CEM888.AI 400s ("The final block in an
        # assistant message cannot be `thinking`.") and equivalent errors
        # from third-party CEM888.AI-compatible gateways that can't replay
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for CEM888 tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        cem888ai_auth_retry_attempted=False
        cem888_auth_retry_attempted=False
        cem888_paid_entitlement_refresh_attempted=False
        ...
        # Set only by the final, request-shaped gate below.  Keeping this
        # outside the retry loop lets us leave cleanly without converting a
        # denied dispatch into an ordinary provider failure/retry.
        _final_pre_api_block = None

        while retry_count < max_retries:
            # ── CEM888.AI Portal rate limit guard ──────────────────────
            # If another session already recorded that CEM888.AI is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "cem888":
                _nous_remaining = None
                # removed: nous_rate_guard module not included in CEM888 fork
            ...
            try:
                ...
                # FINAL PROVIDER-DISPATCH KILL SWITCH.  This is deliberately
                # after transport preflight and immediately before either
                # streaming or non-streaming adapter invocation.  Plugin
                # hooks are advisory everywhere else; at this seam a
                # structured block prevents *all* model/provider transports
                # from being called, including fallbacks and adapters.
                from cem888_cli.plugins import get_pre_api_request_block_message
                request_messages = api_kwargs.get("messages")
                if not isinstance(request_messages, list):
                    request_messages = api_kwargs.get("input")
                _final_pre_api_block = get_pre_api_request_block_message(
                    task_id=effective_task_id,
                    ...
                    request_messages=list(request_messages) if isinstance(request_messages, list) else list(api_messages),
                ...
                if _final_pre_api_block:
                    _turn_exit_reason = "pre_api_lifecycle_block"
                    break

                if env_var_enabled("CEM888_DUMP_REQUESTS"):
                ...
                elif agent.api_mode == "cem888ai_messages":
                ...
                if finish_reason == "length":
                    ...
                    # and cem888ai_messages.  For CEM888.AI we use the same
                    ...
                    if agent.api_mode == "cem888ai_messages":
                        _trunc_result = _trunc_transport.normalize_response(
                            response, strip_tool_prefix=agent._is_cem888ai_oauth
                        )
                    else:
                    ...
                    if agent.api_mode in {"chat_completions", "bedrock_converse", "cem888ai_messages"}:
                ...
                _telemetry_usage = {}
                _telemetry_cost = {"cost_status": "unknown", "cost_source": "none"}
                if hasattr(response, 'usage') and response.usage:
                    ...
                    # only True for CEM888.AI-style marker injection.
                    # ``canonical_usage`` is already normalised from all
                    # three API shapes (CEM888.AI / Codex / OpenAI-chat)
                ...
                _emit_inference(
                    ...
                    duration_ms=round(api_duration * 1000, 3),
                    **_telemetry_usage,
                    **_telemetry_cost,
                )
                ...
                # Clear CEM888.AI rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting CEM888.AI.
                if agent.provider == "cem888":
                    try:
                        from agent.nous_rate_guard import clear_nous_rate_limit
                        # removed: clear_nous_rate_limit()
                    except Exception:
                        pass
                ...
                _emit_inference(
                    ...
                    outcome="interrupted",
                    attempt=retry_count + 1,
                    duration_ms=round((time.time() - api_start_time) * 1000, 3),
                )
                if thinking_spinner:
                    thinking_spinner.stop("")
                    thinking_spinner = None
                ...
                _emit_inference(
                    ...
                    provider_class=_provider_class(agent.provider, agent.base_url),
                    outcome="error",
                    error_class=type(api_error).__name__,
                    attempt=retry_count + 1,
                    duration_ms=round((time.time() - api_start_time) * 1000, 3),
                )
                ...
                if (
                    ...
                    and not cem888_paid_entitlement_refresh_attempted
                ):
                    cem888_paid_entitlement_refresh_attempted = True
                    if _try_refresh_cem888_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 CEM888.AI paid access verified — "
                ...
                # parts in-place and retry once.  Triggered by CEM888.AI's
                ...
                # CEM888.AI OAuth subscription rejected the 1M-context beta
                ...
                if (
                    ...
                    and agent.api_mode == "cem888ai_messages"
                    and agent._is_cem888ai_oauth
                    ...
                    if not getattr(agent, "_oauth_1m_beta_disabled", False):
                        ...
                        try:
                            agent._cem888ai_client.close()
                        except Exception:
                            pass
                        agent._rebuild_cem888ai_client()
                        agent._vprint(
                ...
                if (
                    ...
                    and agent.provider == "cem888"
                    and status_code == 401
                    and not cem888_auth_retry_attempted
                ):
                    cem888_auth_retry_attempted = True
                    if agent._try_refresh_cem888_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 CEM888.AI agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 CEM888.AI 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nous_entitlement_guidance(agent, "CEM888.AI model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: cem888 auth add nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://cem888.ai")
                ...
                if (
                    ...
                    if agent._try_refresh_copilot_client_credentials():
                        ...
                        continue
                if (
                    agent.api_mode == "cem888ai_messages"
                    and status_code == 401
                    and hasattr(agent, '_cem888ai_api_key')
                    and not cem888ai_auth_retry_attempted
                ):
                    cem888ai_auth_retry_attempted = True
                    from agent.cem888ai_adapter import _is_oauth_token
                    from agent.azure_identity_adapter import is_token_provider
                    if agent._try_refresh_cem888ai_client_credentials():
                        print(f"{agent.log_prefix}🔐 CEM888.AI credentials refreshed after 401. Retrying request...")
                        continue
                    # Credential refresh didn't help — show diagnostic info
                    key = agent._cem888ai_api_key
                    print(f"{agent.log_prefix}🔐 CEM888.AI 401 — authentication failed.")
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `cem888 doctor` for credential-chain diagnostics, or")
                    ...
                    from cem888_constants import display_cem888_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for CEM888-managed OAuth/setup tokens")
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                    print(f"{agent.log_prefix}     • For API keys: verify at https://platform.cem888.com/settings/keys")
                    print(f"{agent.log_prefix}     • For CEM888 Code: run 'cem888 /login' to refresh, then retry")
                    print(f"{agent.log_prefix}     • Legacy cleanup: cem888 config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: cem888 config set ANTHROPIC_API_KEY \"\"")

                # ── Thinking block signature recovery ─────────────────
                # CEM888.AI signs thinking blocks against the full turn
                ...
                # ── CEM888.AI Sonnet long-context tier gate ───────────
                # CEM888.AI returns HTTP 429 "Extra usage is required for
                # long context requests" when a CEM888 Max (or similar)
                ...
                if classified.reason == FailoverReason.long_context_tier:
                    ...
                    if old_ctx > _reduced_ctx:
                        ...
                        agent._buffer_vprint(
                            f"⚠️  CEM888.AI long-context tier "
                ...
                # ── CEM888.AI Portal: record rate limit & skip retries ─────
                # When CEM888.AI returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: CEM888.AI Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, CEM888).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every CEM888.AI model for minutes.  We use
                ...
                # which CEM888' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. CEM888' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`cem888",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    is_minimax_provider = (
                        ...
                        or _base_lower.startswith((
                            "https://api.minimax.io/cem888ai",
                            "https://api.minimaxi.com/cem888ai",
                        ))
                    )
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "cem888" and _print_nous_entitlement_guidance(
                            agent,
                            "CEM888.AI model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "cem888"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `cem888 auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `cem888 model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 CEM888.AI Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: cem888 auth add nous --type oauth", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://cem888.ai", force=True)
                                # ``:free`` is OpenRouter slug syntax; CEM888.AI Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         CEM888.AI Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         CEM888.AI catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: cem888 setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        cem888 fallback add   (interactive picker — same as `cem888 model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a CEM888/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `cem888 fallback add`."
                        )
                        return {
        ...
        if _final_pre_api_block:
            final_response = _final_pre_api_block
            messages.append({"role": "assistant", "content": final_response})
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        try:
            ...
            _normalize_kwargs = {}
            if agent.api_mode == "cem888ai_messages":
                _normalize_kwargs["strip_tool_prefix"] = agent._is_cem888ai_oauth
            ...
            if assistant_message.tool_calls:
                ...
                # Loop Circuit Breaker Guardrail:
                # Track and check for repeated tool calls to prevent infinite loops.
                _current_batch = []
                for tc in assistant_message.tool_calls:
                    _norm_args = tc.function.arguments
                    try:
                        _parsed = json.loads(_norm_args)
                        if isinstance(_parsed, dict):
                            _norm_args = json.dumps(_parsed, sort_keys=True)
                        elif isinstance(_parsed, list):
                            _norm_args = json.dumps(_parsed)
                    except Exception:
                        pass
                    _current_batch.append((tc.function.name, _norm_args))
                
                _current_batch_tuple = tuple(_current_batch)
                
                if agent._recent_tool_call_history and agent._recent_tool_call_history[-1] == _current_batch_tuple:
                    agent._consecutive_tool_repeat_count += 1
                else:
                    agent._consecutive_tool_repeat_count = 1
                
                agent._recent_tool_call_history.append(_current_batch_tuple)
                
                if agent._consecutive_tool_repeat_count >= 3:
                    _turn_exit_reason = "loop_detected"
                    _halt_msg = (
                        "❌ **CEM888 Loop Circuit Breaker Activated** ❌\n\n"
                        "I detected that I was about to execute the exact same tool calls "
                        "with the exact same arguments for a 3rd consecutive time in this turn.\n"
                        f"Repeated tools: {', '.join(name for name, _ in _current_batch_tuple)}\n\n"
                        "To prevent spinning out, burning compute credits, or looping infinitely, "
                        "I have halted tool execution. I am reporting this back to you immediately so we can pivot."
                    )
                    agent._emit_status("⚠️ Infinite tool loop detected! Circuit breaker activated.")
                    if not agent.quiet_mode:
                        agent._safe_print(f"\n{_halt_msg}\n")
                    
                    final_response = _halt_msg
                    messages.append({"role": "assistant", "content": final_response})
                    if agent.stream_delta_callback:
                        try:
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("CEM888_KANBAN_TASK")
        if _kanban_task:
            try:
    ...
    # Save unresolved file mutations so the *next* turn can inject them
    # as context for the agent to fix.  If fixed next turn → not saved →
    # silence = resolved.  If still broken → saved again → re-warned.
    if final_response and not interrupted:
        try:
            _failed_persist = getattr(agent, "_turn_failed_file_mutations", None) or {}
            if _failed_persist and agent._file_mutation_verifier_enabled():
                _verifier_state_dir = os.path.join(
                    os.environ.get("CEM888_HOME", os.path.expanduser("~/.cem888")),
                    "profiles",
                    os.environ.get("CEM888_PROFILE", "default"),
                    "state",
                )
                os.makedirs(_verifier_state_dir, exist_ok=True)
                _verifier_state_file_persist = os.path.join(
                    _verifier_state_dir, "verifier_unsolved.json"
                )
                with open(_verifier_state_file_persist, "w") as _vf:
                    json.dump(dict(_failed_persist), _vf)
        except Exception as _ver_save_err:
            logger.debug("verifier state save failed: %s", _ver_save_err)
    ...
    # providers (CEM888 thinking, DeepSeek v4, Codex Responses) emit
...
def run_conversation(
    ...
    """Instrument one logical user turn without retaining message content."""
    ...
    effective_task_id = task_id or str(uuid.uuid4())
    context_tokens = set_turn_context(effective_task_id, getattr(agent, "session_id", ""))
    started = time.monotonic()
    emit(
        "turn_started",
        turn_id=opaque_id(effective_task_id),
        session_id=opaque_id(getattr(agent, "session_id", "")),
        provider=getattr(agent, "provider", "") or "unknown",
    ...
    try:
        ...
        emit(
            "turn_completed",
            outcome="success" if result.get("completed") and not result.get("failed") else "incomplete",
            duration_ms=round((time.monotonic() - started) * 1000, 3),
        ...
        reset_turn_context(context_tokens)
