...
def _cem888_home_path() -> Path:
    """Resolve the active CEM888_HOME (profile-aware) without circular imports."""
    try:
        from cem888_constants import get_cem888_home  # local import to avoid cycles
        return get_cem888_home()
    except Exception:
        return Path(os.path.expanduser("~/.cem888"))


def _cem888_root_path() -> Path:
    """Resolve the CEM888 root dir (always the parent of any profile, never per-profile)."""
    try:
        from cem888_constants import get_default_cem888_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    cem888_home = _cem888_home_path()
    cem888_root = _cem888_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(cem888_root / ".env"),
            # Active profile CEM888.AI PKCE credential store.
            str(cem888_home / ".cem888ai_oauth.json"),
            # Top-level CEM888.AI PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(cem888_root / ".cem888ai_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved CEM888_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("CEM888_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # CEM888 control-plane files: block both the ACTIVE profile's view
    # (cem888_home) AND the global root view. Without the root pass, a
    ...
    cem888_dirs = []
    for base in (_cem888_home_path(), _cem888_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in cem888_dirs:
                cem888_dirs.append(real)
        except Exception:
            continue

    for base_real in cem888_dirs:
    ...
    # Tier-0 protected-path enforcement. Outside declared protected roots the
    # existing general-purpose agent behavior is unchanged; inside CEM888's
    # agent/profile/repo namespace, the manifest decides and fails closed.
    try:
        from agent.authority import load_agent_authority

        authority = load_agent_authority()
        if authority is not None and authority.write_denial(resolved) is not None:
            return True
    except Exception:
        # A present/required invalid manifest is rejected during startup. Keep
        # this helper import-safe for setup/migration commands and legacy tests.
        pass

    return False


def get_authority_write_block_error(path: str) -> Optional[str]:
    """Return the structured Tier-0 denial for a protected write target."""
    try:
        from agent.authority import load_agent_authority

        authority = load_agent_authority(required=True)
        if authority is None:
            return None
        return authority.write_denial(path)
    except Exception as exc:
        return f"AUTHORITY_UNAVAILABLE: protected write authority could not be resolved: {exc}"
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied CEM888 path.

    Three categories are blocked:

      * Internal CEM888 cache files under ``CEM888_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under CEM888_HOME and the global CEM888
        root: ``auth.json``, ``auth.lock``, ``.cem888ai_oauth.json``,
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.cem888/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active CEM888_HOME (profile-aware) AND the global
    # CEM888 root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (CEM888_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    cem888_dirs: list[Path] = []
    for base in (_cem888_home_path(), _cem888_root_path()):
        try:
            real = base.resolve()
            if real not in cem888_dirs:
    ...
    for hd in cem888_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal CEM888 cache file "
    ...
    for hd in cem888_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a CEM888 credential store "
    ...
    for hd in cem888_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the CEM888 MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a CEM888 MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# CEM888 profiles are separate CEM888_HOME dirs under
...
#
# Reference: May 2026 incident where a cem888-security profile session
# edited skills under both ``~/.cem888/profiles/cem888-security/skills/``
# AND ``~/.cem888/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under CEM888_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from CEM888_HOME.
    ...
    """
    try:
        home_real = _cem888_home_path().resolve()
        root_real = _cem888_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside CEM888 scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    CEM888 entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to CEM888 profile {info['target_profile']!r}, but the "
