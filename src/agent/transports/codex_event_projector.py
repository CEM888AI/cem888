"""Projects codex app-server events into CEM888' messages list.

The translator that lets CEM888' memory/skill review keep working under the
...
CEM888' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape CEM888
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way CEM888' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but CEM888 stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way CEM888 already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — CEM888' messages store text only.
        text_parts: list[str] = []
