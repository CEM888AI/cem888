...
Owns one Codex thread per CEM888 session. Drives `turn/start`, consumes
...
# CEM888' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CEM888_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse CEM888/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per CEM888 session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CEM888_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CEM888_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="cem888",
            client_title="CEM888 Agent",
            client_version=_get_cem888_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but CEM888' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into CEM888' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own cem888-tools callback we
            # auto-accept — the user already approved CEM888' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map CEM888 approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    CEM888 returns 'once', 'session', 'always', or 'deny'.
...
def _get_cem888_version() -> str:
    """Best-effort CEM888 version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("cem888-agent")
