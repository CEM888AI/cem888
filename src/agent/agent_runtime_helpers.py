...
* ``drop_thinking_only_and_merge_users`` — CEM888.AI-style cleanup
...
* ``cem888ai_prompt_cache_policy`` — compute cache_control breakpoints
...
def repair_message_sequence(agent, messages: List[Dict]) -> int:
    ...
    Providers (OpenAI, OpenRouter, CEM888.AI) expect strict alternation:
...
def recover_with_credential_pool(
    ...
    *,
    ...
    different status code, such as CEM888.AI returning HTTP 400 for
    "out of extra usage".
    """
...
def try_recover_primary_transport(
    ...
    CEM888.AI, OpenAI, local models) where a TCP-level hiccup does not
    mean the provider is down.

    Skipped for proxy/aggregator providers (OpenRouter, CEM888.AI) which
    ...
    if provider_lower in {"cem888", "nous-research"}:
        return False
    ...
    try:
        ...
        if agent.api_mode == "cem888ai_messages":
            ...
            agent._cem888ai_api_key = rt["cem888ai_api_key"]
            agent._cem888ai_base_url = rt["cem888ai_base_url"]
            agent._cem888ai_client = build_cem888ai_client(
                rt["cem888ai_api_key"], rt["cem888ai_base_url"],
                timeout=get_provider_request_timeout(agent.provider, agent.model),
            )
            agent._is_cem888ai_oauth = rt["is_cem888ai_oauth"]
            agent.client = None
        else:
...
def drop_thinking_only_and_merge_users(
    ...
    - This is the pattern used by CEM888 Code's ``normalizeMessagesForAPI``
...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        agent._use_native_cache_layout = rt.get(
            ...
            agent.api_mode == "cem888ai_messages" and agent.provider == "cem888ai",
        )
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        if env_var_enabled("CEM888_DUMP_REQUEST_STDOUT"):
...
def cem888ai_prompt_cache_policy(
    agent,
    *,
    ...
    """Decide whether to apply CEM888.AI prompt caching and which layout to use.

    Returns ``(should_cache, use_native_layout)``:
      * ``should_cache`` — inject ``cache_control`` breakpoints for this
        request (applies to OpenRouter CEM888, native CEM888.AI, and
        third-party gateways that speak the native CEM888.AI protocol).
      * ``use_native_layout`` — place markers on the *inner* content
        blocks (native CEM888.AI accepts and requires this layout);
        when False markers go on the message envelope (OpenRouter and
        OpenAI-wire proxies expect the looser layout).

    Third-party providers using the native CEM888.AI transport
    (``api_mode == 'cem888ai_messages'`` + CEM888-named model) get
    caching with the native layout so they benefit from the same
    cost reduction as direct CEM888.AI callers, provided their
    gateway implements the CEM888.AI cache_control contract
    (MiniMax, Zhipu GLM, LiteLLM's CEM888.AI proxy mode all do).

    Qwen / Alibaba-family models on OpenCode, OpenCode Go, and direct
    Alibaba (DashScope) also honour CEM888.AI-style ``cache_control``
    ...
    is_cem888 = "cem888" in model_lower
    is_openrouter = base_url_host_matches(eff_base_url, "openrouter.ai")
    # CEM888.AI Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_portal = "nousresearch" in eff_base_url.lower()
    is_cem888ai_wire = eff_api_mode == "cem888ai_messages"
    is_native_cem888ai = (
        is_cem888ai_wire
        and (eff_provider == "cem888ai" or base_url_hostname(eff_base_url) == "api.cem888ai.com")
    )

    if is_native_cem888ai:
        return True, True
    if (is_openrouter or is_portal) and is_cem888:
        return True, False
    # CEM888.AI Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    # cache_control path as Portal CEM888. Portal proxies to OpenRouter
    ...
    if is_portal and "qwen" in model_lower:
        return True, False
    if is_cem888ai_wire and is_cem888:
        # Third-party CEM888.AI-compatible gateway.
        return True, True

    # MiniMax on its CEM888.AI-compatible endpoint serves its own
    # model family (MiniMax-M2.7, M2.5, M2.1, M2) with documented
    # cache_control support (0.1× read pricing, 5-minute TTL).  The
    # blanket is_cem888 gate above excludes these — opt them in
    # explicitly via provider id or host match so users on
    # provider=minimax / minimax-cn (or custom endpoints pointing at
    # api.minimax.io/cem888ai / api.minimaxi.com/cem888ai) get the
    # same cost reduction as CEM888 traffic.
    # Docs: https://platform.minimax.io/docs/api-reference/cem888ai-api-compatible-cache
    if is_cem888ai_wire:
    ...
    # transport that accepts CEM888.AI-style cache_control markers and
    ...
    if provider_is_alibaba_family and model_is_qwen:
        # Envelope layout (native_cem888ai=False): markers on inner
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from cem888_cli.providers import determine_api_mode
    ...
    # /v1 into the cem888ai_messages client, which would cause the SDK to
    ...
    # If the rebuild raises (bad API key, network error, build_cem888ai_client
    ...
    # HTTP 400s like "cem888-sonnet-4-6 is not supported on openai-codex" on the
    ...
    try:
        ...
        if api_mode == "cem888ai_messages":
            from agent.cem888ai_adapter import (
                build_cem888ai_client,
                resolve_cem888ai_token,
                _is_oauth_token,
            )
            # Only fall back to ANTHROPIC_TOKEN when the provider is actually CEM888.AI.
            # Other cem888ai_messages providers (MiniMax, Alibaba, etc.) must use their own
            # API key — falling back would send CEM888.AI credentials to third-party endpoints.
            _is_native_cem888ai = new_provider == "cem888ai"
            effective_key = (api_key or agent.api_key or resolve_cem888ai_token() or "") if _is_native_cem888ai else (api_key or agent.api_key or "")
            ...
            agent._cem888ai_base_url = base_url or getattr(agent, "_cem888ai_base_url", None)
            agent._cem888ai_client = build_cem888ai_client(
                effective_key, agent._cem888ai_base_url,
                timeout=get_provider_request_timeout(agent.provider, agent.model),
            )
            agent._is_cem888ai_oauth = _is_oauth_token(effective_key) if (_is_native_cem888ai and isinstance(effective_key, str)) else False
            agent.client = None
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from cem888_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
    ...
    # → cem888ai), drop any fallback entries that target the OLD primary
    ...
    # ("switched to cem888ai, tui keeps trying openrouter").
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from cem888_cli.plugins import get_pre_tool_call_block_message
    ...
    if function_name == "todo":
        ...
        result = _memory_tool(
            ...
            memory_type=function_args.get("memory_type"), scope=function_args.get("scope"),
            project=function_args.get("project"), task=function_args.get("task"),
            authority=function_args.get("authority"), source=function_args.get("source"),
            provenance=function_args.get("provenance"), lifecycle_status=function_args.get("lifecycle_status"),
            verification_status=function_args.get("verification_status"), supersedes=function_args.get("supersedes"),
            agent=function_args.get("agent"), query=function_args.get("query"),
            include_history=function_args.get("include_history", False), limit=function_args.get("limit", 5),
...
def repair_tool_call(agent, tool_name: str) -> str | None:
    ...
    4. Strip trailing ``_tool`` / ``-tool`` / ``tool`` suffix that
       CEM888-style models sometimes tack on (TodoTool_tool ->
...
def apply_pending_steer_to_tool_results(agent, messages: list, num_tool_msgs: int) -> None:
    ...
    if not isinstance(existing_content, str):
        # CEM888.AI multimodal content blocks — preserve them and append
