...
See the ``cem888-agent-dev`` skill (``references/self-improvement-loop.md``)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with CEM888, e.g. 'cem888-agent').\n"
    "  • Hub-installed skills (installed via 'cem888 skills install').\n"
    "Pinned skills (marked via 'cem888 curator pin') CAN be improved — "
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("CEM888_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require CEM888' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so CEM888
            ...
            # in the request body — CEM888.AI's cache key includes it.
            ...
            # CEM888.AI/OpenRouter prefix cache the parent warmed.
            # Without this, the fork rebuilds the system prompt from
            # scratch (fresh _cem888_now() timestamp, fresh
            ...
            guards_disabled = os.getenv(
                "CEM888_DISABLE_AUTHORITY_GUARDS", ""
