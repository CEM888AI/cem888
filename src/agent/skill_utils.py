...
from cem888_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``CEM888_PLATFORM`` or
            ``CEM888_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``cem888`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.cem888/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``cem888`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against CEM888_HOME, not cwd
        if not p.is_absolute():
            p = (cem888_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.cem888/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    cem888 = metadata.get("cem888") or {}
    if not isinstance(cem888, dict):
        cem888 = {}
    return {
        "fallback_for_toolsets": cem888.get("fallback_for_toolsets", []),
        "requires_toolsets": cem888.get("requires_toolsets", []),
        "fallback_for_tools": cem888.get("fallback_for_tools", []),
        "requires_tools": cem888.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    cem888 = metadata.get("cem888")
    if not isinstance(cem888, dict):
        return []
    raw = cem888.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes CEM888 metadata, VCS, virtualenv/dependency, and cache
