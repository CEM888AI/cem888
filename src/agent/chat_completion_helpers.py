...
from cem888_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from cem888_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    def _call():
        try:
            if agent.api_mode == "codex_responses":
                ...
                result["response"] = agent._cem888ai_messages_create(api_kwargs)
    ...
    # activity. Operators can tune via CEM888_CODEX_TTFB_TIMEOUT_SECONDS and
    # CEM888_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    _ttfb_timeout = _env_float("CEM888_CODEX_TTFB_TIMEOUT_SECONDS", 12.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("CEM888_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 25_000.0)
        _ttfb_strict = os.environ.get("CEM888_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            logger.info(
                ...
                "Set CEM888_CODEX_TTFB_STRICT=1 to force early reconnects.",
                f"{_est_tokens_for_codex_watchdog:,}",
                _ttfb_disable_above,
            )
        else:
            _ttfb_cap = _env_float("CEM888_CODEX_TTFB_MAX_SECONDS", 20.0)
            if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
                logger.info(
                    "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                    "(context=~%s tokens). Set CEM888_CODEX_TTFB_MAX_SECONDS to tune.",
                    _ttfb_timeout,
                    _ttfb_cap,
                    f"{_est_tokens_for_codex_watchdog:,}",
                )
    ...
    _codex_idle_timeout = _env_float(
        "CEM888_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    if agent.api_mode == "cem888ai_messages":
        ...
        cem888ai_messages = agent._prepare_cem888ai_messages_for_api(api_messages)
        ...
        return _transport.build_kwargs(
            ...
            is_oauth=agent._is_cem888ai_oauth,
            preserve_dots=agent._cem888ai_preserve_dots(),
            context_length=ctx_len,
            base_url=getattr(agent, "_cem888ai_base_url", None),
    ...
    # CEM888 max-output override on aggregators
    _ant_max = None
    if (_is_or or _is_nous) and "cem888" in (agent.model or "").lower():
        try:
            from agent.cem888ai_adapter import _get_cem888ai_max_output
            _ant_max = _get_cem888ai_max_output(agent.model)
        except Exception:
            pass
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "cem888",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects CEM888_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
    ...
    # (glm, MiniMax, gpt-5.x via aigw, CEM888.AI via openai-compat shims)
    ...
    if hasattr(assistant_message, 'reasoning_details') and assistant_message.reasoning_details:
        ...
        # CEM888.AI, OpenAI) can maintain reasoning continuity across turns.
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in cem888_cli/config.py).
        ...
        elif fb_provider == "cem888ai" or fb_base_url.rstrip("/").lower().endswith("/cem888ai"):
            fb_api_mode = "cem888ai_messages"
        elif _fb_is_azure:
        ...
        if fb_api_mode == "cem888ai_messages":
            # Build native CEM888.AI client instead of using OpenAI client
            from agent.cem888ai_adapter import build_cem888ai_client, resolve_cem888ai_token, _is_oauth_token
            effective_key = (fb_client.api_key or resolve_cem888ai_token() or "") if fb_provider == "cem888ai" else (fb_client.api_key or "")
            agent.api_key = effective_key
            agent._cem888ai_api_key = effective_key
            agent._cem888ai_base_url = fb_base_url
            agent._cem888ai_client = build_cem888ai_client(
                effective_key, agent._cem888ai_base_url, timeout=_fb_timeout,
            )
            agent._is_cem888ai_oauth = _is_oauth_token(effective_key) if fb_provider == "cem888ai" else False
            agent.client = None
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        # turns so CEM888.AI-family providers don't 400 the summary call.
        ...
        if agent.api_mode == "codex_responses":
            ...
            if agent.api_mode == "cem888ai_messages":
                ...
                _ant_kw = _tsum.build_kwargs(model=agent.model, messages=api_messages, tools=None,
                               ...
                               is_oauth=agent._is_cem888ai_oauth,
                               preserve_dots=agent._cem888ai_preserve_dots())
                summary_response = agent._cem888ai_messages_create(_ant_kw)
                _summary_result = _tsum.normalize_response(summary_response, strip_tool_prefix=agent._is_cem888ai_oauth)
        ...
        if final_response:
            ...
            if agent.api_mode == "codex_responses":
                ...
                _ant_kw2 = _tretry.build_kwargs(model=agent.model, messages=api_messages, tools=None,
                                ...
                                max_tokens=agent.max_tokens, reasoning_config=agent.reasoning_config,
                                preserve_dots=agent._cem888ai_preserve_dots())
                retry_response = agent._cem888ai_messages_create(_ant_kw2)
                _retry_result = _tretry.normalize_response(retry_response, strip_tool_prefix=agent._is_cem888ai_oauth)
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    - cem888ai_messages: client.messages.stream() via CEM888.AI SDK
    ...
    # callbacks — same UX as CEM888.AI and chat_completions streaming.
    if agent.api_mode == "bedrock_converse":
        result = {"response": None, "error": None}
    ...
    def _call_chat_completions():
        ...
        # wins over the CEM888_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else float(os.getenv("CEM888_API_TIMEOUT", 1800.0))
        )
        # Read timeout: config wins here too.  Otherwise use
        # CEM888_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = float(os.getenv("CEM888_STREAM_READ_TIMEOUT", 120.0))
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode CEM888_STREAM_READ_TIMEOUT.
    ...
    def _call_cem888ai():
        """Stream an CEM888.AI Messages API response.

        Fires delta callbacks for real-time token delivery, but returns
        the native CEM888.AI Message object from get_final_message() so
        ...
        _diag = agent._stream_diag_init()
        request_client_holder["diag"] = _diag
        # Use the CEM888.AI SDK's streaming context manager
        with agent._cem888ai_client.messages.stream(**api_kwargs) as stream:
            # The CEM888.AI SDK exposes the raw httpx response on
            ...
            # Return the native CEM888.AI Message for downstream processing
            return stream.get_final_message()

    def _call():
        import httpx as _httpx

        _max_stream_retries = int(os.getenv("CEM888_STREAM_RETRIES", 2))
        ...
        try:
            for _stream_attempt in range(_max_stream_retries + 1):
                ...
                try:
                    if agent.api_mode == "cem888ai_messages":
                        agent._try_refresh_cem888ai_client_credentials()
                        result["response"] = _call_cem888ai()
                    else:
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = float(os.getenv("CEM888_STREAM_STALE_TIMEOUT", 180.0))
    # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
    # for prefill on large contexts.  Disable the stale detector unless
    # the user explicitly set CEM888_STREAM_STALE_TIMEOUT.
