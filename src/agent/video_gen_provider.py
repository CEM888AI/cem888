...
as ``kind: backend``) or ``~/.cem888/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``cem888 tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CEM888_HOME/cache/videos/``, creating parents as needed."""
    from cem888_constants import get_cem888_home

    path = get_cem888_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CEM888_HOME/cache/videos/``.
