...
SUMMARY_PREFIX = (
    ...
    "IMPORTANT: Your persistent memory (memories/*_MANIFEST.md + the durable "
    "typed store) in the system "
...
# dimensions (CEM888.AI ≈ width×height/750, GPT-4o up to ~1700 for
...
# Matches CEM888 Code's IMAGE_TOKEN_ESTIMATE constant.
...
def _content_length_for_budget(raw_content: Any) -> int:
    ...
    (``image_url`` / ``input_image`` / CEM888.AI-style ``image``). This
...
def _is_image_part(part: Any) -> bool:
    ...
    Recognizes all three shapes the agent handles:
      ...
      - CEM888.AI native:        ``{"type": "image", "source": {...}}``
    """
    if not isinstance(part, dict):
        return False
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the cem888 compressor emits).
    """
    if not messages:
        return messages
    ...
    def _generate_summary(self, turns_to_summarize: List[Dict[str, Any]], focus_topic: str = None) -> Optional[str]:
        ...
        Args:
            focus_topic: Optional focus string for guided compression.  When
                ...
                everything else.  Inspired by CEM888 Code's ``/compact``.
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        for i in range(compress_start):
            ...
            if i == 0 and msg.get("role") == "system":
                ...
                _compression_note = "[Note: Some earlier conversation turns have been compacted into a handoff summary to preserve context space. The current session state may still reflect earlier work, so build on that summary and state rather than re-doing work. Your persistent memory (memories/*_MANIFEST.md + the durable typed store) remains fully authoritative regardless of compaction.]"
