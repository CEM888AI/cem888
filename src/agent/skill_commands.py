...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``CEM888_PLATFORM`` env var and
    ``CEM888_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("CEM888_PLATFORM")
            or get_session_env("CEM888_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("CEM888_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.cem888/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.cem888.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_cem888_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.cem888/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.cem888/skills/`` and any ``skills.external_dirs`` so the
...
def resolve_skill_command_key(command: str) -> Optional[str]:
    ...
    (which disallow hyphens, so ``/cem888-code`` is registered as
    ``/cem888_code`` and comes back in the underscored form).
