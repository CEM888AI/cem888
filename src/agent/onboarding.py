...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time CEM888 starts and finds ``~/.openclaw/``.

    Points users at ``cem888 claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``cem888 claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to CEM888, run "
        "`cem888 claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `cem888 claw cleanup` (renames it to ~/.openclaw.pre-migration — "
