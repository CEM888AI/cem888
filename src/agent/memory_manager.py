...
class MemoryManager:
    ...
    def prefetch_all(self, query: str, *, session_id: str = "") -> str:
        ...
        merged = "\n\n".join(parts)
        try:
            from agent.inference_telemetry import emit

            emit(
                "memory_prefetch_completed",
                outcome="success" if not provider_errors else "partial",
                provider_count=len(self._providers),
                provider_hits=len(parts),
                provider_errors=provider_errors,
                context_chars=len(merged),
                duration_ms=round((time.monotonic() - started) * 1000, 3),
            )
        except Exception:
            logger.debug("memory prefetch telemetry failed", exc_info=True)
        return merged
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``cem888_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_cem888_home()`` themselves.
        """
        if "cem888_home" not in kwargs:
            from cem888_constants import get_cem888_home
            kwargs["cem888_home"] = str(get_cem888_home())
        for provider in self._providers:
            try:
