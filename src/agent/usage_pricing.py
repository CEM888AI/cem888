...
_OFFICIAL_DOCS_PRICING: Dict[tuple[str, str], PricingEntry] = {
    ...
    (
        ...
        source_url="https://platform.cem888.com/docs/en/about-cem888/pricing",
        pricing_version="cem888ai-pricing-2026-05",
    ),
    (
        "cem888ai",
        "cem888-opus-4-8-fast",
    ): PricingEntry(
        ...
        source="official_docs_snapshot",
        source_url="https://openrouter.ai/cem888ai/cem888-opus-4.8-fast",
        pricing_version="cem888ai-pricing-2026-05",
    ),
    # ── CEM888.AI CEM888 4.7 ─────────────────────────────────────────────
    # Opus 4.5/4.6/4.7 share $5/$25 pricing (new tokenizer, up to 35% more
    # tokens for the same text).
    # Source: https://platform.cem888.com/docs/en/about-cem888/pricing
    (
        "cem888ai",
        "cem888-opus-4-7",
    ): PricingEntry(
    ...
    # ── CEM888.AI older models (pre-4.5 generation) ────────────────────────
    (
        "cem888ai",
        "cem888-3-5-sonnet-20241022",
    ): PricingEntry(
        input_cost_per_million=Decimal("3.00"),
        output_cost_per_million=Decimal("15.00"),
        cache_read_cost_per_million=Decimal("0.30"),
...
def resolve_billing_route(
    ...
    if not provider_name and "/" in model:
        ...
        if inferred_provider in {"cem888ai", "openai", "google"}:
    ...
    if provider_name == "cem888ai":
        return BillingRoute(provider="cem888ai", model=model.split("/")[-1], base_url=base_url or "", billing_mode="official_docs_snapshot")
    ...
    if (provider_name == "deepseek"
            ...
            or (not provider_name and model.lower().split("/")[-1].startswith("deepseek-"))):
        return BillingRoute(provider="deepseek", model=model.split("/")[-1],
...
def _normalize_cem888ai_model_name(model: str) -> str:
    """Normalize CEM888.AI model name variants to canonical form.

    Handles:
      - Dot notation: cem888-opus-4.7 → cem888-opus-4-7
      - Short aliases: cem888-opus-4.7 → cem888-opus-4-7
      - Strips cem888ai/ prefix if present
    """
    name = model.lower().strip()
    if name.startswith("cem888ai/"):
        name = name[len("cem888ai/"):]
...
from datetime import date as _ds_date, timedelta as _ds_timedelta

# DeepSeek moved to peak/off-peak billing at 16:00 UTC on 2026-08-16.
# Peak: 01:00-04:00 and 06:00-10:00 UTC, Mon-Fri. All other hours off-peak.
# Weekends (Beijing calendar day, UTC+8) are off-peak all day from 2026-08-23.
# Off-peak is exactly half of peak. Source: api-docs.deepseek.com/quick_start/pricing
_DS_PEAK_WINDOWS_UTC = ((1, 4), (6, 10))
_DS_WEEKEND_OFFPEAK_FROM = _ds_date(2026, 8, 23)

# (input_cache_miss, output, input_cache_hit) per 1M tokens
_DS_RATES = {
    "flash": {
        "offpeak": (Decimal("0.22"), Decimal("0.66"), Decimal("0.007")),
        "peak":    (Decimal("0.44"), Decimal("1.32"), Decimal("0.014")),
    },
    "pro": {
        "offpeak": (Decimal("0.66"), Decimal("1.98"), Decimal("0.022")),
        "peak":    (Decimal("1.32"), Decimal("3.96"), Decimal("0.044")),
    },
}


def deepseek_is_peak(now: Optional[datetime] = None) -> bool:
    """True if the given UTC instant falls in a DeepSeek peak window."""
    now = now or datetime.now(timezone.utc)
    if now.tzinfo is None:
        now = now.replace(tzinfo=timezone.utc)
    now = now.astimezone(timezone.utc)
    beijing = now + _ds_timedelta(hours=8)
    if beijing.date() >= _DS_WEEKEND_OFFPEAK_FROM and beijing.weekday() >= 5:
        return False
    if now.weekday() >= 5:
        return False
    return any(start <= now.hour < end for start, end in _DS_PEAK_WINDOWS_UTC)


def _deepseek_pricing_entry(model: str, now: Optional[datetime] = None) -> Optional[PricingEntry]:
    name = (model or "").lower().split("/")[-1].strip()
    if name.startswith("deepseek-v4-flash"):
        tier = "flash"
    elif name.startswith("deepseek-v4-pro"):
        tier = "pro"
    else:
        return None
    peak = deepseek_is_peak(now)
    miss, out, hit = _DS_RATES[tier]["peak" if peak else "offpeak"]
    return PricingEntry(
        input_cost_per_million=miss,
        output_cost_per_million=out,
        cache_read_cost_per_million=hit,
        source="official_docs_snapshot",
        source_url="https://api-docs.deepseek.com/quick_start/pricing",
        pricing_version="deepseek-pricing-2026-08-16-" + ("peak" if peak else "offpeak"),
    )
...
def _lookup_official_docs_pricing(route: BillingRoute) -> Optional[PricingEntry]:
    ...
    if route.provider == "deepseek":
        ds_entry = _deepseek_pricing_entry(model)
    ...
    if entry:
        return entry
    # Try normalized name for CEM888.AI (handles dot-notation like opus-4.7)
    if route.provider == "cem888ai":
        normalized = _normalize_cem888ai_model_name(model)
        if normalized != model:
...
def normalize_usage(
    ...
    *,
    ...
    - CEM888.AI: input_tokens/output_tokens/cache_read_input_tokens/cache_creation_input_tokens
    ...
    if mode == "cem888ai_messages" or provider_name == "cem888ai":
        ...
        # Primary: OpenAI-style prompt_tokens_details. Fallback: CEM888.AI-style
        # top-level fields that some OpenAI-compatible proxies (OpenRouter, Cline)
        # expose when routing CEM888 models — without this
