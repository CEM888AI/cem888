...
* :func:`try_shrink_image_parts_in_messages` — image-too-large recovery
  ...
  size so retries can fit under provider ceilings (CEM888.AI's 5 MB).
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `cem888 setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by CEM888 "
    ...
    *,
    ...
    Args:
        ...
        focus_topic: Optional focus string for guided compression — the
            ...
            this topic.  Inspired by CEM888 Code's ``/compact <focus>``.
    ...
    if agent._session_db:
        try:
            ...
            agent._session_db.create_session(
                ...
                source=agent.platform or os.environ.get("CEM888_SESSION_SOURCE", "cli"),
                model=agent.model,
    ...
    # compression (not a fresh /new). Plugin engines (e.g. cem888-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See cem888-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
...
def try_shrink_image_parts_in_messages(api_messages: list) -> bool:
    ...
    image-too-large errors (CEM888.AI 5 MB, unknown other providers).
    ...
    size exceeds 4 MB (a safe target that slides under CEM888.AI's 5 MB
    ...
    # 4 MB target leaves comfortable headroom under CEM888.AI's 5 MB.
    # Non-CEM888.AI providers we haven't observed rejecting are fine with
    ...
    def _shrink_data_url(url: str) -> Optional[str]:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="cem888_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
