...
``auth.json``.  They intentionally have no dependency on ``cem888_cli.auth`` so
...
# Sources CEM888 owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and CEM888-owned OAuth/device-code state)
