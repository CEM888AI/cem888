...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into CEM888 with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.cem888/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of CEM888
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``cem888 tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``cem888 setup`` for
