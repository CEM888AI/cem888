...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # CEM888.AI tool-gateway (subscribers)
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + CEM888.AI Subscriber token are available.

    Reads ``read_cem888_access_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._read_cem888_access_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._read_cem888_access_token
    ) is not None
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``cem888 tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_cem888_tools_enabled():
        return ""
    return (
        ", or use the CEM888.AI Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_cem888_tools_enabled():
        message += (
            " With your CEM888.AI subscription you can also use the Tool Gateway. "
            "run `cem888 tools` and select CEM888.AI Subscription as the web provider."
        )
    else:
        message += " " + _wt.cem888_tool_gateway_unavailable_message(
...
def _get_firecrawl_client() -> Any:
    ...
    ``_read_cem888_access_token``, ``Firecrawl``) are also looked up via
    ...
    if direct_config is not None and not _wt.prefers_gateway("web"):
        ...
        kwargs = {
            "api_key": managed_gateway.cem888_user_token,
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "tag": (
                ...
                "CEM888.AI tool-gateway routing."
            ),
            "env_vars": [
                {
