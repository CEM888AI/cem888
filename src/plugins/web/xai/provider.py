...
``{title, url, description, position}`` rows every other CEM888 web
provider produces.
...
prefers CEM888-managed xAI Grok OAuth (via ``cem888 auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.cem888/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``cem888 tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `cem888 auth` to sign in with "
