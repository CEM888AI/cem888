...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with cem888-agent)
2. User-installed providers: ``$CEM888_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$CEM888_HOME/plugins/`` or None if unavailable."""
    try:
        from cem888_constants import get_cem888_home
        d = get_cem888_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$CEM888_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_cem888_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_cem888_user_memory.{active_provider}.cli"
    try:
