...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: cem888)
...
Or via $CEM888_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so CEM888 can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $CEM888_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_cem888_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "cem888": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "cem888"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this CEM888 config."""
    profile = config.get("profile", "cem888")
    return str(profile or "cem888")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active CEM888 profile name (from agent_identity)
      {workspace} — CEM888 workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``cem888-{user}`` with no user becomes ``cem888``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between CEM888 Agent and the User"
    ...
    def save_config(self, values, cem888_home):
        """Write config to $CEM888_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(cem888_home) / "hindsight"
    ...
    def post_setup(self, cem888_home: str, config: dict) -> None:
        ...
        from cem888_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "cem888")
        ...
        self.save_config(provider_config, cem888_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(cem888_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(cem888_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            # Local embedded mode
            {"key": "llm_provider", "description": "LLM provider", "default": "openai", "choices": ["openai", "cem888ai", "gemini", "groq", "openrouter", "minimax", "ollama", "lmstudio", "openai_compatible"], "when": {"mode": "local_embedded"}},
            {"key": "llm_base_url", "description": "Endpoint URL (e.g. http://192.168.1.10:8080/v1)", "default": "", "when": {"mode": "local_embedded", "llm_provider": "openai_compatible"}},
            {"key": "llm_api_key", "description": "LLM API key (optional for openai_compatible)", "secret": True, "env_var": "HINDSIGHT_LLM_API_KEY", "when": {"mode": "local_embedded"}},
            {"key": "llm_model", "description": "LLM model", "default": "gpt-4o-mini", "default_from": {"field": "llm_provider", "map": _PROVIDER_DEFAULT_MODELS}, "when": {"mode": "local_embedded"}},
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "cem888"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: cem888-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between CEM888 Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "cem888"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "cem888"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "cem888", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "cem888")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between CEM888 Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See cem888-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When CEM888 created/used that client on the shared async loop,
