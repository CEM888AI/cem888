...
def _load_supermemory_config(cem888_home: str) -> dict:
    config = _default_config()
    config_path = Path(cem888_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, cem888_home: str) -> None:
    config_path = Path(cem888_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `cem888 memory setup`.
        # All other options are documented for $CEM888_HOME/supermemory.json
    ...
    def save_config(self, values, cem888_home):
        ...
        _save_supermemory_config(sanitized, cem888_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from cem888_constants import get_cem888_home
        self._cem888_home = kwargs.get("cem888_home") or str(get_cem888_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._cem888_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "cem888", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "cem888_memory", "target": target, "type": "explicit_memory"},
