"""ACP auth helpers — detect and advertise CEM888 authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "cem888-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active CEM888 runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if CEM888 can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for CEM888.
    ...
    not have CEM888 provider credentials configured yet, so CEM888 always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate CEM888 using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure CEM888 provider",
            description=(
                "Open CEM888' interactive model/provider setup in a terminal. "
                "Use this when CEM888 has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
