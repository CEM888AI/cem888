"""ACP session manager — maps ACP sessions to CEM888 AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.cem888/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when CEM888 itself is running in WSL.

    Windows ACP clients can launch ``cem888 acp`` inside WSL while still sending
    ...
    """
    from cem888_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch CEM888 from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["cem888-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed CEM888 agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by CEM888 AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current CEM888 runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.cem888/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``CEM888_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_cem888_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=cem888_home / "state.db")
            return self._db_instance
        except Exception:
