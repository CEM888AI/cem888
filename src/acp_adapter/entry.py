"""CLI entry point for the cem888-agent ACP adapter.

Loads environment variables from ``~/.cem888/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from CEM888_HOME (default ``~/.cem888``)."""
    from cem888_cli.env_loader import load_cem888_dotenv

    cem888_home = get_cem888_home()
    loaded = load_cem888_dotenv(cem888_home=cem888_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", cem888_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="cem888-acp",
        description="Run CEM888 Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print CEM888 version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(cem888_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import CEM888ACPAgent  # noqa: F401

    print("CEM888 ACP check OK")


def _run_setup() -> None:
    from cem888_cli.main import main as cem888_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "cem888", "model"]
        cem888_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``cem888 postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting cem888-agent ACP adapter")
    ...
    import acp
    from .server import CEM888ACPAgent
