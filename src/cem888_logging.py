"""Centralized logging setup for CEM888 Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.cem888/logs/`` (profile-aware via ``get_cem888_home()``).
...
from cem888_constants import get_config_path, get_cem888_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_cem888_session_injector", False):
    ...
    _session_record_factory._cem888_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``cem888 logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "cem888_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the CEM888 logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    cem888_home
        Override for the CEM888 home directory.  Falls back to
        ``get_cem888_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_cem888_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._cem888_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the cem888 group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from cem888_cli.config import is_managed
