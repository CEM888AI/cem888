"""Slash command definitions and autocomplete for the CEM888 CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of CEM888 config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal CEM888 works on across turns until achieved", "Session",
    ...
    CommandDef("fast", "Toggle fast mode — OpenAI Priority Processing / CEM888.AI Fast Mode (Normal/Fast)", "Configuration",
    ...
    CommandDef("busy", "Control what Enter does while CEM888 is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.cem888/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update CEM888 Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`cem888_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/cem888`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full CEM888
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``cem888 skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``cem888 skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``cem888 skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/cem888 <verb>`` subcommand.
    ...
    can still reach them via ``/cem888 <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/cem888`` is always reserved as the first entry so the
    legacy ``/cem888 <subcommand>`` form keeps working for anything that
    ...
    # Reserve /cem888 as the catch-all top-level command.
    entries.append(("cem888", "Talk to CEM888 or run a subcommand", "[subcommand] [args]"))
    seen.add("cem888")
...
def slack_app_manifest(request_url: str = "https://cem888-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /cem888 handler.

    Maps both canonical names and aliases so /cem888 bg do stuff works
    the same as /cem888 background do stuff.

    Plugin-registered slash commands are included so ``/cem888 <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _context_completions(self, word: str, limit: int = 30):
        """Yield CEM888 Code-style @ context completions.
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from cem888_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def get_completions(self, document, complete_event):
        ...
        if not text.startswith("/"):
            # Try @ context completion (CEM888 Code-style)
