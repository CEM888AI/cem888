...
Used by ``cem888 profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``cem888_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the CEM888 Agent kanban board.
...
Rules:
  ...
  - Never write "CEM888 Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from cem888_constants import get_cem888_home  # type: ignore
            profile_dir = Path(get_cem888_home())
        else:
