...
Add, remove, or reorder entries here — both `cem888 setup` and
`cem888` provider-selection will pick up the change automatically.
"""
...
from cem888_cli import __version__ as _CEM888_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_CEM888_USER_AGENT = f"cem888-cli/{_CEM888_VERSION}"
...
OPENROUTER_MODELS: list[tuple[str, str]] = [
    ("cem888ai/cem888-opus-4.8",              ""),
    ("cem888ai/cem888-opus-4.8-fast",         "2x price, higher output speed"),
    ("cem888ai/cem888-opus-4.7",              ""),
    ("cem888ai/cem888-opus-4.6",              ""),
    ("cem888ai/cem888-sonnet-4.6",            ""),
    ("moonshotai/kimi-k2.6",                   "recommended"),
    ("openrouter/pareto-code",                 "auto-routes to cheapest coder meeting openrouter.min_coding_score"),
    ("qwen/qwen3.7-max",                       ""),
    ("cem888ai/cem888-haiku-4.5",             ""),
    ("openai/gpt-5.5",                         ""),
    ("openai/gpt-5.5-pro",                     ""),
    ("openai/gpt-5.4-mini",                    ""),
    ("openai/gpt-5.4-nano",                    ""),
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `cem888 model`
    flow without maintaining a separate static list.
    """
    from cem888_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $CEM888_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CEM888_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. CEM888 refreshes the cache from
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    "cem888ai": [
        ...
        "cem888-opus-4-6",
        "cem888-sonnet-4-6",
        "cem888-opus-4-5-20251101",
        "cem888-sonnet-4-5-20250929",
        "cem888-opus-4-20250514",
        "cem888-sonnet-4-20250514",
        "cem888-haiku-4-5-20251001",
    ],
    "deepseek": [
        "deepseek-v4-pro",
        "deepseek-v4-flash",
    ...
    # or https://dashscope-intl.aliyuncs.com/apps/cem888ai (CEM888.AI-compat).
    "alibaba": [
        "qwen3.7-max",
        "qwen3.6-plus",
        "kimi-k2.5",
    ...
    "bedrock": [
        ...
        "us.cem888ai.cem888-opus-4-6-v1",
        "us.cem888ai.cem888-haiku-4-5-20251001-v1:0",
        "us.cem888ai.cem888-sonnet-4-5-20250929-v1:0",
...
# ---------------------------------------------------------------------------
# CEM888.AI Portal free-model helper
# ---------------------------------------------------------------------------
# The CEM888.AI Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# CEM888.AI Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_cem888_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user's CEM888.AI Portal account/subscription info.
    ...
    """
    base = (portal_base_url or "https://cem888.ai").rstrip("/")
...
def is_cem888_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_cem888_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split CEM888.AI models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/cem888/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["cem888"]`` list happens to contain or whether the
    ...
    older CEM888 that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/cem888/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["cem888"]`` list happens to contain or whether
    ...
    picker even if the user is running an older CEM888 that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_cem888_models_by_tier` keeps free models selectable;
...
def check_cem888_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current CEM888.AI Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# CEM888.AI Portal recommended models
#
...
CEM888_RECOMMENDED_MODELS_PATH = "/api/cem888/recommended-models"
_CEM888_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_cem888_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def fetch_cem888_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the CEM888.AI Portal's curated recommended-models payload.

    Hits ``<portal>/api/cem888/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_CEM888_RECOMMENDED_CACHE_TTL`` seconds; pass ``force_refresh=True`` to
    bypass the cache.
    ...
    base = (portal_base_url or "https://cem888.ai").rstrip("/")
    now = time.monotonic()
    cached = _cem888_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _CEM888_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{CEM888_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    _cem888_recommended_cache[base] = (data, now)
    return data
...
def _resolve_portal_url() -> str:
    ...
    try:
        from cem888_cli.auth import (
            DEFAULT_CEM888_PORTAL_URL,
            get_provider_auth_state,
        )
        state = get_provider_auth_state("cem888") or {}
        portal = str(state.get("portal_base_url") or "").strip()
        if portal:
            return portal.rstrip("/")
        return str(DEFAULT_CEM888_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://cem888.ai"
...
def get_cem888_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_cem888_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_portal_url()
    payload = fetch_cem888_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_cem888_free_tier()
        except Exception:
...
# this list:  cem888 model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `cem888 model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `cem888 model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("cem888",           "CEM888.AI Portal",              "CEM888.AI Portal (CEM888.AI subscription)"),
    ProviderEntry("openrouter",     "OpenRouter",               "OpenRouter (100+ models, pay-per-use)"),
    ProviderEntry("novita",         "NovitaAI",                 "NovitaAI (AI-native cloud: Model API, Agent Sandbox, GPU Cloud)"),
    ProviderEntry("lmstudio",       "LM Studio",                "LM Studio (local desktop app with built-in model server)"),
    ProviderEntry("cem888ai",      "CEM888.AI",                "CEM888.AI (CEM888 models — API key or CEM888 Code)"),
    ...
    ProviderEntry("bedrock",        "AWS Bedrock",              "AWS Bedrock (CEM888, Nova, Llama, DeepSeek — IAM or API key)"),
    ProviderEntry("azure-foundry",  "Azure Foundry",            "Azure Foundry (OpenAI-style or CEM888.AI-style endpoint — your Azure AI deployment)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``cem888 model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``cem888 auth add openai-codex`` without
    ``cem888 model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    cem888-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (CEM888.AI Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from cem888_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — cem888-agent
...
def get_curated_cem888_model_ids() -> list[str]:
    """Return the curated CEM888.AI Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["cem888"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from cem888_cli.model_catalog import get_curated_cem888_models
        remote = get_curated_cem888_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("cem888", []))
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, CEM888.AI Portal).
    """
...
_DEFAULT_CEM888_INFERENCE_BASE = "https://api.cem888ai.com"


def _resolve_cem888_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for CEM888.AI Portal pricing.

    The CEM888.AI inference ``/v1/models`` endpoint exposes pricing without
    ...
    """
    try:
        from cem888_cli.auth import resolve_cem888_runtime_credentials
        creds = resolve_cem888_runtime_credentials()
        if creds:
            return (creds.get("api_key", ""), creds.get("base_url", ""))
    except Exception:
        pass
    return ("", _DEFAULT_CEM888_INFERENCE_BASE)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, cem888, novita)."""
    ...
    if normalized == "cem888":
        api_key, base_url = _resolve_cem888_pricing_credentials()
        if base_url:
            # CEM888.AI base_url typically looks like https://api.cem888ai.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``cem888 model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from cem888_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        openrouter:cem888ai/cem888-sonnet-4.5  →  ("openrouter", "cem888ai/cem888-sonnet-4.5")
        cem888:cem888-3                           →  ("cem888", "cem888-3")
        cem888ai/cem888-sonnet-4.5             →  (current_provider, "cem888ai/cem888-sonnet-4.5")
        gpt-5.4                                 →  (current_provider, "gpt-5.4")

    The colon is only treated as a provider delimiter if the left side is a
    recognized provider name or alias.  This avoids misinterpreting model names
    that happen to contain colons (e.g. ``cem888ai/cem888-3.5-sonnet:beta``).
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, CEM888.AI, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"cem888", "openrouter", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from cem888_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model cem888` or `/model cem888ai`, treat it as a
...
def _find_openrouter_slug(model_name: str) -> Optional[str]:
    ...
    Handles:
    - Exact match: ``cem888ai/cem888-opus-4.6`` → as-is
    - Bare name: ``deepseek-chat`` → ``deepseek/deepseek-chat``
    - Bare name: ``cem888-opus-4.6`` → ``cem888ai/cem888-opus-4.6``
    """
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to CEM888' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``cem888_cli.auth.resolve_provider()`` to resolve it to a concrete
...
# Models that support CEM888.AI Fast Mode (speed="fast").
# See https://platform.cem888.com/docs/en/build-with-cem888/fast-mode
#
# Pattern-based matching — any cem888-* model is eligible. The cem888ai
# adapter gates speed=fast on native CEM888.AI endpoints only (see
# _is_third_party_cem888ai_endpoint in agent/cem888ai_adapter.py), so
# third-party proxies that would reject the beta header are protected.


def _strip_vendor_prefix(model_id: str) -> str:
    """Strip vendor/ prefix from a model ID (e.g. 'cem888ai/cem888-opus-4-6' -> 'cem888-opus-4-6')."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether CEM888 should expose the /fast toggle for this model."""
    return _is_cem888ai_fast_model(model_id) or _is_openai_fast_model(model_id)


def _is_cem888ai_fast_model(model_id: Optional[str]) -> bool:
    """Return True if the model is a CEM888 model eligible for CEM888.AI Fast Mode.

    Fast mode is currently supported on CEM888 Opus 4.6 only. Per CEM888.AI's
    docs (https://platform.cem888.com/docs/en/build-with-cem888/fast-mode):
...
def resolve_fast_mode_overrides(model_id: Optional[str]) -> dict[str, Any] | None:
    ...
    - CEM888.AI models: ``{"speed": "fast"}`` (CEM888.AI Fast Mode beta)

    The overrides are injected into the API request kwargs by
    ``_build_api_kwargs`` in run_agent.py — each API path handles its own
    keys (service_tier for OpenAI/Codex, speed for CEM888.AI Messages).
    """
    if not model_supports_fast_mode(model_id):
        return None
    if _is_cem888ai_fast_model(model_id):
        return {"speed": "fast"}
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``cem888 auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.cem888/.env``.
    ...
    """
    try:
        from cem888_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from cem888_cli.auth import read_credential_pool
        from cem888_cli.copilot_auth import (
...
#   - "cem888": curated list and Portal /models endpoint are the source of
#     truth for the subscription tier.
# Also excluded: providers that already have dedicated live-endpoint
# branches below (copilot, cem888ai, ollama-cloud, custom,
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, CEM888.AI),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a CEM888 release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a CEM888 release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "cem888":
        # Try live CEM888.AI Portal /models endpoint
        try:
            from cem888_cli.auth import fetch_cem888_models, resolve_cem888_runtime_credentials
            creds = resolve_cem888_runtime_credentials()
            if creds:
                live = fetch_cem888_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["cem888"] snapshot — so newly
        # added Portal models still surface without a CEM888 release.
        manifest_ids = get_curated_cem888_model_ids()
        if manifest_ids:
            return manifest_ids
...
# provider's /v1/models endpoint. On a well-configured user (cem888ai +
# openai + copilot + gemini + huggingface + ...) that's 2+ seconds of cold
# HTTP roundtrips just to render the provider list.
#
# Cache strategy:
#   - One JSON file at $CEM888_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_cem888_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, cem888ai-via-cem888-code, cem888 portal), the
    relevant tokens live in ``$CEM888_HOME/auth.json`` and external
    ...
    for path in (
        ...
        _os.path.expanduser("~/.cem888/.credentials.json"),
...
def _fetch_cem888ai_models(timeout: float = 5.0) -> Optional[list[str]]:
    """Fetch available models from the CEM888.AI /v1/models endpoint.

    Uses resolve_cem888ai_token() to find credentials (env vars or
    CEM888 Code auto-discovery).  Returns sorted model IDs or None.
    """
    try:
        from agent.cem888ai_adapter import resolve_cem888ai_token, _is_oauth_token
    except ImportError:
        return None
    ...
    headers: dict[str, str] = {"cem888ai-version": "2023-06-01"}
    is_oauth = _is_oauth_token(token)
    if is_oauth:
        headers["Authorization"] = f"Bearer {token}"
        from agent.cem888ai_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
        headers["cem888ai-beta"] = ",".join(_COMMON_BETAS + _OAUTH_ONLY_BETAS)
    else:
        headers["x-api-key"] = token

    def _do_request(h: dict[str, str]):
        req = urllib.request.Request(
            "https://api.cem888ai.com/v1/models",
            headers=h,
        )
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            return json.loads(resp.read().decode())
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).debug("Failed to fetch CEM888.AI models: %s", e)
        return None
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _CEM888_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    "cem888ai/cem888-opus-4.6": "cem888-opus-4.6",
    "cem888ai/cem888-sonnet-4.6": "cem888-sonnet-4.6",
    "cem888ai/cem888-sonnet-4": "cem888-sonnet-4",
    "cem888ai/cem888-sonnet-4.5": "cem888-sonnet-4.5",
    "cem888ai/cem888-haiku-4.5": "cem888-haiku-4.5",
    # Dash-notation fallbacks: CEM888' default CEM888 IDs elsewhere use
    # hyphens (cem888ai native format), but Copilot's API only accepts
    # dot-notation.  Accept both so users who configure copilot + a
    # default hyphenated CEM888 model don't hit HTTP 400
    # "model_not_supported".  See issue #6879.
    "cem888-opus-4-6": "cem888-opus-4.6",
    "cem888-sonnet-4-6": "cem888-sonnet-4.6",
    "cem888-sonnet-4-0": "cem888-sonnet-4",
    "cem888-sonnet-4-5": "cem888-sonnet-4.5",
    "cem888-haiku-4-5": "cem888-haiku-4.5",
    "cem888ai/cem888-opus-4-6": "cem888-opus-4.6",
    "cem888ai/cem888-sonnet-4-6": "cem888-sonnet-4.6",
    "cem888ai/cem888-sonnet-4-0": "cem888-sonnet-4",
    "cem888ai/cem888-sonnet-4-5": "cem888-sonnet-4.5",
    "cem888ai/cem888-haiku-4-5": "cem888-haiku-4.5",
}
...
def _should_use_copilot_responses_api(model_id: str) -> bool:
    ...
    Chat Completions.  All non-GPT models (CEM888, Gemini, etc.) use
    Chat Completions.
    """
    import re
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # Secondary: check catalog for non-GPT-5 models (CEM888 via /v1/messages, etc.)
    if catalog:
...
def azure_foundry_model_api_mode(model_name: Optional[str]) -> Optional[str]:
    ...
    Intentionally does NOT return ``cem888ai_messages``; CEM888.AI-style
    Azure endpoints are disambiguated by URL (``/cem888ai`` suffix) in
    ``runtime_provider._detect_api_mode_for_url`` and by the user setting
    ``model.api_mode: cem888ai_messages`` explicitly.
    """
...
def opencode_model_api_mode(provider_id: Optional[str], model_id: Optional[str]) -> str:
    ...
    - CEM888 models on Zen use ``/v1/messages``
    ...
    if provider == "opencode-zen":
        if normalized.startswith("cem888-"):
            return "cem888ai_messages"
...
def probe_api_models(
    ...
    For ``cem888ai_messages`` mode, uses ``x-api-key`` and
    ``cem888ai-version`` headers (CEM888.AI's native auth) instead of
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _CEM888_USER_AGENT}
    if api_key and api_mode == "cem888ai_messages":
        headers["x-api-key"] = api_key
        headers["cem888ai-version"] = "2023-06-01"
    elif api_key:
        headers["Authorization"] = f"Bearer {api_key}"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from cem888_constants import get_cem888_home
    return get_cem888_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"CEM888 will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "cem888ai_messages":
            message += (
                "\n  Many CEM888.AI-compatible proxies do not implement the Models API "
        ...
        return {
            "accepted": api_mode == "cem888ai_messages",
            "persist": True,
            "recognized": False,
            "message": message,
        }
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so CEM888 cannot verify the model name."
                    "\n  The model may still work if it exists on the server."
                ),
            }

    # Native CEM888.AI provider: /v1/models requires x-api-key (or Bearer for
    # OAuth) plus cem888ai-version headers.  The generic OpenAI-style probe
    # below uses plain Bearer auth and 401s against CEM888.AI, so dispatch to
    # the native fetcher which handles both API keys and CEM888-Code OAuth
    # tokens.  (The api_mode=="cem888ai_messages" branch below handles the
    # Messages-API transport case separately.)
    if normalized == "cem888ai":
        cem888ai_models = _fetch_cem888ai_models()
        if cem888ai_models is not None:
            if requested_for_lookup in set(cem888ai_models):
                return {
                    "accepted": True,
                    "persist": True,
                    "recognized": True,
            ...
            auto = get_close_matches(requested_for_lookup, cem888ai_models, n=1, cutoff=0.9)
            if auto:
                return {
                    "accepted": True,
                    "persist": True,
            ...
            suggestions = get_close_matches(requested, cem888ai_models, n=3, cutoff=0.5)
            suggestion_text = ""
            if suggestions:
                suggestion_text = "\n  Similar models: " + ", ".join(f"`{s}`" for s in suggestions)
            # Accept anyway — CEM888.AI sometimes gates newer/preview models
            ...
            return {
                ...
                "recognized": False,
                "message": (
                    f"Note: `{requested}` was not found in CEM888.AI's /v1/models listing. "
                    f"It may still work if you have early-access or snapshot IDs."
                    f"{suggestion_text}"
                ),
            }
        # _fetch_cem888ai_models returned None — no token resolvable or
        # network failure.  Fall through to the generic warning below.

    # CEM888.AI Messages API: many proxies don't implement /v1/models.
    ...
    if api_mode == "cem888ai_messages":
        ...
        # doesn't implement the CEM888.AI Models API).
        return {
            "accepted": True,
            "persist": True,
            "recognized": False,
            "message": (
                ...
                f"model listing.  Many CEM888.AI-compatible proxies do not "
