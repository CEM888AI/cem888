"""
Doctor command for cem888 CLI.

Diagnoses issues with CEM888 Agent setup.
"""
...
from cem888_cli.config import get_project_root, get_cem888_home, get_env_path
...
_DHH = display_cem888_home()  # user-facing display path (e.g. ~/.cem888 or ~/.cem888/profiles/coder)

# Load environment variables from ~/.cem888/.env so API key checks work
_env_path = get_env_path()
load_cem888_dotenv(cem888_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from cem888_cli.colors import Colors, color
from cem888_cli.models import _CEM888_USER_AGENT
from cem888_constants import OPENROUTER_MODELS_URL
...
from cem888_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.cem888/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("CEM888_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from cem888_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from cem888_cli.auth import get_minimax_oauth_auth_status
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-cem888 and dashboard static services are up
    ...
    for static in ("main-cem888", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `cem888 profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    # check (e.g. CEM888.AI native API requires x-api-key, not Bearer).
    _dedicated_canonical = {"cem888ai", "openrouter", "bedrock"}
    _known_canonical.update(_dedicated_canonical)
    try:
        from providers import list_providers
        from providers.base import ProviderProfile as _PP
        try:
            from cem888_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `cem888`.
    os.environ.setdefault("CEM888_INTERACTIVE", "1")

    # Handle `cem888 doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.cem888/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 CEM888 Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`cem888 doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.cem888/.env (primary location for user config)
    env_path = CEM888_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'cem888 setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'cem888 setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'cem888 setup' to create one")
                issues.append("Run 'cem888 setup' to create .env")
    
    # Check ~/.cem888/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = CEM888_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from cem888_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from cem888_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'cem888 config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            # types (OAuth, SDK, cem888ai/custom/auto) have their own env-var
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from cem888_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.cem888/.env or run 'cem888 setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'cem888 setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'cem888 config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from cem888_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from cem888_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'cem888 setup' to migrate config")
                else:
                    issues.append("Run 'cem888 doctor --fix' or 'cem888 setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'cem888 doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from cem888_cli.auth import (
            get_cem888_auth_status,
            get_codex_auth_status,
            get_gemini_oauth_auth_status,
            get_minimax_oauth_auth_status,
        )

        cem888_status = get_cem888_auth_status()
        if cem888_status.get("logged_in"):
            check_ok("CEM888.AI Portal auth", "(logged in)")
        else:
            check_warn("CEM888.AI Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses CEM888' own device-code flow — the Codex CLI is
    ...
    # disrupt the already-printed CEM888.AI/Codex/Gemini/MiniMax rows above.
    try:
        from cem888_cli.auth import get_xai_oauth_auth_status
    ...
    cem888_home = CEM888_HOME
    if cem888_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        cem888_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = cem888_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give CEM888 a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# CEM888 Agent Persona\n\n"
                "<!-- Edit this file to customize how CEM888 communicates. -->\n\n"
                "You are CEM888, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = cem888_home / "memories"
    if memories_dir.exists():
        check_ok(f"{_DHH}/memories/ directory exists")
        # CEM-170: the injected manifests are *_MANIFEST.md.  The old bare names
        # (memories/MEMORY.md, memories/USER.md) collided with the REAL profile
        # file at profile-root USER.md and were renamed; check the new names, and
        # accept a not-yet-migrated legacy file rather than reporting a healthy
        # profile as broken.
        memory_file = memories_dir / "MEMORY_MANIFEST.md"
        user_file = memories_dir / "USER_MANIFEST.md"
        if not memory_file.exists() and (memories_dir / "MEMORY.md").exists():
            memory_file = memories_dir / "MEMORY.md"
        if not user_file.exists() and (memories_dir / "USER.md").exists():
            user_file = memories_dir / "USER.md"
        if memory_file.exists():
            size = len(memory_file.read_text(encoding="utf-8").strip())
            check_ok(f"{memory_file.name} exists ({size} chars)")
        else:
            check_info(f"{memory_file.name} not created yet (will be created when the agent first writes a memory)")
        if user_file.exists():
            size = len(user_file.read_text(encoding="utf-8").strip())
            check_ok(f"{user_file.name} exists ({size} chars)")
        else:
            check_info(f"{user_file.name} not created yet (will be created when the agent first writes a memory)")
    else:
    ...
    state_db_path = cem888_home / "state.db"
    ...
    wal_path = cem888_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'cem888 doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "cem888"
        ...
        _cmd_link = _cmd_link_dir / "cem888"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(cem888 not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/cem888 → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/cem888 points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/cem888 → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/cem888 — run 'cem888 doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/cem888 exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/cem888 not found",
                    "(cem888 command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/cem888 → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/cem888 symlink — run 'cem888 doctor --fix'")
    ...
    try:
        from cem888_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `cem888 doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'cem888 config set model.provider <provider>' "
    ...
    def _probe_cem888ai() -> _ConnectivityResult:
        from cem888_cli.auth import get_cem888ai_key
        key = get_cem888ai_key()
        if not key:
            return _ConnectivityResult("CEM888.AI API", [], [])
        try:
            import httpx
            ...
            # check doesn't falsely report CEM888.AI as unreachable.
            if (
                is_oauth
            ...
            if r.status_code == 200:
                return _ConnectivityResult(
                    "CEM888.AI API",
                    [(color("✓", Colors.GREEN), "CEM888.AI API", "")],
                    [],
                )
            if r.status_code == 401:
                return _ConnectivityResult(
                    "CEM888.AI API",
                    [(color("✗", Colors.RED), "CEM888.AI API",
                      color("(invalid API key)", Colors.DIM))],
                    [],
                )
            return _ConnectivityResult(
                "CEM888.AI API",
                [(color("⚠", Colors.YELLOW), "CEM888.AI API",
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # CEM888.AI-compat endpoints (/cem888ai, api.kimi.com/coding
            # with no /v1) don't support /models. Rewrite to OpenAI-compat
            # /v1 surface for health checks.
            if base and base.rstrip("/").endswith("/cem888ai"):
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``cem888 doctor``.
    ...
    _probes.append(("CEM888.AI API", _probe_cem888ai))
    ...
    # never the legitimate source for `cem888 doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'cem888 setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = CEM888_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: cem888 skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = CEM888_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: cem888 memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: cem888 memory setup",
                    "No Honcho API key — run 'cem888 memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run cem888 memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: cem888 memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: cem888 memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from cem888_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "cem888 -p" in content:
                            _m = _re.search(r"cem888 -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'cem888 doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
