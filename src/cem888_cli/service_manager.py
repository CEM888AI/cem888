...
the existing module-level functions in cem888_cli.gateway and
cem888_cli.gateway_windows directly. This protocol is a thin facade
...
``cem888 gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``cem888 gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged cem888
    ...
    detection always fails. Since every CEM888 runtime call inside the
    container drops to cem888 via ``s6-setuidgid``, that silent failure
...
# in ``cem888_cli.gateway`` (systemd/launchd) and ``cem888_cli.gateway_windows``
...
# active (set via the ``cem888 -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in cem888_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from cem888_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from cem888_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from cem888_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from cem888_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in cem888_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from cem888_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from cem888_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from cem888_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from cem888_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``cem888_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from cem888_cli import gateway_windows
        from cem888_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `cem888 profile create`
# runs inside the container (Phase 4). Static services (main-cem888, dashboard)
...
# UID/GID of the in-image ``cem888`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the cem888 user.
    ...
    (the ``cem888`` user — UID 10000 — running every CEM888 runtime
    ...
    is dispatched as the cem888 user.
    ...
    with cem888 ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           cem888:cem888, 0755 (parent must already exist)
    ``svc_dir/event/``                     cem888:cem888, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 cem888:cem888, 0755
    ``svc_dir/supervise/event/``           cem888:cem888, 03730
    ``svc_dir/supervise/control``          cem888:cem888, 0660    (FIFO)
    ...
    slot's supervise/ was cem888-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _CEM888_UID, _CEM888_GID)
        except PermissionError:
            # Running as the cem888 user already — directory is cem888-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _CEM888_UID, _CEM888_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is cem888-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _CEM888_UID, _CEM888_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``cem888 -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`cem888 profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-cem888, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources CEM888_HOME (and any extra env) via with-contenv —
             so e.g. ``-e CEM888_HOME=/data/cem888`` is honored at run
          ...
          4. Drops to the cem888 user and exec's
             ``cem888 -p <profile> gateway run`` (or just ``cem888
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``cem888 gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root CEM888_HOME profile" (the implicit profile that exists at
        the top of $CEM888_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``cem888_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$CEM888_HOME/profiles/default/`` — a
        ...
        ``cem888_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export CEM888_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            lines.append("exec s6-setuidgid cem888 cem888 gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid cem888 cem888 -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${CEM888_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the CEM888_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e CEM888_HOME=/data/cem888``
        gets its logs under /data/cem888/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``cem888 logs`` reads and what
        ...
        return (
            ...
            f': "${{CEM888_HOME:=/opt/data}}"\n'
            f'log_dir="$CEM888_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R cem888:cem888 "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid cem888 s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with cem888 ownership
            ...
            # the cem888 user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is cem888-owned (see ``_seed_supervise_skeleton``), and on
