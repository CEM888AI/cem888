"""cem888 claw — OpenClaw migration commands.

Usage:
    cem888 claw migrate              # Preview then migrate (always shows preview first)
    cem888 claw migrate --dry-run    # Preview only, no changes
    cem888 claw migrate --yes        # Skip confirmation prompt
    cem888 claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    cem888 claw migrate --no-backup  # Skip pre-migration snapshot
    cem888 claw cleanup              # Archive leftover OpenClaw directories
    cem888 claw cleanup --dry-run    # Preview what would be archived
"""
...
from cem888_cli.config import get_cem888_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "CEM888 may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a CEM888 gateway is running with connected platforms.
    ...
    print()
    print_error(
        "CEM888 gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'cem888 stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_cem888.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_cem888", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: cem888 claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to CEM888")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'cem888 claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → CEM888 migration."""
    ...
    # migrate-cem888 posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: cem888 claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {cem888_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.cem888/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: cem888 claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the CEM888 home ─────────
    # Delegates to cem888_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `cem888 import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from cem888_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(cem888_home=cem888_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: cem888 import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the CEM888 home."
            )
    ...
    # 'cem888 claw cleanup' separately.
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: cem888 claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        try:
            workspace_dirs = [
                ...
                and any((d / name).exists() for name in ("todo.json", "SOUL.md", "MEMORY.md",
                                     "MEMORY_MANIFEST.md", "USER.md"))
            ]
        except OSError:
            workspace_dirs = []
        ...
        if workspace_dirs:
            ...
            for ws in workspace_dirs[:5]:
                ...
                for _mf in ("MEMORY_MANIFEST.md", "MEMORY.md"):
                    if (ws / _mf).exists():
                        items.append(_mf)
                        break
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: cem888 claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  cem888 claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  cem888 claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  cem888 config set OPENROUTER_API_KEY sk-or-v1-...")
