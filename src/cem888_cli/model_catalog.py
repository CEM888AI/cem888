...
The CEM888 docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
CEM888.AI Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_URL = (
    "https://cem888.ai/docs/api/model-catalog.json"
)
...
def _cache_path() -> Path:
    ...
    return get_cem888_home() / "cache" / "model_catalog.json"
...
def get_curated_cem888_models() -> list[str] | None:
    """Return CEM888.AI Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("cem888")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``cem888 model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
