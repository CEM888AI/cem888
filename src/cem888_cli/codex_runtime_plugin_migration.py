"""Migrate CEM888' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads CEM888' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  CEM888 mcp_servers.<n>.command/args/env  → codex stdio transport
  CEM888 mcp_servers.<n>.url/headers       → codex streamable_http transport
  CEM888 mcp_servers.<n>.timeout           → codex tool_timeout_sec
  CEM888 mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  CEM888-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. CEM888' own
...
MIGRATION_MARKER = (
    "# managed by cem888-agent — `cem888 codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end cem888-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in CEM888 config.")
...
# CEM888 keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # CEM888' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, cem888_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one CEM888 MCP server config to the codex inline-table dict
    ...
    if not isinstance(cem888_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(cem888_cfg.get("command"))
    has_url = bool(cem888_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (CEM888_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by CEM888)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert CEM888' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before CEM888' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior CEM888
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="cem888-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a CEM888_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed cem888-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.cem888`` symlink that happens to live under ``/private/var/folders``
...
def _build_cem888_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches CEM888' own
    ...
    so a cem888 installed under /opt/, /usr/local/, or a venv all work.
    CEM888_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # CEM888_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_cem888_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever CEM888_HOME its launcher sets
    ...
    # — that would override the launcher's CEM888_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("CEM888_HOME", tmp_path) would
    ...
    cem888_home = os.environ.get("CEM888_HOME") or ""
    if cem888_home and _looks_like_test_tempdir(cem888_home):
        cem888_home = ""
    if cem888_home:
        env["CEM888_HOME"] = cem888_home
    # PYTHONPATH passes through so a worktree-launched cem888 finds the
    ...
    env["CEM888_QUIET"] = "1"
    env["CEM888_REDACT_SECRETS"] = env.get("CEM888_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.cem888_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_cem888_tools: bool = True,
) -> MigrationReport:
    """Translate CEM888 mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        cem888_config: full ~/.cem888/config.yaml dict
        ...
        expose_cem888_tools: when True (default), register CEM888' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into CEM888 for tools
    ...
    cem888_servers = (cem888_config or {}).get("mcp_servers") or {}
    if not isinstance(cem888_servers, dict):
        report.errors.append(
            "mcp_servers in CEM888 config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in cem888_servers.items():
    ...
    # Inject CEM888' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into CEM888 for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/cem888_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_cem888_tools:
        translated["cem888-tools"] = _build_cem888_tools_mcp_entry()
        if "cem888-tools" not in report.migrated:
            report.migrated.append("cem888-tools")
