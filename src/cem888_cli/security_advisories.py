"""
Security advisory checker for CEM888 Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``cem888 doctor --ack <id>``; the ack is persisted to
...
1. ``cem888 doctor`` (and ``cem888 doctor --ack <id>``)
...
run in environments where the rest of CEM888 failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including cem888 when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.cem888/.env (OpenRouter, CEM888.AI, OpenAI, "
            "CEM888.AI, GitHub, AWS, Google, Mistral, etc.).",
            ...
            "After cleanup: cem888 doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — CEM888 requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'cem888 doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.cem888/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_cem888_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``cem888 doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `cem888 doctor` on the gateway host for details.")
