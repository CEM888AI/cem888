"""MCP picker — interactive `cem888 mcp picker` (also the default `cem888 mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``cem888 mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `cem888 plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from cem888_cli.cli_output import prompt_yes_no
from cem888_cli.curses_ui import curses_single_select
from cem888_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new CEM888 session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from cem888_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `cem888 mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: cem888 mcp install <name>    Picker: cem888 mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their CEM888 is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer CEM888 — run `cem888 update` "
...
def show_catalog() -> None:
    """`cem888 mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`cem888 mcp picker` (and default `cem888 mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`cem888 mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from cem888_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `cem888 mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
