"""
MCP Server Management CLI — ``cem888 mcp`` subcommand.

Implements ``cem888 mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.cem888/config.yaml under the ``mcp_servers`` key.
"""
...
from cem888_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_cem888_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from cem888_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # cem888_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  cem888 mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  cem888 mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  cem888 mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_cem888_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: cem888 mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_cem888_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `cem888 mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  cem888 mcp add <name> --url <endpoint>')
        _info('  cem888 mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `cem888 mcp remove` + `cem888 mcp add` to reconfigure auth.")
        return
...
# ─── cem888 mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'cem888 mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``cem888 mcp`` subcommands."""
    ...
    if action == "picker":
        from cem888_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from cem888_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from cem888_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `cem888 plugin`.
        from cem888_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("cem888 mcp                                    Open the catalog picker (default)")
        _info("cem888 mcp catalog                            List CEM888.AI-approved MCPs")
        _info("cem888 mcp install <name>                     Install a catalog MCP")
        _info("cem888 mcp serve                              Run as MCP server")
        _info("cem888 mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("cem888 mcp add <name> --command <cmd>         Add a stdio server")
        _info("cem888 mcp add <name> --preset <preset>       Add from a known preset")
        _info("cem888 mcp remove <name>                      Remove a server")
        _info("cem888 mcp list                               List configured servers")
        _info("cem888 mcp test <name>                        Test connection")
        _info("cem888 mcp configure <name>                   Toggle tools")
        _info("cem888 mcp login <name>                       Re-authenticate OAuth")
        print()
