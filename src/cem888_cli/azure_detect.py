...
Inspect a Microsoft Foundry / Azure OpenAI endpoint to determine:
  - API transport (OpenAI-style ``chat_completions`` vs
    CEM888.AI-style ``cem888ai_messages``)
...
# Default Azure CEM888.AI ``api-version``.  Matches the value used by
# ``agent/cem888ai_adapter.py`` when building the CEM888.AI client.
...
class DetectionResult:
    ...
    #: ``"cem888ai_messages"``, or ``None`` when detection failed.
    ...
    #: ``True`` when the URL was determined to be an CEM888.AI-style
    #: endpoint (from path suffix or live probe).
    is_cem888ai: bool = False
...
def _http_get_json(url: str,
    ...
    req.add_header("User-Agent", "cem888-agent/azure-detect")
    try:
...
def _looks_like_cem888ai_path(url: str) -> bool:
    """Return True when the URL's path ends in ``/cem888ai`` or
    contains a ``/cem888ai/`` segment.  Used by Azure Foundry
    resources that route CEM888 traffic through a dedicated path."""
    try:
        parsed = urlparse(url)
        path = (parsed.path or "").lower().rstrip("/")
        return path.endswith("/cem888ai") or "/cem888ai/" in path + "/"
    except Exception:
        return False
...
def _probe_cem888ai_messages(base_url: str,
                              api_key: Any,
                              *,
                              token_provider: Optional[Callable[[], str]] = None,
                              ) -> bool:
    """Send a zero-token request to ``<base>/v1/messages`` and check
    whether the endpoint at least *recognises* the CEM888.AI Messages
    shape (any 4xx that mentions ``messages`` or ``model``, or a 400
    ``invalid_request`` with an CEM888.AI error shape).  Never completes
    a real chat.
    """
    ...
    token, mode = _resolve_credential(api_key, token_provider)
    _apply_auth_headers(req, token, mode)
    req.add_header("cem888ai-version", "2023-06-01")
    req.add_header("content-type", "application/json")
    req.add_header("User-Agent", "cem888-agent/azure-detect")
    try:
        with urllib_request.urlopen(req, timeout=6.0) as resp:
            # Should never 200 — "probe" isn't a real deployment.  But
            # if it does, the endpoint definitely speaks CEM888.AI.
            return resp.status < 500
    except HTTPError as exc:
        # 4xx with an CEM888.AI-shaped error body = CEM888.AI endpoint.
        try:
            body = exc.read().decode("utf-8", errors="replace")
            lowered = body.lower()
            if "cem888ai" in lowered or '"type"' in lowered and '"error"' in lowered:
                return True
            # Pre-Azure-v1 Azure Foundry returns a plain 404 for
            # CEM888.AI-style calls on non-CEM888.AI deployments.  A
            # 400 "model not found" IS CEM888.AI though.
...
def detect(base_url: str,
    ...
    # 1. Path sniff.  Azure Foundry exposes CEM888.AI-style deployments
    #    under a dedicated ``/cem888ai`` path.
    if _looks_like_cem888ai_path(base_url):
        result.is_cem888ai = True
        result.api_mode = "cem888ai_messages"
        result.reason = "URL path ends in /cem888ai → CEM888.AI Messages API"
        return result
    ...
    # 3. Fallback: probe the CEM888.AI Messages shape.  Slower and more
    #    intrusive than /models, so only run it when the OpenAI probe
    #    failed.
    if _probe_cem888ai_messages(base_url, api_key, token_provider=token_provider):
        result.is_cem888ai = True
        result.api_mode = "cem888ai_messages"
        result.reason = "Endpoint accepts CEM888.AI Messages shape"
        return result
