...
on Japanese locales, etc.).  CEM888's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``cem888_cli/main.py`` CLI dispatch,
``gateway/run.py`` startup) call :func:`configure_windows_stdio` exactly
once early in startup.

Patterns cribbed from CEM888 Code (``src/utils/platform.ts``), OpenCode
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. CEM888 is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CEM888_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CEM888_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the CEM888-managed Git install directories so
    ...
    # already-running shells don't see the change — which means cem888
    ...
    # install without CEM888-managed PortableGit).
    ...
    # stdin is re-configured for completeness; CEM888's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and CEM888's
    ``cem888 config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching CEM888 (User env var in Windows Settings, or
    export in a PowerShell profile) and CEM888 picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known CEM888-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\cem888\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``cem888`` from right after install) retain their old PATH.

    Any subprocess CEM888 spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known CEM888-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "cem888", "git", "cmd"),
        os.path.join(local_appdata, "cem888", "git", "bin"),
        os.path.join(local_appdata, "cem888", "git", "usr", "bin"),
        # CEM888 venv Scripts directory — host of the cem888.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes cem888, but harmless to include.
        os.path.join(local_appdata, "cem888", "cem888-agent", "venv", "Scripts"),
