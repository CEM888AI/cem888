"""
Backup and import commands for cem888 CLI.

`cem888 backup` creates a zip archive of the entire ~/.cem888/ directory
(excluding the cem888-agent repo and transient files).

`cem888 import` restores from a backup zip, overlaying onto the current
CEM888_HOME root.
"""
...
from cem888_constants import get_default_cem888_root, get_cem888_home, display_cem888_home
...
_EXCLUDED_DIRS = {
    "cem888-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to cem888 root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CEM888_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the CEM888 home directory."""
    cem888_root = get_default_cem888_root()

    if not cem888_root.is_dir():
        print(f"Error: CEM888 home directory not found at {cem888_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"cem888-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"cem888-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_cem888_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(cem888_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(cem888_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(cem888_root)
    ...
    print(f"\nRestore with: cem888 import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a CEM888 backup.
    ...
    # Look for telltale files that a cem888 home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a CEM888 backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.cem888/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a cem888 dir name
        if prefix in {".cem888", "cem888"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_cem888_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (cem888_root / "config.yaml").exists()
        has_env = (cem888_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has CEM888 configuration.")
        ...
        cem888_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = cem888_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(cem888_root.resolve())
            except ValueError:
        ...
        profiles_dir = cem888_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # cem888_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: cem888 profile list  (after installing cem888)")

        # Guidance
        print()
        if not (cem888_root / "cem888-agent").is_dir():
            print("Note: The cem888-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: cem888 update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  cem888 -p {pname} gateway install")

        print("Done. Your CEM888 configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and cem888 backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to CEM888_HOME).
...
# — `cem888 update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(cem888_home: Optional[Path] = None) -> Path:
    home = cem888_home or get_cem888_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    cem888_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(cem888_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(cem888_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for cem888 backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_cem888_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, cem888_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``cem888_root`` to ``out_path``.
...
def _pre_update_backup_dir(cem888_home: Optional[Path] = None) -> Path:
    home = cem888_home or get_cem888_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CEM888_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CEM888_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``cem888 update``) should continue even if the backup fails.
    """
    cem888_root = cem888_home or get_default_cem888_root()
    if not cem888_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(cem888_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, cem888_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `cem888 claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of CEM888_HOME under ``backups/`` before a
    ``cem888 claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``cem888 import <archive>``.  Writes to
    ``<CEM888_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `cem888 import` and the
