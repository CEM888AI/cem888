"""Normalized CEM888.AI Portal account entitlement helpers."""
...
_account_info_cache: tuple[str, float, "CEM888PortalAccountInfo"] | None = None


@dataclass(frozen=True)
class CEM888PortalSubscriptionInfo:
...
class CEM888PortalAccountInfo:
    ...
    cem888_client: Optional[str] = None
    ...
    subscription: Optional[CEM888PortalSubscriptionInfo] = None
...
def cem888_portal_billing_url(account_info: Optional[CEM888PortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized CEM888.AI account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_CEM888_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def format_portal_entitlement_message(
    account_info: Optional[CEM888PortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
) -> Optional[str]:
    """Return user-facing guidance for a missing CEM888.AI paid entitlement.
    ...
    credits does not.
    """
    billing_url = cem888_portal_billing_url(account_info)
    ...
    if account_info is None:
        return (
            f"CEM888 could not verify your CEM888.AI Portal entitlement, so {capability} "
            f"is unavailable. Run `cem888 model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"CEM888.AI inference credentials are configured, but CEM888 cannot verify "
                f"your CEM888.AI Portal paid access for {capability}. Log in with "
                f"`cem888 model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to CEM888.AI Portal to use {capability}: run `cem888 model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"CEM888 could not verify your CEM888.AI Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `cem888 model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"CEM888 could not find a CEM888.AI Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `cem888 model` to "
            f"authenticate again; if the problem persists, contact CEM888.AI support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `cem888 model` to refresh CEM888."
        return message

    return (
        f"Your CEM888.AI Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: CEM888PortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your CEM888.AI Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current CEM888.AI Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your CEM888.AI Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your CEM888.AI Portal account has no usable paid credits{credit_detail}, so "
...
def reset_portal_account_info_cache() -> None:
    ...
    *,
...
) -> CEM888PortalAccountInfo:
    """Return normalized CEM888.AI Portal account entitlement information.
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        ...
        if pool_info is not None:
            return pool_info
        return CEM888PortalAccountInfo(
            logged_in=False,
            source="none",
            fresh=False,
            portal_base_url=portal_base_url,
...
def _fresh_account_info(
    *,
    ...
    try:
        from cem888_cli.auth import get_provider_auth_state, resolve_cem888_access_token

        access_token = resolve_cem888_access_token()
        ...
        payload = _fetch_cem888_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[CEM888PortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque CEM888.AI keys."""
    try:
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_cem888_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _fetch_cem888_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from cem888_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    return CEM888PortalAccountInfo(
        ...
        cem888_client=_coerce_str(claims.get("cem888_client")),
        portal_base_url=portal_base_url,
...
def _subscription_from_payload(value: Any) -> Optional[CEM888PortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return CEM888PortalSubscriptionInfo(
