...
Toggles `model.openai_runtime` between "auto" (= chat_completions, CEM888'
...
The actual runtime resolution happens in cem888_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "cem888"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate CEM888' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into CEM888 for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from cem888_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the cem888-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "cem888-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + CEM888 tool callback are always-on production
            ...
            if "cem888-tools" in mig_report.migrated:
                msg_lines.append(
                    "CEM888 tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default CEM888 runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "CEM888 tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default CEM888 runtime.")
