"""
Interactive setup wizard for CEM888 Agent.
...
from cem888_cli.cem888_subscription import get_cem888_subscription_features
from tools.tool_backend_helpers import managed_cem888_tools_enabled
...
_DOCS_BASE = "https://cem888.ai/docs"
...
_DEFAULT_PROVIDER_MODELS = {
    ...
    "kilocode": ["cem888ai/cem888-opus-4.6", "cem888ai/cem888-sonnet-4.6", "openai/gpt-5.4", "google/gemini-3-pro-preview", "google/gemini-3-flash-preview"],
    "opencode-zen": ["gpt-5.4", "gpt-5.3-codex", "cem888-sonnet-4-6", "gemini-3-flash", "glm-5", "kimi-k2.5", "minimax-m2.7"],
...
# display_cem888_home imported lazily at call sites (stale-module safety during cem888 update)
...
from cem888_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from cem888_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ CEM888 Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure CEM888 using environment variables or config commands:")
    print_info("  cem888 config set model.provider custom")
    print_info("  cem888 config set model.base_url http://localhost:8080/v1")
    print_info("  cem888 config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'cem888 setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from cem888_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from cem888_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'cem888 setup')")


def _print_setup_summary(config: dict, cem888_home):
    ...
    tool_status = []
    subscription_features = get_cem888_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'cem888 setup' to configure"))
    ...
    if subscription_features.web.managed_by_cem888:
        tool_status.append(("Web Search & Extract (CEM888.AI subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_cem888:
        tool_status.append(("Browser Automation (CEM888.AI Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via CEM888.AI), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_cem888:
        tool_status.append(("Image Generation (CEM888.AI subscription)", True, None))
        ...
        try:
            ...
            from cem888_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `cem888 tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from cem888_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_cem888:
        tool_status.append(("Text-to-Speech (OpenAI via CEM888.AI subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'cem888 setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'cem888 setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_cem888:
        tool_status.append(("Modal Execution (CEM888.AI subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'cem888 setup terminal'"))
    elif managed_cem888_tools_enabled() and subscription_features.cem888_auth_present:
        tool_status.append(("Modal Execution (optional via CEM888.AI subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via cem888 auth spotify — check auth.json, not env vars)
    try:
        from cem888_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'cem888 setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {cem888_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('cem888 setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('cem888 setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('cem888 setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('cem888 setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('cem888 setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('cem888 config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('cem888 config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('cem888 config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('cem888', Colors.GREEN)}              Start chatting")
    print(f"   {color('cem888 gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('cem888 doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `cem888 tools` and `cem888 setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``cem888 model``)
    ...
    provider added to ``cem888 model`` is automatically available here.
    ...
    # Delegate to the shared cem888 model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from cem888_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: cem888 model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from cem888_cli.auth_commands import auth_add_command
            ...
            print_info(
                "CEM888 can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        _prov_names = {
            "cem888-api": "CEM888.AI Portal API key",
        ...
        print_info("doesn't currently provide one CEM888 can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'cem888 setup' or configure AUXILIARY_VISION_* settings")


    # Tool Gateway prompt is already shown by _model_flow_cem888() above.
    save_config(config)

    if not quick and selected_provider != "cem888":
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``cem888 model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from cem888_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_cem888_tools_enabled() and subscription_features.cem888_auth_present:
        choices.append("CEM888.AI Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("cem888-openai")
    choices.extend(
        [
    ...
    selected_via_cem888 = selected == "cem888-openai"
    if selected == "cem888-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed CEM888.AI gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.cem888/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_cem888:
        ...
        # via the CEM888 auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via cem888 setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'cem888 setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where CEM888 runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        managed_modal_available = bool(
            managed_cem888_tools_enabled()
            and
            get_cem888_subscription_features(config).cem888_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed CEM888.AI gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CEM888_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CEM888_MAX_ITERATIONS")
    ...
    print_info("  Run `cem888 setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CEM888_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where CEM888 delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `cem888 update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://cem888.ai/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under CEM888_HOME, and print
    ...
    """
    try:
        from cem888_cli.slack_cli import _build_full_manifest
        from cem888_constants import get_cem888_home

        manifest = _build_full_manifest(
            bot_name="CEM888",
            bot_description="Your CEM888 agent on Slack",
        )
        ...
        print_info(
            "   Re-run `cem888 slack manifest --write` anytime to refresh after "
            "CEM888 adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where CEM888 delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects CEM888 to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from cem888_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://cem888.ai/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://cem888.ai/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  cem888 config edit")
    print_info("   Open config in your editor:  cem888 config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from cem888_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with CEM888 from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'cem888 setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     cem888 config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_cem888_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  cem888 gateway")
        else:
            from cem888_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   cem888 gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   cem888 gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `cem888 setup tools` and `cem888 tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    def _has_key(pconfig) -> bool:
        for env_var in pconfig.api_key_env_vars:
            # CEM888_OAUTH_TOKEN is set by CEM888 Code itself, not by
            # the user — mirrors is_provider_explicitly_configured in auth.py.
            if env_var == "CEM888_OAUTH_TOKEN":
                continue
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from cem888_cli.config
    ...
    if section_key == "model":
        ...
        from cem888_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_cem888.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_cem888 migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_cem888", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and CEM888.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure CEM888 to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point CEM888 at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point CEM888 at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point CEM888 at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point CEM888 at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to CEM888 equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing CEM888 config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in CEM888.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ CEM888's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(cem888_home: Path) -> bool:
    ...
    print_info("CEM888 can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: cem888 claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=cem888_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: cem888 claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing CEM888 configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing CEM888 config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in CEM888 (use cem888 claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot CEM888.AI Portal setup — OAuth + provider switch + Tool Gateway.

    Wired into ``cem888 setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``cem888 setup --portal``)
    that gets a brand-new user from zero to a fully working CEM888 session
    ...
    from cem888_cli.auth import get_cem888_auth_status
    ...
    print(color("│     ⚕ CEM888 Setup — CEM888.AI Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your CEM888.AI Portal sub.")
    print()
    print_info("  Sign up: https://cem888.ai/manage-subscription")
    print()

    # Skip OAuth if already logged in (don't re-prompt every time the user
    # runs `cem888 setup --portal` after a successful first run).
    already_logged_in = False
    try:
        already_logged_in = bool((get_cem888_auth_status() or {}).get("logged_in"))
    except Exception:
        already_logged_in = False

    if already_logged_in:
        print_success("  Already logged into CEM888.AI Portal.")
    else:
        # Hand off to the shared auth wiring so the device-code flow is
        # identical to `cem888 auth add cem888 --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print()
            print_error(f"  CEM888.AI Portal login failed (exit {e.code}).")
            print_info("  You can retry later with `cem888 auth add cem888 --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
            ...
            print()
            print_error(f"  CEM888.AI Portal login failed: {exc}")
            print_info("  You can retry later with `cem888 auth add cem888 --type oauth`.")
            return

    # Set provider → cem888 so the model picker, status surfaces, and
    # managed-tool gating all light up. Leave model.model empty so the
    # runtime picks CEM888.AI's default model; the user can change it later
    # with `cem888 model`.
    ...
    save_config(config)
    print()
    print_success("  CEM888.AI set as your inference provider.")

    # Offer the Tool Gateway opt-in (single Y/n) — same flow that fires
    # from `cem888 model` after picking CEM888.AI.
    print()
    try:
    ...
    print_info("  Run `cem888 portal status` to inspect routing.")
    print_info("  Run `cem888` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      cem888 setup           — full or quick (auto-detected)
      cem888 setup model     — just model/provider
      cem888 setup tts       — just text-to-speech
      cem888 setup terminal  — just terminal backend
      cem888 setup gateway   — just messaging platforms
      cem888 setup tools     — just tool configuration
      cem888 setup agent     — just agent settings
    """
    from cem888_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot CEM888.AI Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ CEM888 Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ CEM888 Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your CEM888 Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, cem888_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have CEM888 configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'cem888 setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(cem888_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up CEM888?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, cem888_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {cem888_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'cem888 config edit'")
    ...
    _print_setup_summary(config, cem888_home)


def _run_first_time_quick_setup(config: dict, cem888_home, is_existing: bool):
    ...
    the user can customize later via ``cem888 setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'cem888 setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    cem888 setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  cem888 setup gateway")
    print()

    _print_setup_summary(config, cem888_home)


def _run_quick_setup(config: dict, cem888_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'cem888 setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect CEM888 to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'cem888 setup gateway'.")
