"""
Profile management for multiple isolated CEM888 instances.

Each profile is a fully independent CEM888_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.cem888/profiles/<name>/`` by default.

The "default" profile is ``~/.cem888`` itself — backward compatible,
zero migration needed.

Usage::

    cem888 profile create coder          # fresh profile + bundled skills
    cem888 profile create coder --clone  # also copy config, .env, SOUL.md, skills
    cem888 profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    cem888 -p coder chat                 # or via flag
    cem888 profile use coder             # set as sticky default
    cem888 profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See cem888_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
_CLONE_SUBDIR_FILES = [
    "memories/MEMORY_MANIFEST.md",
    "memories/USER_MANIFEST.md",
]
...
# default profile (``~/.cem888``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   cem888-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `cem888 profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `cem888
...
# `cem888 skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         CEM888 infrastructure directories that only the default profile
         (``~/.cem888``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_cem888_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.cem888) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "cem888-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "cem888", "default", "test", "tmp", "root", "sudo",
})

# CEM888 subcommands that cannot be used as profile names/aliases
_CEM888_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the cem888 root, NOT to the current CEM888_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CEM888_HOME points outside
    ``~/.cem888``, profiles live under ``CEM888_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_cem888_home() / "profiles"


def _get_default_cem888_home() -> Path:
    """Return the default (pre-profile) CEM888_HOME path.

    In standard deployments this is ``~/.cem888``.
    In Docker/custom deployments where CEM888_HOME is outside ``~/.cem888``
    (e.g. ``/opt/data``), returns CEM888_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_cem888_home() / "active_profile"
...
def slugify_profile_name(display_name: str) -> str:
    """Derive the canonical on-disk profile key from a customer display name.

    Spec version 1 — see install/scripts/agent_identity_vectors.json, the single
    contract that this function and the installer-side implementation
    (install/scripts/agent_identity.py) must both satisfy.

    The display name is a HUMAN LABEL and must never be used to build a machine
    path (profile dir, plugin dir, state dir, index dir, service config, or a
    ``--profile`` argument). Only the key returned here may reach a path.

    Raises ``ValueError`` with a typed prefix rather than falling back to a
    generic identity: a missing or unusable name is an error, not an invitation
    to manufacture an agent.
    """
    import re as _re
    import unicodedata as _ud

    if not isinstance(display_name, str) or display_name.strip() == "":
        raise ValueError("MISSING_AGENT_IDENTITY: no display name supplied")
    decomposed = _ud.normalize("NFKD", display_name)
    folded = "".join(ch for ch in decomposed if not _ud.combining(ch))
    slug = _re.sub(r"[^a-z0-9]+", "-", folded.casefold()).strip("-")
    slug = slug[:64].strip("-")
    if not slug:
        raise ValueError(
            f"EMPTY_PROFILE_KEY: display name {display_name!r} contains no usable characters"
        )
    if slug in _RESERVED_NAMES or slug == "default":
        raise ValueError(
            f"RESERVED_PROFILE_KEY: display name {display_name!r} normalizes to the "
            f"reserved key {slug!r} — pick a different name"
        )
    return slug


def resolve_agent_identity(display_name: str) -> dict:
    """One display name in; canonical identity out. The cross-component contract."""
    if not isinstance(display_name, str) or display_name.strip() == "":
        raise ValueError("MISSING_AGENT_IDENTITY: no display name supplied")
    return {
        "spec_version": "1",
        "display_name": display_name,
        "profile_key": slugify_profile_name(display_name),
    }
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the CEM888 installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CEM888_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_cem888_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, cem888 subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _CEM888_SUBCOMMANDS:
        return f"'{canon}' conflicts with a cem888 subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``cem888 profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing CEM888 config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_cem888_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``cem888 update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.cem888)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.cem888/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `cem888 update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`cem888 profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `cem888 update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `cem888 profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `cem888 -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CEM888_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``cem888 profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CEM888_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.cem888).\n"
            "To remove everything, use: cem888 uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from cem888_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CEM888_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CEM888_HOME")
    try:
        os.environ["CEM888_HOME"] = str(profile_dir)
        from cem888_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["CEM888_HOME"] = old_home
        elif "CEM888_HOME" in os.environ:
            del os.environ["CEM888_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.cem888/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: cem888 profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CEM888_HOME.

    Returns ``"default"`` if CEM888_HOME is not set or points to ``~/.cem888``.
    Returns the profile name if CEM888_HOME points into ``~/.cem888/profiles/<name>``.
    Returns ``"custom"`` if CEM888_HOME is set to an unrecognized path.
    """
    from cem888_constants import get_cem888_home
    cem888_home = get_cem888_home()
    resolved = cem888_home.resolve()

    default_resolved = _get_default_cem888_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.cem888 itself — its parent is ~/ and its
        # directory name is ".cem888", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: cem888 profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.cem888 itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.cem888). "
            "Specify a different name: cem888 profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="cem888_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"cem888.{old_name}"
    new_host = f"cem888.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_cem888_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CEM888_HOME path string.

    Called early in the CLI entry point, before any cem888 modules
    are imported, to set the CEM888_HOME environment variable.
    """
