"""
Configuration management for CEM888 Agent.

Config files are stored in ~/.cem888/ for easy access:
- ~/.cem888/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.cem888/.env         - API keys and secrets

This module provides:
- cem888 config          - Show current configuration
- cem888 config edit     - Open config in editor
- cem888 config set      - Set a specific value
- cem888 config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.cem888/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``cem888 logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  cem888 config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` CEM888 makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. CEM888 itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``cem888 update``, the TUI build.
...
#   on every plugin install / ``cem888 update``.
...
# * ``CEM888_HOME`` / ``CEM888_PROFILE`` / ``CEM888_CONFIG`` /
#   ``CEM888_ENV`` — CEM888 runtime location flags. Writing these into
...
#
# IMPORTANT: ``CEM888_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (CEM888_GEMINI_CLIENT_ID,
# CEM888_LANGFUSE_PUBLIC_KEY, CEM888_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # CEM888 runtime location — never via dashboard env writer.
    # NOT a CEM888_* blanket: integration credentials (CEM888_GEMINI_*,
    # CEM888_LANGFUSE_*, CEM888_SPOTIFY_*, ...) ARE allowed.
    "CEM888_HOME", "CEM888_PROFILE", "CEM888_CONFIG", "CEM888_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or CEM888 runtime location "
            "(CEM888_HOME, CEM888_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `cem888 plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "CEM888_LANGFUSE_ENV",
    "CEM888_LANGFUSE_RELEASE",
    "CEM888_LANGFUSE_SAMPLE_RATE",
    "CEM888_LANGFUSE_MAX_CHARS",
    "CEM888_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from cem888_cli.colors import Colors, color
from cem888_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CEM888_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_cem888_home() / ".managed"
...
def is_managed() -> bool:
    """Check if CEM888 is running in package-manager-managed mode.

    Two signals: the CEM888_MANAGED env var (set by the systemd service),
    or a .managed marker file in CEM888_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade cem888-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how CEM888 was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.cem888/.install_method`` file (written by installers)
    2. CEM888_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_cem888_home() / ".install_method"
    try:
    ...
    from cem888_constants import is_container
    if is_container():
        return "docker"
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.cem888/.install_method."""
    stamp = get_cem888_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade cem888-agent"
    if method == "docker":
        return "docker pull ghcr.io/cem888ai/cem888-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade cem888-agent"
        return "pip install --upgrade cem888-agent"
    return "cem888 update"
...
# Long-form text for ``cem888 update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# cem888_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side CEM888, it doesn't update
...
✗ ``cem888 update`` doesn't apply inside the Docker container.

CEM888 Agent runs as a published image (ghcr.io/cem888ai/cem888-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull ghcr.io/cem888ai/cem888-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate cem888-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm ghcr.io/cem888ai/cem888-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/cem888ai/cem888-agent/tags
  • Your config and session history live under ``$CEM888_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``cem888 update`` inside Docker.
...
def format_managed_message(action: str = "modify this CEM888 installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CEM888_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this CEM888 installation is managed by NixOS "
            f"(CEM888_MANAGED={env_hint}).\n"
            "Edit services.cem888-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this CEM888 installation is managed by Homebrew "
            f"(CEM888_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade cem888-agent"
        )

    return (
        f"Cannot {action}: this CEM888 installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall CEM888."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CEM888_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, cem888_bin
    or None if container mode is not active, we're already inside the
    container, or CEM888_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CEM888_DEV") == "1":
        return None
    ...
    container_mode_file = get_cem888_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "cem888-agent")
    exec_user = info.get("exec_user", "cem888")
    cem888_bin = info.get("cem888_bin", "/data/current-package/bin/cem888")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "cem888_bin": cem888_bin,
    }
...
# Re-export from cem888_constants — canonical definition lives there.
from cem888_constants import get_cem888_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_cem888_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_cem888_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the cem888 group can
    share state with the gateway service.

    The mode can be overridden via the CEM888_HOME_MODE environment variable
    (e.g. CEM888_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CEM888_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CEM888_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When CEM888 runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CEM888_CONTAINER") or os.environ.get("CEM888_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CEM888_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CEM888_HOME if the user doesn't have one yet."""
...
def ensure_cem888_home():
    """Ensure ~/.cem888 directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_cem888_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_cem888_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CEM888_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the CEM888-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, CEM888 auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), CEM888 sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when CEM888
        ...
        #  "/home/user/.cem888/cache/documents:/output"]
        ...
        # bundled CEM888 image, which drops to the `cem888` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, CEM888 sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and CEM888 should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``cem888 chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.cem888/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: cem888 sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, CEM888 truncates the
    ...
    # CEM888.AI prompt caching (CEM888 via OpenRouter or native CEM888.AI API).
    # cache_ttl must be "5m" or "1h" (CEM888.AI-supported tiers); other values are ignored.
    "prompt_caching": {
        "cache_ttl": "5m",
    },
    ...
    #   This is separate from CEM888.AI prompt caching and works alongside it.
    ...
    "bedrock": {
        ...
        "discovery": {
            ...
            "provider_filter": [],     # Only show models from these providers (e.g. ["cem888ai", "amazon"])
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | cem888 | codex | custom
        ...
        # Invoked by ``cem888 kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``cem888 kanban decompose`` and the kanban
        ...
        # ``cem888 profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `cem888 model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `cem888 --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `cem888 -c` muscle memory.  Default off so existing
        # users aren't surprised.  CEM888_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/cem888`. Per-platform overrides go under
    ...
    "dashboard": {
        "theme": "default",  # Dashboard visual theme: "default", "midnight", "forge", "mono", "cyberpunk", "rose"
        ...
        # ``--insecure`` is not). The bundled CEM888.AI Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``CEM888_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``CEM888_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://cem888.ai).
        "oauth": {
            "client_id": "",  # agent:{instance_id} — Portal provisions this
            "portal_url": "",  # blank → use plugin default (production Portal)
        },
        # Public URL override (env: ``CEM888_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/cem888``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.cem888/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.cem888/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, CEM888.AI, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        "api_mode": "",    # wire protocol for delegation.base_url: "chat_completions",
                           # "codex_responses", or "cem888ai_messages". Empty = auto-detect
                           # from URL (e.g. /cem888ai suffix → cem888ai_messages). Set this
    ...
    # response. If not, CEM888 feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before CEM888 auto-pauses the goal and
    ...
    # always goes to ~/.cem888/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CEM888_SKILL_DIR} and ${CEM888_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `cem888 curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.cem888/skills/ into
        # ~/.cem888/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `cem888 curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for cem888-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *CEM888 Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CEM888_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.cem888/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CEM888_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `cem888 doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``cem888_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow CEM888 to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via CEM888_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `cem888 -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `cem888` launches with
        ...
        # decomposition is manual via `cem888 kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     cem888-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.cem888/logs/.
    ...
    # curated model lists for OpenRouter and CEM888.AI Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a cem888-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://cem888.ai/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `cem888 model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.cem888/.env,
        ...
        # behavior: files must live under the CEM888 cache, under
        ...
        # user. Bridged to CEM888_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the CEM888
        # cache (~/.cem888/cache/{documents,images,audio,video,screenshots})
        ...
        # to CEM888_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # CEM888_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CEM888_MEDIA_TRUST_RECENT_SECONDS.
        # Only consulted when ``strict`` is true.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.cem888/state.db.
    ...
    "sessions": {
        ...
        # default of ``cem888 sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.cem888/sessions/session_{sid}.json`` on every turn
    ...
    # ``cem888 update`` behaviour.
    "updates": {
        # Run a full ``cem888 backup``-style zip of CEM888_HOME before every
        # ``cem888 update``.  Backups land in ``<CEM888_HOME>/backups/`` and
        # can be restored with ``cem888 import <path>``.  Off by default —
        # on large CEM888_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<CEM888_HOME>/lsp/bin/`` on first use.
    ...
    # `cem888 tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.cem888/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.cem888/.env (or your shell) and never in config.yaml.
            ...
            # ~/.cem888/bin/ on first use.  When False you must install
            ...
            # `cem888 secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
# selection step (CEM888.AI Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "CEM888_BASE_URL": {
        "description": "CEM888.AI Portal base URL override",
        "prompt": "CEM888.AI Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'cem888 model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for CEM888.AI Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for CEM888.AI Subscribers only, used to derive vendor hosts, e.g. cem888.ai -> firecrawl-gateway.cem888.ai",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for CEM888.AI Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit CEM888.AI Subscriber access token for tool-gateway requests (optional; otherwise read from the CEM888 auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @cem888:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CEM888_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #cem888)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: cem888-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'cem888-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote CEM888 API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote CEM888 API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote CEM888 API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CEM888_TOOL_PROGRESS and CEM888_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "CEM888_TOOL_PROGRESS": {
    ...
    "CEM888_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.cem888.config`` entries, then checks
    ...
    try:
        ...
        # should never break `cem888 update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older CEM888 versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``cem888_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'cem888 setup' to create a valid config")]
    ...
    if fb is not None:
        if isinstance(fb, list):
            ...
            issues.append(ConfigIssue(
                ...
                "  fallback_model:\n"
                "    provider: openrouter\n"
                "    model: cem888ai/cem888-sonnet-4",
            ))
        elif fb:
            ...
            if not fb.get("model"):
                issues.append(ConfigIssue(
                    ...
                    "Add: model: cem888ai/cem888-sonnet-4 (or another model)",
                ))
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — CEM888 won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'cem888 doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CEM888_HOME", "~/.cem888")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CEM888_TOOL_PROGRESS")
            old_mode = get_env_value("CEM888_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CEM888_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CEM888_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CEM888_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CEM888_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    # The new CEM888.AI auth flow no longer uses this env var.
    if current_ver < 9:
        try:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CEM888_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_cem888_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `cem888 plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `cem888 curator status` has no
    ...
    #   3. Creates `~/.cem888/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_cem888_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_cem888_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: cem888 config set <key> <value>")
    ...
    # metadata.cem888.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _cem888_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.cem888/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.cem888/config.yaml.
    ...
    (which change ``CEM888_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — cem888 auth) — OpenAI Codex
#   cem888         (OAuth — cem888 auth) — CEM888.AI Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.cem888/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.cem888/.env.
    ...
    interactive menu render (`cem888 tools`, `cem888 setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `cem888 tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real CEM888 env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.cem888/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.cem888/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.cem888/.env and os.environ.
...
def save_cem888ai_oauth_token(value: str, save_fn=None):
    """Persist an CEM888.AI OAuth/setup token and clear the API-key slot."""
    writer = save_fn or save_env_value
    writer("ANTHROPIC_TOKEN", value)
    writer("ANTHROPIC_API_KEY", "")


def use_cem888ai_cem888_code_credentials(save_fn=None):
    """Use CEM888 Code's own credential files instead of persisting env tokens."""
    writer = save_fn or save_env_value
    writer("ANTHROPIC_TOKEN", "")
    writer("ANTHROPIC_API_KEY", "")


def save_cem888ai_api_key(value: str, save_fn=None):
    """Persist an CEM888.AI API key and clear the OAuth/setup-token slot."""
    writer = save_fn or save_env_value
...
def reload_env() -> int:
    """Re-read ~/.cem888/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to CEM888 — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known CEM888 vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.cem888/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ CEM888 Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    cem888ai_value = get_cem888ai_key()
    print(f"  {'CEM888.AI':<14} {redact_key(cem888ai_value)}")
    ...
    print(color("  cem888 config edit     # Edit config file", Colors.DIM))
    print(color("  cem888 config set <key> <value>", Colors.DIM))
    print(color("  cem888 setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: cem888 config set <key> <value>")
            print()
            print("Examples:")
            print("  cem888 config set model cem888ai/cem888-sonnet-4")
            print("  cem888 config set terminal.backend docker")
            print("  cem888 config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'cem888 config migrate' to add them")
        ...
        print("  cem888 config           Show current configuration")
        print("  cem888 config edit      Open config in editor")
        print("  cem888 config set <key> <value>   Set a config value")
        print("  cem888 config check     Check for missing/outdated config")
        print("  cem888 config migrate   Update config with new options")
        print("  cem888 config path      Show config file path")
        print("  cem888 config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``cem888 config`` UI so users
