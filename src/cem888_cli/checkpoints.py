"""`cem888 checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.cem888/checkpoints/``.  Actions:

    cem888 checkpoints               # same as `status`
    cem888 checkpoints status        # total size, project count, breakdown
    cem888 checkpoints list          # per-project checkpoint counts + workdir
    cem888 checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    cem888 checkpoints clear [-f]    # nuke the entire base (asks first)
    cem888 checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    cem888 checkpoints
    cem888 checkpoints prune --retention-days 3 --max-size-mb 200
    cem888 checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: cem888 checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``cem888 checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `cem888 checkpoints` → status
