#!/usr/bin/env python3
"""
CEM888 CLI - Main entry point.

Usage:
    cem888                     # Interactive chat (default)
    cem888 chat                # Interactive chat
    cem888 gateway             # Run gateway in foreground
    cem888 gateway start       # Start gateway as service
    cem888 gateway stop        # Stop gateway service
    cem888 gateway status      # Show gateway status
    cem888 gateway install     # Install gateway service
    cem888 gateway uninstall   # Uninstall gateway service
    cem888 setup               # Interactive setup wizard
    cem888 logout              # Clear stored authentication
    cem888 status              # Show status of all components
    cem888 cron                # Manage cron jobs
    cem888 cron list           # List cron jobs
    cem888 cron status         # Check if cron scheduler is running
    cem888 doctor              # Check configuration and dependencies
    cem888 honcho setup                    # Configure Honcho AI memory integration
    cem888 honcho status                   # Show Honcho config and connection status
    cem888 honcho sessions                 # List directory → session name mappings
    cem888 honcho map <name>               # Map current directory to a session name
    cem888 honcho peer                     # Show peer names and dialectic settings
    cem888 honcho peer --user NAME         # Set user peer name
    cem888 honcho peer --ai NAME           # Set AI peer name
    cem888 honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    cem888 honcho mode                     # Show current memory mode
    cem888 honcho mode [hybrid|honcho|local]  # Set memory mode
    cem888 honcho tokens                   # Show token budget settings
    cem888 honcho tokens --context N       # Set session.context() token cap
    cem888 honcho tokens --dialectic N     # Set dialectic result char cap
    cem888 honcho identity                 # Show AI peer identity representation
    cem888 honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    cem888 honcho migrate                  # Step-by-step migration guide: OpenClaw native → CEM888 + Honcho
    cem888 version             Show version
    cem888 update              Update to latest version
    cem888 uninstall           Uninstall CEM888 Agent
    cem888 acp                 Run as an ACP server for editor integration
    cem888 sessions browse     Interactive session picker with search

    cem888 claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: cem888_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``cem888_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``cem888 update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``cem888_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, cem888 crashes on import and the user can't run
# ``cem888 update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import cem888_bootstrap  # noqa: F401
...
# `entry.tsx`; this is just the earlier cousin. ``CEM888_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("CEM888_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not (os.environ.get("CEM888_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`cem888 --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from cem888_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"CEM888 Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``cem888 --version`` before config/logging imports on Termux."""
    if os.environ.get("CEM888_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to CEM888_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (cem888 tools, cem888 setup, cem888 model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'cem888 {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any cem888 module import.
#
# Many modules cache CEM888_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CEM888_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.cem888/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CEM888_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors cem888_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If CEM888_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.cem888/profiles/coder or
    # /opt/data/profiles/coder).  If CEM888_HOME points to the cem888 root
    # instead (e.g. systemd hardcodes CEM888_HOME=/root/.cem888), we must
    # still read active_profile — the user may have switched profiles via
    # `cem888 profile use` and the gateway should honour that choice.
    # See issue #22502.
    cem888_home_env = os.environ.get("CEM888_HOME", "")
    if profile_name is None and cem888_home_env:
        if Path(cem888_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the cem888 root
    if profile_name is None:
        try:
            from cem888_constants import get_default_cem888_root

            active_path = get_default_cem888_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set CEM888_HOME
    if profile_name is not None:
        try:
            from cem888_cli.profiles import resolve_profile_env

            cem888_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent cem888 from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["CEM888_HOME"] = cem888_home
...
# Bind and validate Tier-0 agent authority before config, plugins, memory, or
# any agent/runtime module can consume profile state. Profiles without a
# manifest remain backward compatible unless CEM888_REQUIRE_AUTHORITY=1.
try:
    from agent.authority import AuthorityError as _AuthorityError
    from agent.authority import load_agent_authority as _load_agent_authority

    _load_agent_authority()
except _AuthorityError as exc:
    print(f"Error: Tier-0 runtime authority validation failed: {exc}", file=sys.stderr)
    raise SystemExit(1) from exc

# Load .env from ~/.cem888/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from cem888_cli.config import get_cem888_home
from cem888_cli.env_loader import load_cem888_dotenv

load_cem888_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → CEM888_REDACT_SECRETS env
# var BEFORE cem888_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_cem888_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "CEM888_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["CEM888_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `cem888` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from cem888_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from cem888_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if cem888_constants not importable yet
...
from cem888_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_cem888_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("CEM888_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("CEM888_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from cem888_cli.config import get_env_path, get_cem888_home, load_config
    from cem888_cli.auth import get_auth_status

    # Determine whether CEM888 itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (CEM888 Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from cem888_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_cem888_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from cem888_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for CEM888.AI Portal OAuth credentials
    auth_file = get_cem888_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Check for CEM888 Code OAuth credentials (~/.cem888/.credentials.json)
    # Only count these if CEM888 has been explicitly configured — CEM888 Code
    # being installed doesn't mean the user wants CEM888 to use their tokens.
    if _has_cem888_config:
        try:
            from agent.cem888ai_adapter import (
                read_cem888_code_credentials,
                is_cem888_code_token_valid,
            )

            creds = read_cem888_code_credentials()
            if creds and (
                is_cem888_code_token_valid(creds) or creds.get("refreshToken")
            ):
                return True
        except Exception:
            pass
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from cem888_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, cem888_bin
        cli_args: the original CLI arguments (everything after 'cem888')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    cem888_bin = container_info["cem888_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo cem888 {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo cem888 {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  cem888 --tui --resume {target}")
    if title:
        print(f'  cem888 --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @cem888/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@cem888" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/cem888-ink/package.json",
    "packages/cem888-ink/package-lock.json",
    "packages/cem888-ink/index.js",
    "packages/cem888-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``CEM888_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("CEM888_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``CEM888_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("CEM888_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    cem888_home = os.environ.get("CEM888_HOME") or str(Path.home() / ".cem888")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "CEM888_HOME": cem888_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(cem888_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(cem888_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if cem888_cli_dir is None:
        cem888_cli_dir = Path(__file__).parent
    bundled = cem888_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (CEM888_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("CEM888_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from cem888_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("CEM888_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with CEM888_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset CEM888_TUI_DIR (e.g. `unset CEM888_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("CEM888_QUIET"):
    ...
    if tui_dev:
        # Keep the local @cem888/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @cem888/ink resolves through
        # packages/cem888-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "cem888-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from cem888_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="cem888-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["CEM888_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["CEM888_PYTHON_SRC_ROOT"] = os.environ.get(
        "CEM888_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("CEM888_PYTHON", sys.executable)
    env.setdefault("CEM888_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["CEM888_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["CEM888_MODEL"] = model
        env["CEM888_INFERENCE_MODEL"] = model
    if provider:
        env["CEM888_TUI_PROVIDER"] = provider
        env["CEM888_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["CEM888_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["CEM888_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["CEM888_TUI_SKILLS"] = value
    if query:
        env["CEM888_TUI_QUERY"] = query
    if image:
        env["CEM888_TUI_IMAGE"] = image
    if checkpoints:
        env["CEM888_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["CEM888_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["CEM888_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["CEM888_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["CEM888_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["CEM888_ACCEPT_HOOKS"] = "1"
    ...
    # CEM888_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `cem888 --tui` try to
    ...
    # still set CEM888_TUI_RESUME themselves.
    env.pop("CEM888_TUI_RESUME", None)
    if resume_session_id:
        env["CEM888_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `cem888 update` so
    ...
    if code == 42:
        from cem888_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``CEM888_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``cem888 kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``cem888 kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("CEM888_KANBAN_BOARD"):
        return
    try:
        from cem888_cli.kanban_db import get_current_board

        os.environ["CEM888_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("CEM888_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'cem888 sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from cem888_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from cem888_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'cem888 doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like CEM888 isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  cem888 setup")
        print()

        from cem888_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'cem888 setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from cem888_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["CEM888_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.cem888/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["CEM888_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["CEM888_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["CEM888_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from cem888_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from cem888_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from cem888_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with CEM888?")
        print()
    ...
    # has no creds.json.  Every subsequent `cem888 gateway` then paid a 30s
    ...
    session_dir = get_cem888_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: cem888 gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `cem888 gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  cem888 gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ CEM888 Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  cem888 gateway")
        ...
        print()
        print("  Or install as a service: cem888 gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'cem888 whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from cem888_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from cem888_cli.config import stamp_install_method
    from cem888_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ CEM888 post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from cem888_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``cem888 model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from cem888_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from cem888_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from cem888_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("CEM888_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from cem888_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'cem888 model' for "
                "available providers, or run 'cem888 doctor' to diagnose config "
                "issues."
            )
    ...
    from cem888_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    elif selected_provider == "cem888":
        _model_flow_cem888(config, current_model, args=args)
    ...
    elif selected_provider == "cem888ai":
        _model_flow_cem888ai(config, current_model)
    ...
    # a leftover OPENAI_BASE_URL in ~/.cem888/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.cem888/.env if the active provider is not 'custom'.
    ...
    """
    from cem888_cli.config import get_env_value, save_env_value, load_config
...
#
# CEM888 uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `cem888 model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `cem888 model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`cem888_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from cem888_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from cem888_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from cem888_cli.config import load_config
    ...
    while True:
        ...
        print("  CEM888 only falls back to a lightweight backend (OpenRouter,")
        print("  CEM888.AI Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``cem888 model`` flow, then route aux tasks to them here.
    """
    from cem888_cli.config import load_config
    from cem888_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from cem888_cli.auth import _prompt_model_selection
    from cem888_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from cem888_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from cem888_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from cem888_constants import OPENROUTER_BASE_URL
    from cem888_cli.auth import (
        ProviderConfig,
    ...
    from cem888_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.cem888/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `cem888 setup` from scratch. OpenRouter
    ...
    from cem888_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_cem888(config, current_model="", args=None):
    """CEM888.AI Portal provider: ensure logged in, then pick model."""
    from cem888_cli.auth import (
        get_provider_auth_state,
        CEM888_INFERENCE_AUTH_MODE_LEGACY,
        _prompt_model_selection,
        _save_model_choice,
        _update_config_for_provider,
        resolve_cem888_runtime_credentials,
        AuthError,
        format_auth_error,
        _login_cem888,
        PROVIDER_REGISTRY,
    ...
    from cem888_cli.cem888_subscription import prompt_enable_tool_gateway

    state = get_provider_auth_state("cem888")
    if not state or not state.get("access_token"):
        print("Not logged into CEM888.AI Portal. Starting login...")
        print()
        try:
            ...
            _login_cem888(mock_args, PROVIDER_REGISTRY["cem888"])
            ...
            return
        # login_cem888 already handles model selection + config update
        return

    # Already logged in — use curated model list (same as OpenRouter defaults).
    # The live /models endpoint returns hundreds of models; the curated list
    # shows only agentic models users recognize from OpenRouter.
    from cem888_cli.models import (
        get_curated_cem888_model_ids,
        get_pricing_for_provider,
        check_cem888_free_tier,
        partition_cem888_models_by_tier,
        union_with_portal_free_recommendations,
        union_with_portal_paid_recommendations,
    )

    model_ids = get_curated_cem888_model_ids()
    if not model_ids:
        print("No curated models available for CEM888.AI Portal.")
        return

    # Verify credentials are still valid (catches expired sessions early)
    try:
        creds = resolve_cem888_runtime_credentials(min_key_ttl_seconds=5 * 60)
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            print(f"Session expired: {msg}")
            print("Re-authenticating with CEM888.AI Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    # Fetch live pricing (non-blocking — returns empty dict on failure)
    pricing = get_pricing_for_provider("cem888")

    # Force fresh account data for model selection so recent credit purchases
    # are reflected immediately.
    free_tier = check_cem888_free_tier(force_fresh=True)
    if not free_tier:
        try:
            refreshed_creds = resolve_cem888_runtime_credentials(
                min_key_ttl_seconds=5 * 60,
                inference_auth_mode=CEM888_INFERENCE_AUTH_MODE_LEGACY,
            )
            if refreshed_creds:
    ...
    _portal_url = ""
    try:
        _cem888_state = get_provider_auth_state("cem888")
        if _cem888_state:
            _portal_url = _cem888_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        try:
            from cem888_cli.cem888_account import (
                format_portal_entitlement_message,
                get_portal_account_info,
            )

            _account_info = get_portal_account_info(force_fresh=True)
            unavailable_message = (
                format_portal_entitlement_message(
                    _account_info,
                    capability="paid CEM888.AI models",
                )
                or ""
            )
        except Exception:
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _portal_url,
        )
        model_ids, unavailable_models = partition_cem888_models_by_tier(
    ...
    if not model_ids and not unavailable_models:
        print("No models available for CEM888.AI Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from cem888_cli.auth import DEFAULT_CEM888_PORTAL_URL

            _url = (_portal_url or DEFAULT_CEM888_PORTAL_URL).rstrip("/")
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate CEM888.AI as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("cem888", inference_url)
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via CEM888.AI Portal)")
        # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from cem888_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `cem888 auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from cem888_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from cem888_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``cem888 model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``cem888 model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``cem888 auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from cem888_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from cem888_cli.auth import _save_model_choice, deactivate_provider
    from cem888_cli.config import get_env_value, load_config, save_config
    from cem888_cli.secret_prompt import masked_secret_prompt
    ...
    from cem888_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `cem888 model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from cem888_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use CEM888 URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
        ...
        (
            "cem888ai_messages",
            "CEM888.AI Messages",
            "Use /v1/messages for CEM888.AI-compatible endpoints.",
        ),
    ]
    ...
    if raw in {"4", "cem888ai", "cem888ai_messages", "messages"}:
        return "cem888ai_messages"
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    CEM888.AI-style (``/v1/messages``) endpoints, and two authentication
    modes:
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from cem888_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from cem888_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from cem888_cli import azure_detect
    ...
    print("CEM888.AI-style API endpoints.  CEM888 will probe your")
    ...
    try:
        _placeholder = (
            ...
            or "e.g. https://<resource>.openai.azure.com/openai/v1 "
              "or https://<resource>.services.ai.azure.com/cem888ai"
        )
        base_url = input(
    ...
    print("     Recommended by Microsoft. Works for both OpenAI-style and CEM888.AI-style endpoints.")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  CEM888 will install it now (the preflight below "
    ...
    if api_mode:
        mode_label = (
            "OpenAI-style" if api_mode == "chat_completions" else "CEM888.AI-style"
        )
        ...
        print("  2. CEM888.AI-style  (POST /v1/messages)")
        print("     For: CEM888 models deployed via CEM888.AI API format")
        try:
            default_choice = "2" if current_api_mode == "cem888ai_messages" else "1"
            mode_choice = (
            ...
            print("\nCancelled.")
            return
        api_mode = "cem888ai_messages" if mode_choice == "2" else "chat_completions"
    ...
    if discovered_models:
        ...
        try:
            model_name = input(
                f"Model / deployment name [{current_model or 'e.g. gpt-5.4, cem888-sonnet-4-6'}]: "
            ).strip()
        except (KeyboardInterrupt, EOFError):
            print("\nCancelled.")
            return
    ...
    mode_label = "OpenAI-style" if api_mode == "chat_completions" else "CEM888.AI-style"
    auth_label = (
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from cem888_cli.curses_ui import flush_stdin
...
# downstream call sites read `cem888_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `cem888` invocation — including
# fast paths like `cem888 --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from cem888_cli.config import save_env_value, load_config, save_config
    from cem888_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from cem888_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from cem888_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from cem888_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates CEM888 turns to `copilot --acp`.")
    print("  CEM888 currently starts its own ACP subprocess for each request.")
    print("  CEM888 uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set CEM888_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``cem888 setup`` / ``cem888 model``.
    ...
    recover from a malformed paste without editing ``~/.cem888/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from cem888_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from cem888_cli.config import save_env_value
    from cem888_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `cem888 setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from cem888_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so cem888 knows where to find it
...
def _model_flow_bedrock(config, current_model=""):
    ...
    if live_models:
        ...
        _RECOMMENDED = [
            "us.cem888ai.cem888-sonnet-4-6",
            "us.cem888ai.cem888-opus-4-6",
            "us.cem888ai.cem888-haiku-4-5",
            "us.amazon.nova-pro",
            "us.amazon.nova-lite",
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   CEM888 typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with CEM888, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, CEM888.AI."
                )
                print()
    ...
    if provider_id == "lmstudio":
        from cem888_cli.auth import AuthError
        from cem888_cli.models import fetch_lmstudio_models
        ...
        from cem888_cli.models import fetch_ollama_cloud_models
...
def _run_cem888ai_oauth_flow(save_env_value):
    """Run the CEM888 OAuth setup-token flow. Returns True if credentials were saved."""
    from agent.cem888ai_adapter import (
    ...
    from cem888_cli.config import (
        save_cem888ai_oauth_token,
        use_cem888ai_cem888_code_credentials,
    )

    def _activate_cem888_code_credentials_if_available() -> bool:
        try:
            creds = read_cem888_code_credentials()
        except Exception:
            creds = None
        if creds and (
            is_cem888_code_token_valid(creds) or bool(creds.get("refreshToken"))
        ):
            use_cem888ai_cem888_code_credentials(save_fn=save_env_value)
            print("  ✓ CEM888 Code credentials linked.")
            from cem888_constants import display_cem888_home as _dhh_fn

            print(
                f"    CEM888 will use CEM888's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        print()
        print("  Running 'cem888 setup-token' — follow the prompts below.")
        print("  A browser window will open for you to authorize access.")
        print()
        token = run_oauth_setup_token()
        if token:
            if _activate_cem888_code_credentials_if_available():
                return True
            save_cem888ai_oauth_token(token, save_fn=save_env_value)
        ...
        try:
            ...
            return False
        if manual_token:
            save_cem888ai_oauth_token(manual_token, save_fn=save_env_value)
        ...
        # CEM888 CLI not installed — guide user through manual setup
        print()
        print("  The 'cem888' CLI is required for OAuth login.")
        print()
        print("  To install and authenticate:")
        print()
        print("    1. Install CEM888 Code:  npm install -g @cem888ai-ai/cem888-code")
        print("    2. Run:                  cem888 setup-token")
        print("    3. Follow the browser prompts to authorize")
        print("    4. Re-run:               cem888 model")
        print()
        ...
        if token:
            ...
            print("  ✓ Setup-token saved.")
            return True
        print("  Cancelled — install CEM888 Code and try again.")
        return False


def _model_flow_cem888ai(config, current_model=""):
    """Flow for CEM888.AI provider — OAuth subscription, API key, or CEM888 Code creds."""
    from cem888_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    from cem888_cli.config import (
        ...
        load_config,
        save_config,
        save_cem888ai_api_key,
    )
    from cem888_cli.models import _PROVIDER_MODELS

    # Check ALL credential sources
    from cem888_cli.auth import get_cem888ai_key

    existing_key = get_cem888ai_key()
    cc_available = False
    try:
        from agent.cem888ai_adapter import (
            read_cem888_code_credentials,
        ...
        cc_creds = read_cem888_code_credentials()
        if cc_creds and is_cem888_code_token_valid(cc_creds):
            cc_available = True
    except Exception:
        pass
    ...
    if has_creds:
        ...
        if existing_key:
            ...
            # nothing is wired up.
            source_suffix = ""
            for var in PROVIDER_REGISTRY["cem888ai"].api_key_env_vars:
                if os.getenv(var, "").strip() == existing_key:
                    source_suffix = format_secret_source_suffix(var)
                    if source_suffix:
                        break
            print(
                f"  CEM888.AI credentials: {existing_key[:12]}... ✓{source_suffix}"
            )
        elif cc_available:
            print("  CEM888 Code credentials: ✓ (auto-detected)")
        print()
        print("    1. Use existing credentials")
        print("    2. Reauthenticate (new OAuth login)")
        print("    3. Cancel")
    ...
    if needs_auth:
        ...
        print("  Choose authentication method:")
        print()
        print("    1. CEM888 Pro/Max subscription (OAuth login)")
        print("    2. CEM888.AI API key (pay-per-token)")
        print("    3. Cancel")
        print()
        try:
            choice = input("  Choice [1/2/3]: ").strip()
        ...
        if choice == "1":
            if not _run_cem888ai_oauth_flow(save_env_value):
                return

        elif choice == "2":
            print()
            print("  Get an API key at: https://platform.cem888.com/settings/keys")
            print()
            from cem888_cli.secret_prompt import masked_secret_prompt
            ...
            if not api_key:
                print("  Cancelled.")
                return
            save_cem888ai_api_key(api_key, save_fn=save_env_value)
            print("  ✓ API key saved.")
    ...
    # Model selection
    model_list = _PROVIDER_MODELS.get("cem888ai", [])
    if model_list:
        selected = _prompt_model_selection(model_list, current_model=current_model)
    else:
        try:
            selected = input("Model name (e.g., cem888-sonnet-4-20250514): ").strip()
        except (KeyboardInterrupt, EOFError):
            selected = None
    ...
    if selected:
        ...
        # resolve_runtime_provider() always hardcodes CEM888.AI's URL.
        # Leaving a stale base_url in config can contaminate other
        # providers if the user switches without running 'cem888 model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
            cfg["model"] = model
        model["provider"] = "cem888ai"
        model.pop("base_url", None)
        save_config(cfg)
        deactivate_provider()

        print(f"Default model set to: {selected} (via CEM888.AI)")
    else:
        print("No change.")


def cmd_login(args):
    """Authenticate CEM888 CLI with a provider."""
    from cem888_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from cem888_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from cem888_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from cem888_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from cem888_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from cem888_cli.webhook import webhook_command

    webhook_command(args)


def cmd_portal(args):
    """CEM888.AI Portal status and Tool Gateway routing surface."""
    from cem888_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    Dispatches ``cem888 slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: cem888 slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `cem888 slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from cem888_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from cem888_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from cem888_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from cem888_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `cem888 security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from cem888_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from cem888_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from cem888_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from cem888_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up CEM888 home directory to a zip file."""
    if getattr(args, "quick", False):
        from cem888_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from cem888_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a CEM888 backup from a zip file."""
    from cem888_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from cem888_cli.banner import check_for_updates
        from cem888_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall CEM888 Agent."""
    _require_tty("uninstall")
    from cem888_cli.uninstall import run_uninstall
...
# Critical files that every ``cem888`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``cem888 update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "cem888_cli/main.py",
    "cem888_cli/config.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``cem888`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="cem888-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``cem888 update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``cem888_cli/web_dist/`` (per vite.config.ts
    outDir: "../cem888_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "cem888_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``cem888`` launcher present but ``cem888_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``cem888 update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``cem888 web``).
    ...
    # build path usable on a stock `py -m cem888_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (cem888 web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``cem888 dashboard`` processes other than ourselves.

    ``cem888 dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``cem888 update`` replaces files on disk, the running
    ...
    patterns = [
        "cem888 dashboard",
        "cem888_cli.main dashboard",
        "cem888_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "cem888.*dashboard"`) keeps us consistent
            # with `cem888_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `cem888 update`.
    ...
    print("  Preview now:  cem888 curator run --dry-run")
    print("  Pause it:     cem888 curator pause")
    print(
        "  Docs:         https://cem888.ai/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``cem888 update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``cem888 update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: cem888 curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``cem888 dashboard`` processes.

    Called at the end of ``cem888 update`` (default ``reason``) and also
    from ``cem888 dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    cem888 dashboard --port <port>")
...
def _update_via_zip(args):
    """Update CEM888 Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `cem888 update "
            f"--branch {branch}`, or update against main with `cem888 update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/CEM888AI/CEM888/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="cem888-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"cem888-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for cem888-agent itself
        ...
        # GitHub ZIPs extract to cem888-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"cem888-agent-{branch}")
    ...
    # optional — a failure here only affects ``cem888 dashboard``. Make
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "cem888-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if CEM888 behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes cem888 completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but CEM888 couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but CEM888 couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if CEM888 behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `cem888 update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/CEM888AI/CEM888.git",
    "git@github.com:CEM888AI/CEM888.git",
    "https://github.com/CEM888AI/CEM888",
    "git@github.com:CEM888AI/CEM888",
}
OFFICIAL_REPO_URL = "https://github.com/CEM888AI/CEM888.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_cem888_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_cem888_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official CEM888 repository.")
        print("  This means you may miss updates from CEM888AI/CEM888.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/CEM888AI/CEM888.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/CEM888AI/CEM888.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``cem888 update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from cem888_constants import get_default_cem888_root

    default_home = get_default_cem888_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``cem888 update`` is still progressing even if pip/uv itself is silent.
    """
...
def _cem888_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "cem888.exe",
        scripts_dir / "cem888-gateway.exe",
    ]


def _detect_concurrent_cem888_instances(
    ...
    The CEM888 Desktop Electron app spawns ``cem888.EXE`` as a backend child,
    so during ``cem888 update`` the user-invoked process and the desktop's
    ...
    shims (``cem888.exe`` / ``cem888-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``cem888 update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``cem888.exe``) is a separate process from the Python
    ...
    # entire parent chain. On Windows the setuptools-generated cem888.exe
    ...
    # walking the parent chain, every ``cem888 update`` reports its own
    ...
    for shim in _cem888_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "cem888.exe"
    lines = ["✗ Another cem888.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close CEM888 Desktop, exit any open `cem888` REPLs, and")
    lines.append("  stop the gateway (`cem888 gateway stop`) before retrying.")
    lines.append("  Override with `cem888 update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_cem888_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``cem888.exe``.
    ...
    when ``cem888 update`` runs, ``cem888.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``cem888.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next cem888 invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the CEM888 Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       CEM888 Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _cem888_exe_shims(scripts_dir):
        ...
        print(
            "    Close CEM888 Desktop, exit other `cem888` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `cem888 update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_cem888_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``cem888.exe.old.*`` left by prior updates.

    Called early on every cem888 invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``cem888 update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `cem888 update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``cem888.exe`` / ``cem888-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_cem888_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_cem888_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from cem888_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.cem888/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``cem888 update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.cem888/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``cem888 update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``cem888 update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.cem888/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``cem888 update --gateway``) the update is already
    ...
    try:
        ...
        # cem888_cli.config.get_cem888_home to simulate setup failure.
        from cem888_cli.config import get_cem888_home as _get_cem888_home

        logs_dir = _get_cem888_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``cem888 update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from cem888_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``cem888 update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from cem888_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from cem888_cli.config import recommended_update_command
        from cem888_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``cem888 update`` without requiring a reinstall.  The
    ...
    ``cem888`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/cem888.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v cem888'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/cem888, code at /usr/local/lib/cem888-agent).
    fhs_link = Path("/usr/local/bin/cem888")
    ...
    path_comment = (
        "# CEM888 Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of CEM888_HOME before running the update.
    ...
    CEM888_HOME directories.  The ``--backup`` flag on ``cem888 update``
    ...
    try:
        from cem888_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_cem888_home so the user sees ~/.cem888/...
    try:
        from cem888_constants import get_cem888_home, display_cem888_home

        home = get_cem888_home()
        try:
            display_path = f"{display_cem888_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  cem888 import {out_path}")
...
def cmd_update(args):
    """Update CEM888 Agent to the latest version.
    ...
    if is_managed():
        managed_error("update CEM888 Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `cem888 update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update CEM888 via pip (for PyPI installs)."""
    from cem888_cli import __version__
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "cem888-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "cem888-agent"]
    ...
    print("✓ Update complete! Restart cem888 to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating CEM888 Agent...")
    print()

    # On Windows, abort early if another cem888.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_cem888_instances(scripts_dir)
            if concurrent:
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://cem888.ai/install/linux.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # orphan merge-conflict markers in cem888_cli/config.py bricked
        # every user who ran ``cem888 update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``cem888
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``cem888 update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_cem888_home added to cem888_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload cem888_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_cem888_home() added since the last release.
        try:
            import importlib
            import cem888_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit CEM888_HOME so
        # it is not affected by sync_skills()'s module-level CEM888_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CEM888_HOME env var points at the default or a named profile.
        try:
            from cem888_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: cem888 config migrate")
            else:
                print()
                print("Skipped. Run 'cem888 config migrate' later to configure.")
        else:
        ...
        # check `cem888 curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``cem888 update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from cem888_cli.tools_config import install_cua_driver
        ...
        # When running as ``cem888 update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_cem888_home() / ".update_exit_code"
            try:
        ...
        try:
            from cem888_cli.gateway import (
                is_macos,
            ...
            # Discover all cem888-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. cem888-gateway.service or cem888-gateway-coder.service
                            ...
                            # path in `cem888 gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: cem888 gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: cem888 -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy CEM888 gateway unit files are still installed.
        # When both cem888.service (from a pre-rename install) and the
        # current cem888-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `cem888 update` surfaces the issue until the user migrates.
        try:
            from cem888_cli.gateway import (
                has_legacy_cem888_units,
                _find_legacy_cem888_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_cem888_units():
                print()
                print("⚠ Legacy CEM888 gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_cem888_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (cem888.service) fight the current")
                print("  cem888-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    cem888 gateway migrate-legacy")
                print()
        ...
        print("  cem888 model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``cem888 -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from cem888_constants import display_cem888_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `cem888 profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_cem888_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → cem888 -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.cem888)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  cem888 profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     cem888 -p {name} chat")
                else:
        ...
        from cem888_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from cem888_constants import get_cem888_home as _hh
        ...
        from cem888_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `cem888 profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from cem888_cli.profiles import profile_exists
        ...
        from cem888_cli.profiles import rename_profile
        ...
        from cem888_cli.profiles import export_profile
        ...
        from cem888_cli.profiles import import_profile
        ...
        import tempfile
        from cem888_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="cem888_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  cem888 -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      cem888 -p {plan.manifest.name} chat")
        ...
        from cem888_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `cem888 profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"cem888 -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from cem888_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("cem888_requires"):
            print(f"Requires:     CEM888 {data['cem888_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from cem888_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.cem888_requires:
        print(f"  Requires: CEM888 {mf.cem888_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``cem888 dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``cem888 dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No cem888 dashboard processes running.")
        return 0

    print(f"{len(pids)} cem888 dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No cem888 dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `cem888 update`.
    ...
    if "CEM888_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["CEM888_WEB_DIST"])
            if "CEM888_WEB_DIST" in os.environ
            else PROJECT_ROOT / "cem888_cli" / "web_dist"
        )
    ...
    # (e.g. plugins/dashboard_auth/cem888) registers BEFORE start_server's
    ...
    try:
        from cem888_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from cem888_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("CEM888_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from cem888_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter CEM888 log files."""
    from cem888_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from cem888_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "cem888", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
            "cem888ai", "gemini", "google-gemini-cli", "xai", "bedrock", "azure-foundry",
...
# so that in ``cem888 -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``cem888_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``cem888 -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``cem888`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("CEM888_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from cem888_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from cem888_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["CEM888_DEFER_AGENT_STARTUP"] = "1"
            os.environ["CEM888_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["CEM888_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `cem888 --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("CEM888_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("CEM888_TUI") == "1"):
        return False
...
def main():
    """Main entry point for cem888 CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from cem888_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``cem888.exe.old.*`` quarantine files left by previous
    # ``cem888 update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_cem888_exe``.
    try:
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for CEM888.AI login (default: production portal)",
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for CEM888.AI login (default: production inference API)",
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for CEM888.AI login (default: cem888-cli)",
    )
    model_parser.add_argument(
        "--scope", default=None, help="OAuth scope to request for CEM888.AI login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during CEM888.AI login",
    )
    model_parser.add_argument(
        "--manual-paste",
        action="store_true",
    ...
    model_parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for CEM888.AI login (default: 15)",
    )
    model_parser.add_argument(
        "--ca-bundle", help="Path to CA bundle PEM file for CEM888.AI TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for CEM888.AI login (testing only)",
    )
    ...
    # =========================================================================
    from cem888_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://cem888.ai/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `cem888 model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.cem888/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://cem888.ai/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    # migrate command
    # =========================================================================
    from cem888_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if CEM888_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy cem888.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy CEM888 gateway unit files "
            "(e.g. cem888.service) left over from older installs. Profile "
            "units (cem888-gateway-<profile>.service) and unrelated "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. CEM888.AI Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="cem888",
        help="Upstream provider: cem888 or xai (default: cem888). See `cem888 proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure CEM888 Agent with an interactive wizard. "
        "Run a specific section: cem888 setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'cem888 setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    setup_parser.add_argument(
        "--portal",
        action="store_true",
        help="One-shot CEM888.AI Portal setup: log in via OAuth, set CEM888.AI as the "
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for CEM888.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $CEM888_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "CEM888")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from cem888_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for CEM888 CLI",
    )
    login_parser.add_argument(
        "--provider",
        choices=["cem888", "openai-codex", "xai-oauth"],
        default=None,
        help="Provider to authenticate with (default: cem888)",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: cem888-cli)"
    )
    ...
    logout_parser.add_argument(
        "--provider",
        choices=["cem888", "openai-codex", "xai-oauth", "spotify"],
        default=None,
    ...
    auth_add.add_argument(
        "provider",
        help="Provider id (for example: cem888ai, openai-codex, openrouter)",
    )
    auth_add.add_argument(
        "--type",
        dest="auth_type",
    ...
    auth_add.add_argument("--portal-url", help="CEM888.AI portal base URL")
    auth_add.add_argument("--inference-url", help="CEM888.AI inference base URL")
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate CEM888 with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set CEM888_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of CEM888 Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.cem888/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--workdir",
        help="Absolute path for the job to run from. Injects AGENTS.md / CEM888.md / .cursorrules from that directory and uses it as the cwd for terminal/file/code_exec tools. Omit to preserve old behaviour (no project context files).",
    )
    cron_create.add_argument(
        "--profile",
        help="CEM888 profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.cem888/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="CEM888 profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    # portal command — CEM888.AI Portal status + Tool Gateway routing
    # =========================================================================
    from cem888_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from cem888_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.cem888/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.cem888/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with CEM888 Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `cem888 "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the CEM888 "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.cem888/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the CEM888 Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your CEM888 setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for CEM888 Agent. Use 'cem888 debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    cem888 debug share              Upload debug report and print URL
    cem888 debug share --lines 500  Include more log lines
    cem888 debug share --expire 30  Keep paste for 30 days
    cem888 debug share --local      Print report locally (no upload)
    cem888 debug share --no-redact  Disable upload-time secret redaction
    cem888 debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'cem888 debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up CEM888 home directory to a zip file",
        description="Create a zip archive of your entire CEM888 configuration, "
        "skills, sessions, and data (excludes the cem888-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/cem888-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.cem888/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo cem888 uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from cem888_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a CEM888 backup from a zip file",
        description="Extract a previously created CEM888 backup into your "
        "CEM888 home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage CEM888 Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from cem888_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.cem888/skills/.bundled_manifest) "
            "so future 'cem888 update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from cem888_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from cem888_cli.skills_hub import skills_command
    ...
    from cem888_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/cem888-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `cem888 plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from cem888_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``cem888 --help``, ``cem888 version``, ``cem888 logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from cem888_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from cem888_cli.curator import register_cli as _register_curator_cli
    ...
    memory_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Built-in memory (memories/*_MANIFEST.md + state/memory_provenance.db) is always active."
        ),
    )
    ...
    _reset_parser = memory_sub.add_parser(
        "reset",
        help="Erase all built-in memory (manifests + durable typed store)",
    )
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            # CEM-170: built-in memory lives in TWO places, and a reset that
            # only touches one of them is not a reset.
            #   * memories/*_MANIFEST.md -- the small injected pointer surface
            #   * state/memory_provenance.db -- the durable typed store the
            #     manifests point at, and where recall actually reads from
            # Legacy pre-CEM-170 manifest names are listed too so that an
            # unmigrated profile resets cleanly.  Note the legacy entry
            # memories/USER.md is a MANIFEST, not the real profile: the real
            # profile is profile-root USER.md, which is deliberately NOT
            # touched here -- that collision is the bug CEM-170 fixed.
            files_to_reset = []
            if target in {"all", "memory"}:
                files_to_reset.append(("MEMORY_MANIFEST.md", "operating/project manifest"))
                files_to_reset.append(("MEMORY.md", "operating/project manifest (legacy name)"))
            if target in {"all", "user"}:
                files_to_reset.append(("USER_MANIFEST.md", "preference/identity manifest"))
                files_to_reset.append(("USER.md", "preference/identity manifest (legacy name)"))

            existing = [
                (f, desc) for f, desc in files_to_reset if (mem_dir / f).exists()
            ]

            # The durable typed store -- this is where recall reads from, so it
            # has to be part of "blank slate".
            durable_files = []
            if target == "all":
                for name in (
                    "memory_provenance.db",
                    "memory_provenance.db-wal",
                    "memory_provenance.db-shm",
                ):
                    path = state_dir / name
                    if path.exists():
                        durable_files.append(path)

            if not existing and not durable_files:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_cem888_home()}/memories/ or /state/\n"
                )
                return

            print(f"\n  This will permanently erase the following memory:")
            for f, desc in existing:
                path = mem_dir / f
                size = path.stat().st_size
                print(f"    ◆ {f} ({desc}) — {size:,} bytes")
            for path in durable_files:
                print(f"    ◆ state/{path.name} (durable typed store) — {path.stat().st_size:,} bytes")
            ...
            for path in durable_files:
                path.unlink()
                print(f"  ✓ Deleted state/{path.name} (durable typed store)")

            print(
                f"\n  Memory reset complete. New sessions will start with a blank slate."
            )
            print(f"  Manifest dir: {display_cem888_home()}/memories/")
            print(f"  Durable store: {display_cem888_home()}/state/ (recreated on next write)\n")
        else:
            from cem888_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'cem888 tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # cem888 tools list [--platform cli]
    ...
    # cem888 tools disable <name...> [--platform cli]
    ...
    # cem888 tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from cem888_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from cem888_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `cem888 computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `cem888 tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: cem888 computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: cem888 computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run CEM888 as an MCP server",
        description=(
            "Manage MCP server connections and run CEM888 as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'cem888 mcp add' to connect to a new server, or\n"
            "'cem888 mcp serve' to expose CEM888 conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run CEM888 as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `cem888 mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (CEM888.AI-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `cem888 mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List CEM888.AI-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `cem888 mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from cem888_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_cem888_home() / "sessions"
            ...
            # Launch cem888 --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to CEM888",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to CEM888",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.cem888/ (by default a "
        "single restore-point archive is written to ~/.cem888/backups/ "
        "before apply; restorable with 'cem888 import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from cem888_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update CEM888 Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'cem888 config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, cem888 will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another cem888.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall CEM888 Agent",
        description="Remove CEM888 Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run CEM888 Agent as an ACP (Agent Client Protocol) server",
        description="Start CEM888 Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print CEM888 ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive CEM888 provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.cem888/node/ "
    ...
    def cmd_acp(args):
        """Launch CEM888 Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated CEM888 instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `cem888 update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `cem888 profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a CEM888 profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the CEM888 Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `cem888 --tui` via PTY/WebSocket). "
            "Alternatively set CEM888_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `cem888 dashboard` cmdlines and SIGTERM them directly — the same
    # path `cem888 update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running cem888 dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running cem888 dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter CEM888 log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    cem888 logs                    Show last 50 lines of agent.log
    cem888 logs -f                 Follow agent.log in real time
    cem888 logs errors             Show last 50 lines of errors.log
    cem888 logs gateway -n 100     Show last 100 lines of gateway.log
    cem888 logs --level WARNING    Only show WARNING and above
    cem888 logs --session abc123   Filter by session ID
    cem888 logs --component tools  Only show tool-related lines
    cem888 logs --since 1h         Lines from the last hour
    cem888 logs --since 30m -f     Follow, starting from 30 min ago
    cem888 logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``cem888 -c Pokemon Agent Dev`` → ``cem888 -c 'Pokemon Agent Dev'``
    ...
    from cem888_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'cem888 -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (cem888 hooks list, cron
