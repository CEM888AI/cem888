...
- ``cem888_cli.providers``        -- canonical provider identity + overlays
- ``cem888_cli.model_normalize``  -- per-provider name formatting
...
_CEM888_MODEL_WARNING = (
    "CEM888.AI CEM888 3 & 4 models are NOT agentic and are not designed "
    "for use with CEM888 Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(CEM888, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real CEM888.AI CEM888 3 / CEM888 4 chat families.
# The previous substring check (`"cem888" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``cem888-brain:qwen3-14b-ctx16k`` that just
# happen to carry "cem888" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   CEM888.AI/CEM888-3-Llama-3.1-70B, cem888-4-405b, openrouter/cem8883:70b
# Negative examples it must NOT match:
#   cem888-brain:qwen3-14b-ctx16k, qwen3:14b, cem888-opus-4-6
_CEM888_CEM888_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])cem888[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_cem888_cem888_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real CEM888.AI CEM888 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_CEM888_CEM888_NON_AGENTIC_RE.search(model_name))


def _check_cem888_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a CEM888.AI CEM888 3/4 chat model."""
    if is_cem888_cem888_non_agentic(model_name):
        return _CEM888_MODEL_WARNING
    return ""
...
MODEL_ALIASES: dict[str, ModelIdentity] = {
    # CEM888.AI
    "sonnet":    ModelIdentity("cem888ai", "cem888-sonnet"),
    "opus":      ModelIdentity("cem888ai", "cem888-opus"),
    "haiku":     ModelIdentity("cem888ai", "cem888-haiku"),
    "cem888":    ModelIdentity("cem888ai", "cem888"),
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``cem888 config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from cem888_cli.model_switch import
...
def parse_model_flags(raw_args: str) -> tuple[str, str, bool, bool]:
    ...
    Examples::
        ...
        "sonnet --provider cem888ai"    -> ("sonnet", "cem888ai", False, False)
        "--provider my-ollama"           -> ("", "my-ollama", False, False)
        "--refresh"                      -> ("", "", False, True)
        "sonnet --provider cem888ai --global" -> ("sonnet", "cem888ai", True, False)
    """
    is_global = False
    explicit_provider = ""
    force_refresh = False
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "cem888")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "cem888")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, CEM888.AI, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'cem888 model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'cem888 doctor' — config issues detected:"
    ...
    # Mirrors the same logic in cem888_cli.runtime_provider.resolve_runtime_provider;
    # without it, /model switches into an cem888ai_messages-routed OpenCode
    # model (e.g. `/model minimax-m2.7` on opencode-go, `/model cem888-sonnet-4-6`
    ...
    cem888_warn = _check_cem888_model_warning(new_model)
    if cem888_warn:
        warnings.append(cem888_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from cem888_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from cem888_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by cem888 provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "cem888" pulls from the remote model-catalog manifest published at
    # https://cem888.ai/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a CEM888 release. Falls back to the in-repo
    # _PROVIDER_MODELS["cem888"] snapshot when the manifest is unreachable.
    curated["cem888"] = get_curated_cem888_model_ids()
    ...
    # --- 1. Check CEM888-mapped providers ---
    for cem888_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(cem888_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (CEM888_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from cem888_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(cem888_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `cem888 model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(cem888_id)
        if not model_ids:
            model_ids = curated.get(cem888_id, [])
            if cem888_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(cem888_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check CEM888-only providers (cem888, openai-codex, copilot, opencode-go) ---
    from cem888_cli.providers import CEM888_OVERLAYS
    from cem888_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → CEM888 provider ID.
    # CEM888_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use CEM888 IDs ("copilot").
    _mdev_to_cem888 = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in CEM888_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve CEM888 slug — e.g. "github-copilot" → "copilot"
        cem888_slug = _mdev_to_cem888.get(pid, pid)
        if cem888_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(cem888_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, cem888_slug):
        ...
        # support OAuth (e.g. cem888ai supports both API key and CEM888 Code
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or cem888_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(cem888_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", cem888_slug, exc)
        # Fallback: check external credential files directly.
        # The credential pool gates cem888ai behind
        # is_provider_explicitly_configured() to prevent auxiliary tasks
        # from silently consuming CEM888 Code tokens (PR #4210).
        # But the /model picker is discovery-oriented — we WANT to show
        # providers the user can switch to, even if they aren't currently
        # configured.
        if not has_creds and cem888_slug == "cem888ai":
            try:
                ...
                if (cem888_creds and cem888_creds.get("accessToken")) or \
                   (cc_creds and cc_creds.get("accessToken")):
                    has_creds = True
            except Exception as exc:
                logger.debug("CEM888.AI external creds check failed: %s", exc)
        if not has_creds:
            continue

        if cem888_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(cem888_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(cem888_slug)
                model_ids = _ids if _ids else (curated.get(cem888_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(cem888_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(cem888_slug, []) or curated.get(pid, [])
                if cem888_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(cem888_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": cem888_slug,
            "name": get_label(cem888_slug),
            "is_current": cem888_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "cem888",
        ...
        seen_slugs.add(cem888_slug.lower())
        _record_builtin_endpoint(cem888_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CEM888_OVERLAYS (keeps /model in sync
    # with `cem888 model`).
    try:
        from cem888_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is CEM888's canonical write key (matches
            ...
            # CEM888 writes ``models:`` as a dict keyed by model id
            # (see cem888_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # CEM888's own writer uses; a hyphen variant is accepted
            ...
            # active model. CEM888's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, cem888_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`cem888_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from cem888_cli.models import fetch_openrouter_models
