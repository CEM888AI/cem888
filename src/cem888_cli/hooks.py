"""cem888 hooks — inspect and manage shell-script hooks.

Usage::

    cem888 hooks list
    cem888 hooks test <event> [--for-tool X] [--payload-file F]
    cem888 hooks revoke <command>
    cem888 hooks doctor

Consent records live under ``~/.cem888/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.cem888/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``cem888 hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: cem888 hooks {list|test|revoke|doctor}")
        print("Run 'cem888 hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.cem888/config.yaml.")
        print("See `cem888 hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `cem888 hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `cem888 hooks test` and `cem888 hooks doctor`
...
_DEFAULT_PAYLOADS = {
    ...
    "pre_api_request": {
        ...
        "task_id": "test-task",
        "platform": "cli",
        "model": "cem888-sonnet-4-6",
        "provider": "cem888ai",
        "base_url": "https://api.cem888ai.com",
    ...
    "post_api_request": {
        ...
        "finish_reason": "stop",
        "message_count": 4,
        "response_model": "cem888-sonnet-4-6",
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (CEM888 wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `cem888 hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `cem888 hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `cem888 hooks doctor`.")
