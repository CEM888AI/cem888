"""
Unified tool configuration for CEM888 Agent.

`cem888 tools` and `cem888 setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.cem888/config.yaml under
...
from cem888_cli.cem888_subscription import (
    apply_cem888_managed_defaults,
    get_cem888_subscription_features,
)
from cem888_cli.cem888_account import format_portal_entitlement_message
...
from cem888_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _CEM888_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `cem888 tools` → Video Generation, which walks
...
# `cem888 tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from cem888_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `cem888 tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``cem888 tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from cem888_cli.plugins import discover_plugins, get_plugin_toolsets
...
from cem888_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            {
                "name": "CEM888.AI Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_cem888_auth": True,
                "managed_cem888_feature": "tts",
    ...
    "web": {
        ...
        #   - "CEM888.AI Subscription" — managed Firecrawl billed via CEM888.AI
        #     subscription (requires_cem888_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "CEM888.AI Subscription" — managed FAL billed via the CEM888.AI
        #     subscription (requires_cem888_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_cem888_auth": True,
                "managed_cem888_feature": "image_gen",
    ...
    "x_search": {
        ...
        "setup_note": (
            "CEM888 routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        #   - "CEM888.AI Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via CEM888.AI subscription (requires_cem888_auth +
        ...
        "providers": [
            {
                "name": "CEM888.AI Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_cem888_auth": True,
                "managed_cem888_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. CEM888_CUA_DRIVER_VERSION is an
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("CEM888_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``cem888 update`` and
      by ``cem888 computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `cem888 update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / CEM888 process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_cem888_home()}/cem888-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in cem888-agent directory)")
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `cem888 update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.cem888/config.yaml")
        ...
        # Run the full `cem888 auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.cem888/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from cem888_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: cem888 auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `cem888 auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: cem888 auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: cem888 auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `cem888 auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: cem888 auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: cem888 auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "cem888-cli" (which include all _CEM888_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[cem888-cli, spotify]`` after enabling Spotify via ``cem888
        ...
        # (e.g. "cem888-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``cem888 tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `cem888 tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"cem888-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("cem888-")}
    ...
    # shipped as a bundled plugin but user must opt in via `cem888 tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `cem888 tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by cem888 tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (cem888-cli, cem888-telegram, etc.)
    ...
    # Opening `cem888 tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key in {"web", "image_gen", "tts", "browser"}:
        features = get_cem888_subscription_features(config, force_fresh=force_fresh)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_cem888):
            return True
...
# ("CEM888.AI Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("CEM888.AI Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    for provider in cat.get("providers", []):
        if provider.get("managed_cem888_feature") and not managed_available:
            continue
        if provider.get("requires_cem888_auth") and not features.cem888_auth_present:
            continue
        visible.append(provider)

    # Inject plugin-registered image_gen backends (OpenAI today, more
    # later) so the picker lists them alongside FAL / CEM888.AI Subscription.
    ...
    # remaining hardcoded rows ("CEM888.AI Subscription", "Firecrawl
    ...
    # the hardcoded "CEM888.AI Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_cem888_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Return a reason when a category's CEM888.AI provider is hidden."""
    features = get_cem888_subscription_features(config, force_fresh=force_fresh)
    managed_available = bool(
        features.account_info
        and features.account_info.logged_in
    ...
    if managed_available:
        return ""
    if not any(p.get("managed_cem888_feature") for p in cat.get("providers", [])):
        return ""
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `cem888 tools` gets a silent no-op
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_cem888_message = _hidden_cem888_gateway_message(
        cat,
        config,
        f"the CEM888.AI Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        if hidden_cem888_message:
            for line in hidden_cem888_message.splitlines():
        ...
        # When the user is logged into CEM888.AI, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of CEM888.AI.
        try:
            _cem888_logged_in = bool(
                get_cem888_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).cem888_auth_present
        ...
        for p in providers:
            ...
            # Highlight CEM888.AI-managed entries when the user has Portal auth.
            ...
            # picker unchanged.
            sub_marker = ""
            if _cem888_logged_in and p.get("managed_cem888_feature"):
                sub_marker = "  ★ Included with your CEM888.AI subscription"
...
def _is_provider_active(
    ...
    *,
    ...
    managed_feature = provider.get("managed_cem888_feature")
    if managed_feature:
        features = get_cem888_subscription_features(config, force_fresh=force_fresh)
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_cem888
        if provider.get("tts_provider"):
            return (
                feature.managed_by_cem888
                and cfg_get(config, "tts", "provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = cfg_get(config, "browser", "cloud_provider")
            return feature.managed_by_cem888 and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_cem888 and current == provider["web_backend"]
        return feature.managed_by_cem888
...
def _configure_provider(
    ...
    *,
    ...
    if provider.get("requires_cem888_auth"):
        features = get_cem888_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.cem888_auth_present or not entitled:
            message = format_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'CEM888.AI Subscription')}",
            )
            _print_warning(
                f"  {message or 'CEM888.AI Subscription is only available after logging into CEM888.AI Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your CEM888.AI subscription.")
    ...
    # CEM888.AI-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to CEM888.AI.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_cem888_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_cem888_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_cem888_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.cem888_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through CEM888.AI Portal subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `cem888 tools` and `cem888 setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ CEM888 Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://cem888.ai/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_cem888_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your CEM888.AI subscription defaults", Colors.GREEN))
    ...
    print()
    from cem888_constants import display_cem888_home
    print(color(f"  Tool configuration saved to {display_cem888_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'cem888' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, cem888 mcp
