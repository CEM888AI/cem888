...
DEFAULT_NOUS_CLIENT_ID = "cem888-cli"
...
NOUS_LEGACY_SESSION_KEYS_ENV = "CEM888_AGENT_USE_LEGACY_SESSION_KEYS"
...
# so a new `cem888 --profile <name> auth add nous --type oauth` can one-tap
...
NOUS_SHARED_STORE_FILENAME = "cem888_auth.json"
...
def _cem888_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from cem888_constants import get_default_cem888_root
        real_home_shared = (
            get_default_cem888_root() / "shared" / NOUS_SHARED_STORE_FILENAME
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "instead.\n"
            "Re-authenticate with: cem888 auth add nous"
        )
        relogin = True
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    """Refresh CEM888.AI OAuth from a state dict. Thin wrapper around refresh_nous_oauth_pure."""
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``cem888 auth add nous`` wrote a ``manual:device_code`` pool
    ...
    This helper writes ``providers.nous`` then calls ``load_pool("cem888")`` so
    ...
    ``cem888 auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `cem888 auth add nous --type oauth`. Best-
...
# get_cem888_auth_status() validates state by calling resolve_nous_runtime_credentials(),
