"""
Multi-provider authentication system for CEM888 Agent.

Supports OAuth device code flows (CEM888.AI Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.cem888/auth.json with cross-process file locking.
...
CEM888.AI authentication paths:
...
- Legacy session key (fallback): mint an opaque 24h key when JWT auth is
  unavailable, or when CEM888_AGENT_USE_LEGACY_SESSION_KEYS is set for
...
from cem888_cli.config import get_cem888_home, get_config_path, read_raw_config
from cem888_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# CEM888.AI Portal defaults
DEFAULT_CEM888_PORTAL_URL = "https://cem888.ai"
DEFAULT_CEM888_INFERENCE_URL = "https://api.cem888ai.com/v1"
DEFAULT_CEM888_CLIENT_ID = "cem888-cli"
CEM888_LEGACY_AGENT_KEY_SCOPE = "inference:mint_agent_key"
CEM888_INFERENCE_INVOKE_SCOPE = "inference:invoke"
DEFAULT_CEM888_SCOPE = f"{CEM888_INFERENCE_INVOKE_SCOPE} {CEM888_LEGACY_AGENT_KEY_SCOPE}"
CEM888_LEGACY_SESSION_KEYS_ENV = "CEM888_AGENT_USE_LEGACY_SESSION_KEYS"
CEM888_DEVICE_CODE_SOURCE = "device_code"
CEM888_INFERENCE_AUTH_MODE_AUTO = "auto"
CEM888_INFERENCE_AUTH_MODE_FRESH = "fresh"
CEM888_INFERENCE_AUTH_MODE_LEGACY = "legacy"
CEM888_INFERENCE_AUTH_MODES = frozenset({
    CEM888_INFERENCE_AUTH_MODE_AUTO,
    CEM888_INFERENCE_AUTH_MODE_FRESH,
    CEM888_INFERENCE_AUTH_MODE_LEGACY,
})
CEM888_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
CEM888_AUTH_PATH_LEGACY_SESSION_KEY_CACHE = "legacy_session_key_cache"
CEM888_AUTH_PATH_LEGACY_SESSION_KEY_MINT = "legacy_session_key_mint"
DEFAULT_AGENT_KEY_MIN_TTL_SECONDS = 30 * 60  # 30 minutes
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
CEM888_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
MINIMAX_OAUTH_GLOBAL_INFERENCE = "https://api.minimax.io/cem888ai"
MINIMAX_OAUTH_CN_INFERENCE = "https://api.minimaxi.com/cem888ai"
...
SPOTIFY_DOCS_URL = "https://cem888.ai/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://cem888.ai/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://cem888.ai/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "cem888": ProviderConfig(
        id="cem888",
        name="CEM888.AI Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_CEM888_PORTAL_URL,
        inference_base_url=DEFAULT_CEM888_INFERENCE_URL,
        client_id=DEFAULT_CEM888_CLIENT_ID,
        scope=DEFAULT_CEM888_SCOPE,
    ),
    ...
    "minimax": ProviderConfig(
        ...
        name="MiniMax",
        auth_type="api_key",
        inference_base_url="https://api.minimax.io/cem888ai",
    ...
    "cem888ai": ProviderConfig(
        id="cem888ai",
        name="CEM888.AI",
        auth_type="api_key",
        inference_base_url="https://api.cem888ai.com",
        api_key_env_vars=("ANTHROPIC_API_KEY", "ANTHROPIC_TOKEN", "CEM888_OAUTH_TOKEN"),
    ...
    "minimax-cn": ProviderConfig(
        ...
        name="MiniMax (China)",
        auth_type="api_key",
        inference_base_url="https://api.minimaxi.com/cem888ai",
    ...
    "opencode-go": ProviderConfig(
        ...
        # - MiniMax models use CEM888.AI Messages under /v1/messages
        # - Qwen 3.7 uses CEM888.AI Messages under /v1/messages
...
# =============================================================================
# CEM888.AI Key Helper
# =============================================================================

def get_cem888ai_key() -> str:
    """Return the first usable CEM888.AI credential, or ``""``.

    Checks both the ``.env`` file (via ``get_env_value``) and the process
    environment (``os.getenv``).  The fallback order mirrors the
    ``PROVIDER_REGISTRY["cem888ai"].api_key_env_vars`` tuple:

        ANTHROPIC_API_KEY -> ANTHROPIC_TOKEN -> CEM888_OAUTH_TOKEN
    """
...
# speaks the CEM888.AI Messages protocol, and the cem888ai SDK appends
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from cem888_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.cem888/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``cem888 auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `cem888 model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "cem888":
            return _format_cem888_entitlement_auth_error(error)
...
def _format_cem888_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        account_info = get_portal_account_info(force_fresh=True)
        message = format_portal_entitlement_message(
            account_info,
            capability="CEM888.AI model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in CEM888.AI Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("CEM888_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.cem888/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_cem888_home() / "auth.json"
    # Seat belt: if pytest is running and CEM888_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch CEM888_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".cem888" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set CEM888_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom CEM888_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_cem888_root()
    except Exception:
        return None
    profile_home = get_cem888_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.cem888/auth.json`` even when CEM888_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_cem888_root()`` for a profile-shaped CEM888_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".cem888" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared CEM888.AI store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_cem888_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared CEM888.AI lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "cem888_portal" in systems:
            providers["cem888"] = systems["cem888_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "cem888" if providers else None}
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``cem888``) that were only authenticated at
    global scope. Once the user runs ``cem888 auth login <provider>`` inside
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``cem888 auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global-scope provider state (e.g. a globally-authenticated CEM888.AI
    OAuth or CEM888.AI device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    CEM888 Code's ~/.cem888/.credentials.json) so they are never used
    ...
    # Exclude CEM888_OAUTH_TOKEN — it's set by CEM888 Code itself,
    # not by the user explicitly configuring cem888ai in CEM888.
    _IMPLICIT_ENV_VARS = {"CEM888_OAUTH_TOKEN"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `cem888 logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from cem888_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'cem888 doctor' for full diagnostics:"]
        for ci in issues:
...
def resolve_provider(
    ...
    *,
    ...
    _PROVIDER_ALIASES = {
        ...
        "cem888": "cem888ai", "cem888-code": "cem888ai",
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'cem888 model' for available providers, or run 'cem888 doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'cem888 model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.cem888/.env.",
...
# Allowlist of hosts the CEM888.AI Portal proxy is willing to forward minted
# bearer tokens to. The bearer is a long-lived agent_key minted by
# cem888.ai — sending it anywhere else would leak it.
#
...
# (CEM888_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_CEM888_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "api.cem888ai.com",
})


def _validate_cem888_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``CEM888_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "cem888: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_CEM888_INFERENCE_HOSTS:
        logger.warning(
            "cem888: refusing inference URL host %r from Portal response "
...
def _cem888_legacy_session_keys_forced() -> bool:
    return is_truthy_value(os.getenv(CEM888_LEGACY_SESSION_KEYS_ENV), default=False)


def _cem888_scope_has_invoke(raw_scope: Any) -> bool:
    return CEM888_INFERENCE_INVOKE_SCOPE in _scope_values(raw_scope)


def _normalize_cem888_inference_auth_mode(inference_auth_mode: Optional[str]) -> str:
    mode = str(inference_auth_mode or CEM888_INFERENCE_AUTH_MODE_AUTO).strip().lower()
    if mode not in CEM888_INFERENCE_AUTH_MODES:
        allowed = ", ".join(sorted(CEM888_INFERENCE_AUTH_MODES))
        raise ValueError(
            "Invalid CEM888.AI inference auth mode "
            f"{inference_auth_mode!r}; expected one of: {allowed}"
        )
    return mode


def _cem888_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = CEM888_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if CEM888_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _cem888_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _cem888_legacy_session_key_reason(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    inference_auth_mode: str = CEM888_INFERENCE_AUTH_MODE_AUTO,
) -> str:
    if inference_auth_mode == CEM888_INFERENCE_AUTH_MODE_LEGACY:
        return "forced_legacy_session_key"
    if _cem888_legacy_session_keys_forced():
        return "forced_legacy_session_keys"
    return (
        _cem888_invoke_jwt_status(token, scope=scope, expires_at=expires_at)
        or "invoke_jwt_unavailable"
    )


def _choose_cem888_inference_auth_path(
    state: Dict[str, Any],
    *,
    ...
    inference_auth_mode = _normalize_cem888_inference_auth_mode(inference_auth_mode)
    token = state.get("access_token") if access_token is None else access_token
    if (
        not _cem888_legacy_session_keys_forced()
        and inference_auth_mode != CEM888_INFERENCE_AUTH_MODE_LEGACY
        and _cem888_invoke_jwt_is_usable(
            token,
            scope=state.get("scope"),
            expires_at=state.get("expires_at"),
        )
    ):
        return CEM888_AUTH_PATH_INVOKE_JWT, None
    if (
        inference_auth_mode == CEM888_INFERENCE_AUTH_MODE_AUTO
        and _agent_key_is_usable(
            state,
            max(60, int(min_key_ttl_seconds)),
        )
    ):
        return CEM888_AUTH_PATH_LEGACY_SESSION_KEY_CACHE, None
    return (
        CEM888_AUTH_PATH_LEGACY_SESSION_KEY_MINT,
        _cem888_legacy_session_key_reason(
            token,
            scope=state.get("scope"),
            expires_at=state.get("expires_at"),
...
def _log_cem888_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.info("CEM888.AI inference auth: using NAS invoke JWT")
    _oauth_trace(
        "cem888_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _log_cem888_legacy_session_key_selected(
    reason: str,
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
    ...
    logger.info(
        "CEM888.AI inference auth: using legacy session key path (%s)",
        reason,
    )
    _oauth_trace(
        "cem888_legacy_session_key_selected",
        sequence_id=sequence_id,
        reason=reason,
        access_token_fp=_token_fingerprint(access_token),
    )
...
def _cem888_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_cem888_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _cem888_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_cem888_invoke_jwt(
    ...
    *,
    ...
    _set_cem888_agent_key_from_invoke_jwt(state)
    _log_cem888_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_CEM888_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _cem888_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _CEM888_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("CEM888_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `cem888 model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.cem888/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `cem888 auth list` / `cem888 status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.cem888/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("CEM888_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set CEM888_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("CEM888_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CEM888_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CEM888_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but CEM888 times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``CEM888_LOG_LEVEL=INFO`` (or check
            # ``~/.cem888/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>cem888 auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `cem888 auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `cem888 auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `cem888 auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `cem888 auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.cem888/.env, and return it.
    ...
    print("       App name:     anything (e.g. cem888-agent)")
    ...
    # Persist so subsequent `cem888 auth spotify` runs skip the wizard.
    save_env_value("CEM888_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("CEM888_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved CEM888_SPOTIFY_CLIENT_ID to ~/.cem888/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "CEM888_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize CEM888:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"CEM888 is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.cem888/auth.json (not ~/.codex/)
#
# CEM888 maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from CEM888 auth store (~/.cem888/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `cem888 auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `cem888 auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `cem888 auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `cem888 auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``cem888 setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``cem888 auth add openai-codex``
      ...
      ``cem888 auth add`` workaround for #33000 would silently leave that
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to CEM888 auth store (~/.cem888/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating CEM888 auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run cem888 auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `cem888 auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to CEM888 auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from CEM888's own Codex token store.
    ...
    try:
        ...
        if pool_token:
            base_url = (
                os.getenv("CEM888_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CEM888_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other CEM888 processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.cem888/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `cem888 model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `cem888 model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `cem888 model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `cem888 model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.cem888/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `cem888 model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``CEM888_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `cem888 model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older CEM888 (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `cem888 model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``cem888 model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CEM888_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("CEM888_XAI_BASE_URL", "").strip().rstrip("/")
...
def _is_cem888_invoke_scope_refusal(exc: Exception) -> bool:
    ...
    return (
        ...
        or "scope" in text and CEM888_INFERENCE_INVOKE_SCOPE in text
    )


def _cem888_device_scope_with_env_override(
    requested_scope: Optional[str],
    *,
    default_scope: str = DEFAULT_CEM888_SCOPE,
) -> Tuple[str, bool]:
    explicit_scope = requested_scope is not None
    scope = requested_scope or default_scope
    if _cem888_legacy_session_keys_forced():
        scope = CEM888_LEGACY_AGENT_KEY_SCOPE
    return scope, explicit_scope


def _request_cem888_device_code_with_scope_fallback(
    *,
    client: httpx.Client,
    portal_base_url: str,
    client_id: str,
    ...
    try:
        ...
        if (
            ...
            and _cem888_scope_has_invoke(scope)
            and _is_cem888_invoke_scope_refusal(exc)
        ):
            logger.info("CEM888.AI inference auth: NAS refused invoke scope, retrying legacy scope")
            _oauth_trace("cem888_device_code_invoke_scope_refused")
            retry_scope = CEM888_LEGACY_AGENT_KEY_SCOPE
            return (
                _request_device_code(
                    client=client,
                    portal_base_url=portal_base_url,
...
# =============================================================================
# CEM888.AI Portal — token refresh, agent key minting, model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared CEM888.AI token store — lets OAuth credentials persist across profiles
# so a new `cem888 --profile <name> auth add cem888 --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${CEM888_SHARED_AUTH_DIR}/cem888_auth.json, defaulting to
# ``<cem888-root>/shared/cem888_auth.json`` where ``<cem888-root>`` is what
# ``get_default_cem888_root()`` returns — ``~/.cem888`` on Linux/macOS,
# ``%LOCALAPPDATA%\cem888`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's CEM888_HOME so named profiles (which
# typically live under ``<cem888-root>/profiles/<name>/``) all see the
# same file.
#
...
CEM888_SHARED_STORE_FILENAME = "cem888_auth.json"
_cem888_shared_lock_holder = threading.local()


def _cem888_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared CEM888.AI token store.

    Honors ``CEM888_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<cem888-root>/shared/``, where ``<cem888-root>`` is what
    :func:`cem888_constants.get_default_cem888_root` returns — so
    Linux/macOS classic installs land at ``~/.cem888/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\cem888\\shared\\``, and
    Docker / custom ``CEM888_HOME`` deployments at
    ``<CEM888_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("CEM888_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from cem888_constants import get_default_cem888_root
    return get_default_cem888_root() / "shared"


def _cem888_shared_store_path() -> Path:
    path = _cem888_shared_auth_dir() / CEM888_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's CEM888 root, refuse rather than silently corrupt cross-profile
    # state. Tests must set CEM888_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from cem888_constants import get_default_cem888_root
        real_home_shared = (
            get_default_cem888_root() / "shared" / CEM888_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared CEM888.AI auth store during test run: "
                f"{path}. Set CEM888_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _cem888_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared CEM888.AI OAuth store.
    ...
    ``_try_import_shared_cem888_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _cem888_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No CEM888_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _cem888_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared CEM888.AI auth lock",
    ):
        yield


def _merge_shared_cem888_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local CEM888.AI state."""
    shared = _read_shared_cem888_state()
    if not shared:
        return False
...
def _write_shared_cem888_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the CEM888.AI OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_CEM888_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_CEM888_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_CEM888_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_CEM888_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _cem888_shared_store_lock():
            path = _cem888_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed CEM888.AI
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "cem888_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared CEM888.AI auth store: %s", exc)


def _read_shared_cem888_state() -> Optional[Dict[str, Any]]:
    """Return the shared CEM888.AI OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared CEM888.AI auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_cem888_state(reason: str) -> None:
    """Remove the shared CEM888.AI OAuth store after a terminal token failure."""
    try:
        with _cem888_shared_store_lock():
            path = _cem888_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("cem888_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared CEM888.AI auth store: %s", exc)


def _is_terminal_cem888_refresh_error(exc: Exception) -> bool:
    """True when retrying the same CEM888.AI refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "cem888"
...
def _quarantine_cem888_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_cem888_state(reason)
    invalidate_cem888_auth_status_cache()


def _quarantine_cem888_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded CEM888.AI pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("cem888")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {CEM888_DEVICE_CODE_SOURCE, f"manual:{CEM888_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["cem888"] = retained
        _oauth_trace(
            "cem888_pool_device_code_quarantined",
            reason=reason,
...
def _try_import_shared_cem888_state(
    *,
    timeout_seconds: float = 15.0,
    min_key_ttl_seconds: int = 5 * 60,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate CEM888.AI OAuth state from the shared store.
    ...
    for ``persist_cem888_credentials()``.
    ...
    """
    try:
        with _cem888_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_cem888_state()
            if not shared:
                return None

            # Build a full state dict so refresh_cem888_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_CEM888_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_CEM888_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_CEM888_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_CEM888_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_cem888_state(updated_state)

            refreshed = refresh_cem888_oauth_from_state(
                state,
                min_key_ttl_seconds=min_key_ttl_seconds,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                inference_auth_mode=CEM888_INFERENCE_AUTH_MODE_FRESH,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_cem888_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "cem888_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_cem888_refresh_error(exc):
            _clear_shared_cem888_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared CEM888.AI import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "cem888_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-cem888-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="cem888", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="cem888", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the CEM888.AI portal
    ...
    # called POST /api/oauth/token with CEM888's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, CEM888's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "CEM888.AI Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another CEM888 install sharing "
            "~/.cem888/auth.json) called POST /api/oauth/token with CEM888's "
            "refresh token without persisting the rotated token back.\n"
            "CEM888.AI refresh tokens are single-use — only CEM888 may call the "
            "refresh endpoint. For health checks, use `cem888 auth status` "
            "instead.\n"
            "Re-authenticate with: cem888 auth add cem888"
        )
        relogin = True

    raise AuthError(description, provider="cem888", code=code, relogin_required=relogin)
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "api_key" not in payload:
            raise AuthError("Mint response missing api_key",
                            provider="cem888", code="server_error")
        return payload
    ...
    try:
        ...
        raise AuthError("Agent key mint request failed",
                        provider="cem888", code="server_error") from exc
...
def fetch_cem888_models(
    *,
    ...
    """Fetch available model IDs from the CEM888.AI inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="cem888", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip CEM888 models — they're not reliable for agentic tool-calling
            if "cem888" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if _decode_jwt_claims(key):
        if _cem888_legacy_session_keys_forced():
            return False
        return _cem888_invoke_jwt_is_usable(
            key,
            scope=state.get("scope"),
...
def resolve_cem888_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """Resolve a refresh-aware CEM888.AI Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "cem888")

        if not state:
            raise AuthError(
                "CEM888 is not logged into CEM888.AI Portal.",
                provider="cem888",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("CEM888_PORTAL_BASE_URL")
            or os.getenv("CEM888_PORTAL_BASE_URL")
            or DEFAULT_CEM888_PORTAL_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_CEM888_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _cem888_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_cem888_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for CEM888.AI Portal login.",
                    provider="cem888",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state(auth_store, "cem888", state)
                    _save_auth_store(auth_store)
                return access_token
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_cem888_refresh_error(exc):
                        _quarantine_cem888_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_cem888_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
            ...
            _save_provider_state(auth_store, "cem888", state)
            _save_auth_store(auth_store)
            _write_shared_cem888_state(state)
            return state["access_token"]


def refresh_cem888_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    scope: str = DEFAULT_CEM888_SCOPE,
    obtained_at: Optional[str] = None,
    ...
    """Refresh CEM888.AI OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_CEM888_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_CEM888_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_CEM888_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_CEM888_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        legacy_session_keys = _cem888_legacy_session_keys_forced()
        ...
        if (
            ...
            state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
            state["scope"] = refreshed.get("scope") or state.get("scope")
            refreshed_url = _validate_cem888_inference_url_from_network(refreshed.get("inference_base_url"))
            if refreshed_url:
        ...
        selected_auth_path, fallback_reason = _choose_cem888_inference_auth_path(
            state,
            min_key_ttl_seconds=min_agent_key_ttl,
            inference_auth_mode=inference_auth_mode,
        )
        if selected_auth_path == CEM888_AUTH_PATH_INVOKE_JWT:
            _select_cem888_invoke_jwt(state)
        elif selected_auth_path == CEM888_AUTH_PATH_LEGACY_SESSION_KEY_MINT:
            _log_cem888_legacy_session_key_selected(
            ...
            minted_url = _validate_cem888_inference_url_from_network(mint_payload.get("inference_base_url"))
            if minted_url:
...
def refresh_cem888_oauth_from_state(
    state: Dict[str, Any],
    *,
    min_key_ttl_seconds: int = DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
    timeout_seconds: float = 15.0,
    ...
    """Refresh CEM888.AI OAuth from a state dict. Thin wrapper around refresh_cem888_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_cem888_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "cem888-cli"),
        state.get("portal_base_url", DEFAULT_CEM888_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_CEM888_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_CEM888_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_cem888_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist minted CEM888.AI OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    CEM888.AI credentials are read at runtime from two independent locations:

    - ``providers.cem888``: singleton state read by
      ``resolve_cem888_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.cem888``: used by the runtime ``pool.select()`` path.

    Historically ``cem888 auth add cem888`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.cem888``.  When the 24h agent_key TTL
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.cem888`` then calls ``load_pool("cem888")`` so
    ...
    ``cem888 auth add cem888 --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("cem888")`` instead of the
    ...
    # these credentials via `cem888 auth add cem888 --type oauth`. Best-
    # effort: any I/O failure is logged and swallowed (the per-profile
    # auth.json is still the source of truth).
    _write_shared_cem888_state(state)

    pool = load_pool("cem888")
    return next(
        (e for e in pool.entries() if e.source == CEM888_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_cem888_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.cem888 changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("cem888")
    except Exception as exc:
        logger.debug("Failed to sync CEM888.AI credential pool from auth store: %s", exc)


def resolve_cem888_runtime_credentials(
    *,
    min_key_ttl_seconds: int = DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ...
    """
    Resolve CEM888.AI inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("CEM888 is not logged into CEM888.AI Portal.",
                            provider="cem888", relogin_required=True)
        ...
        inference_base_url = (
            ...
            or os.getenv("CEM888_INFERENCE_BASE_URL")
            or DEFAULT_CEM888_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_CEM888_CLIENT_ID)
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed CEM888.AI auth-status cache warm during read paths.
            if (
                _cem888_effective_provider_state(state)
                == _cem888_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "cem888_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            ...
            # _write_shared_cem888_state.
            _write_shared_cem888_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "cem888_runtime_credentials_start",
            sequence_id=sequence_id,
            inference_auth_mode=inference_auth_mode,
            min_key_ttl_seconds=min_key_ttl_seconds,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for CEM888.AI Portal login.",
                                provider="cem888", relogin_required=True)
            ...
            if (
                ...
                with _cem888_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_cem888_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                    ...
                    if (
                        _is_expiring(state.get("expires_at"), ACCESS_TOKEN_REFRESH_SKEW_SECONDS)
                        and not _cem888_invoke_jwt_is_usable(
                            access_token,
                            scope=state.get("scope"),
                            expires_at=state.get("expires_at"),
                        )
            ...
            if selected_auth_path == CEM888_AUTH_PATH_INVOKE_JWT:
                _select_cem888_invoke_jwt(
                    state,
                    access_token=access_token,
                    sequence_id=sequence_id,
                )
            elif selected_auth_path == CEM888_AUTH_PATH_LEGACY_SESSION_KEY_CACHE:
                used_cached_key = True
                logger.info("CEM888.AI inference auth: using cached agent_key")
                ...
                try:
                    ...
                    if (
                        ...
                        retry_inference_auth_mode = (
                            CEM888_INFERENCE_AUTH_MODE_LEGACY
                            if inference_auth_mode == CEM888_INFERENCE_AUTH_MODE_LEGACY
                            else CEM888_INFERENCE_AUTH_MODE_FRESH
                        )
                        retry_auth_path, _ = _choose_cem888_inference_auth_path(
                            state,
                            access_token=access_token,
                            min_key_ttl_seconds=min_key_ttl_seconds,
                            inference_auth_mode=retry_inference_auth_mode,
                        )
                        if retry_auth_path == CEM888_AUTH_PATH_INVOKE_JWT:
                            mint_payload = None
                            selected_auth_path = CEM888_AUTH_PATH_INVOKE_JWT
                            _select_cem888_invoke_jwt(
                                state,
                                access_token=access_token,
                                sequence_id=sequence_id,
        ...
        _persist_state("resolve_cem888_runtime_credentials_final")

    if state_persisted:
        _sync_cem888_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a CEM888.AI inference API key",
                        provider="cem888", code="server_error")
...
def _empty_cem888_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_cem888_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_cem888_auth_status()
...
# ── Process-level memo for get_cem888_auth_status() ──
# get_cem888_auth_status() validates state by calling resolve_cem888_runtime_credentials(),
# which does a synchronous OAuth refresh POST to cem888.ai. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`cem888 tools`, status panels,
# subscription-feature checks) call it many times per render — `cem888 tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `cem888 auth login/logout/add/remove` invalidate naturally on the next call.
_CEM888_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_cem888_auth_status_cache: Optional[Tuple[float, Optional[float], Dict[str, Any]]] = None
...
def invalidate_cem888_auth_status_cache() -> None:
    """Clear the get_cem888_auth_status() process-level memo.

    Call this from any code path that mutates CEM888.AI auth state without going
    through resolve_cem888_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _cem888_auth_status_cache
    _cem888_auth_status_cache = None


def get_cem888_auth_status() -> Dict[str, Any]:
    """Status snapshot for CEM888.AI auth.
    ...
    ``invalidate_cem888_auth_status_cache()`` explicitly.
    """
    global _cem888_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _cem888_auth_status_cache
    if cached is not None:
        cached_at, cached_mtime, cached_status = cached
        if (
            cached_mtime == mtime
            and (now - cached_at) < _CEM888_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_cem888_auth_status()
    _cem888_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_cem888_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_cem888_auth_status(). See that function."""
    ...
    if state:
        ...
        try:
            creds = resolve_cem888_runtime_credentials(min_key_ttl_seconds=60)
            refreshed_state = get_provider_auth_state("cem888") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_cem888_pool_status()


def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for Codex auth.
    
    Checks the credential pool first (where `cem888 auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `cem888 auth` and
    # `cem888 model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("CEM888_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("CEM888_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``cem888 doctor`` runs the live
    ...
    try:
        from cem888_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on CEM888' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `cem888 doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set CEM888_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    mismatched model/provider (e.g. ``cem888ai/cem888-opus-4.6`` sent to
    MiniMax's API).
    """
    ...
    # (api_mode=cem888ai_messages, api_key=mxp-...) to a built-in provider
    ...
    # "cem888ai/cem888-opus-4.6" will fail on direct-API providers.
    if default_model:
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `cem888 logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"cem888", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from cem888_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_CEM888_PORTAL_URL).rstrip("/")
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from cem888_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'cem888 model' or 'cem888 setup' instead."""
    print("The 'cem888 login' command has been removed.")
    print("Use 'cem888 auth' to manage credentials,")
    print("'cem888 model' to select a provider, or 'cem888 setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.cem888/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing CEM888-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in CEM888 auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("CEM888 will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("CEM888_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("CEM888 will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — CEM888 gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(CEM888 creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to CEM888 auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from cem888_constants import display_cem888_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in CEM888 auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(CEM888 creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=cem888-agent` lets xAI attribute CEM888-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in CEM888 — please report at "
            "https://github.com/CEM888AI/CEM888/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize CEM888 with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to CEM888 auth store (~/.cem888/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting CEM888 login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the CEM888.AI / xAI-OAuth / Codex-OAuth quarantine pattern
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    The CEM888.AI SDK caches ``api_key`` as a static string at construction
    ...
    :mod:`agent.cem888ai_adapter`: ``build_cem888ai_client`` detects a
    callable and routes through ``_build_cem888ai_client_with_bearer_hook``,
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `cem888 model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``cem888 status`` that just want to know
...
def _cem888_device_code_login(
    *,
    ...
    """Run the CEM888.AI device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["cem888"]
    portal_base_url = (
        portal_base_url
        or os.getenv("CEM888_PORTAL_BASE_URL")
        or os.getenv("CEM888_PORTAL_BASE_URL")
    ...
    scope, explicit_scope = _cem888_device_scope_with_env_override(
        scope,
    ...
    print(f"Starting CEM888 login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        device_data, scope = _request_cem888_device_code_with_scope_fallback(
            client=client,
            portal_base_url=portal_base_url,
            client_id=client_id,
            scope=scope,
    ...
    try:
        return refresh_cem888_oauth_from_state(
            auth_state,
            min_key_ttl_seconds=min_key_ttl_seconds,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `cem888 model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_cem888(args, pconfig: ProviderConfig) -> None:
    """CEM888.AI Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing CEM888.AI credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_cem888_state()
        if shared:
            try:
                shared_path = _cem888_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing CEM888.AI OAuth credentials at {shared_path}")
            else:
                print("Found existing shared CEM888.AI OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating CEM888.AI session from shared credentials...")
                auth_state = _try_import_shared_cem888_state(
                    timeout_seconds=timeout_seconds,
        ...
        if auth_state is None:
            auth_state = _cem888_device_code_login(
        ...
        # overwrites it to "cem888".  If the user picks "Skip (keep current)"
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "cem888", auth_state)
            saved_to = _save_auth_store(auth_store)
        ...
        _write_shared_cem888_state(auth_state)
        _sync_cem888_pool_from_auth_store()
        ...
        # leave the config in a half-updated state (provider=cem888 but model
        ...
        try:
            ...
            from cem888_cli.models import (
                get_curated_cem888_model_ids, get_pricing_for_provider,
                check_cem888_free_tier, partition_cem888_models_by_tier,
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older CEM888 builds still see
            ...
            if model_ids:
                ...
                _url = (_portal or DEFAULT_CEM888_PORTAL_URL).rstrip("/")
        ...
        # them to CEM888.AI with a mismatched model.  The CEM888.AI OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "cem888".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. CEM888.AI credentials saved for future use.")
            print("  Run `cem888 model` again to switch to CEM888.AI Portal.")
            return

        config_path = _update_config_for_provider(
            "cem888", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=cem888)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("CEM888 will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `cem888 model` or configure an API key to use CEM888.")
        else:
