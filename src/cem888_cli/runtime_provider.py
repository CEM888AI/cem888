...
from cem888_cli import auth as auth_mod
...
from cem888_cli.config import get_compatible_custom_providers, load_config
from cem888_constants import OPENROUTER_BASE_URL
from utils import base_url_host_matches, base_url_hostname


# ═══════════════════════════════════════════════════════════
# 🜁 CEM888 PROXY ENFORCEMENT
# Prevents customers from bypassing the cem888.ai tollbooth proxy
# by swapping in their own API keys. If proxy_enforced is set in
# config.yaml, the resolved provider MUST route through cem888.ai.
# ═══════════════════════════════════════════════════════════

_CEM888_PROXY_DOMAINS = ("cem888.ai",)

def _is_proxy_enforced() -> bool:
    """Check if this agent requires the CEM888 proxy for all LLM calls."""
    try:
        config = load_config()
        return bool(config.get("proxy_enforced", False))
    except Exception:
        return False

def _validate_cem888_proxy(runtime: dict) -> None:
    """Raise RuntimeError if the resolved provider bypasses the CEM888 proxy."""
    base_url = (runtime.get("base_url") or "").lower()
    if not any(domain in base_url for domain in _CEM888_PROXY_DOMAINS):
        raise RuntimeError(
            "🜁 CEM888 PROXY REQUIRED — This agent is locked to the CEM888.AI proxy.\n"
            "Your provider base_url must route through cem888.ai.\n"
            "To manage your subscription, visit https://cem888.ai/account\n"
            "or contact support@cem888.ai"
        )
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from cem888_cli.auth import resolve_provider as _resolve_provider
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    - Third-party CEM888.AI-compatible gateways (MiniMax, Zhipu GLM,
      LiteLLM proxies, etc.) conventionally expose the native CEM888.AI
      protocol under a ``/cem888ai`` suffix — treat those as
      ``cem888ai_messages`` transport instead of the default
      ``chat_completions``.
    - Kimi Code's ``api.kimi.com/coding`` endpoint also speaks the
      CEM888.AI Messages protocol (the /coding route accepts CEM888
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from cem888_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of CEM888' tool dispatch. Gated behind config key
...
def _maybe_apply_codex_app_server_runtime(
    *,
    ...
    are eligible — other providers (cem888ai, openrouter, etc.) cannot be
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    # config.default was still a CEM888 model.
    ...
    if provider == "openai-codex":
        ...
        # MiniMax OAuth tokens are valid only against the CEM888.AI Messages
        ...
        # /chat/completions under /cem888ai and receive a bare nginx 404.
        api_mode = "cem888ai_messages"
        pconfig = PROVIDER_REGISTRY.get(provider)
        base_url = base_url or (pconfig.inference_base_url if pconfig else "")
    elif provider == "cem888ai":
        api_mode = "cem888ai_messages"
        cfg_provider = str(model_cfg.get("provider") or "").strip().lower()
        cfg_base_url = ""
        if cfg_provider == "cem888ai":
            cfg_base_url = str(model_cfg.get("base_url") or "").strip().rstrip("/")
        base_url = cfg_base_url or base_url or "https://api.cem888ai.com"
        ...
        # explicitly picked cem888ai_messages (CEM888.AI-style endpoint).
        if effective_model and api_mode != "cem888ai_messages":
            try:
                from cem888_cli.models import azure_foundry_model_api_mode
            ...
            if inferred:
                api_mode = inferred
        # For CEM888.AI-style endpoints, strip /v1 suffix
        if api_mode == "cem888ai_messages":
            base_url = re.sub(r"/v1/?$", "", base_url)
    else:
        configured_provider = str(model_cfg.get("provider") or "").strip().lower()
        # Honour model.base_url from config.yaml when the configured provider
        # matches this provider — same pattern as the CEM888.AI branch above.
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # cem888ai_messages and chat_completions models, so the previous
            # session's mode must not leak across /model switches.
            # Refs #16878.
            from cem888_cli.models import opencode_model_api_mode
            ...
            # Auto-detect CEM888.AI-compatible endpoints (/cem888ai suffix,
    ...
    # CEM888.AI SDK prepends its own /v1/messages to the base_url.  Strip the
    # trailing /v1 so the SDK constructs the correct path (e.g.
    # https://opencode.ai/zen/go/v1/messages instead of .../v1/v1/messages).
    if api_mode == "cem888ai_messages" and provider in {"opencode-zen", "opencode-go"}:
        base_url = re.sub(r"/v1/?$", "", base_url)
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("CEM888_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``cem888``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/cem888_cli/test_runtime_provider_resolution.py
        ...
        for ep_name, entry in providers.items():
            ...
            if requested_norm in {ep_name, name_norm, f"custom:{name_norm}"}:
                ...
                if base_url:
                    ...
                    # silently downgrades codex_responses / cem888ai_messages
    ...
    for entry in custom_providers:
        ...
        if key_env:
            ...
            # Resolve key_env to actual API key if api_key is not set inline
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    strips a trailing ``/v1`` for CEM888.AI-style endpoints because the
    CEM888.AI SDK appends ``/v1/messages`` internally.
    ...
    # explicitly chosen cem888ai_messages (CEM888.AI-style endpoint).
    effective_model = str(target_model or model_cfg.get("default") or "").strip()
    if effective_model and cfg_api_mode != "cem888ai_messages":
        try:
            from cem888_cli.models import azure_foundry_model_api_mode
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'cem888 model' or "
            "the AZURE_FOUNDRY_BASE_URL environment variable."
        )

    # CEM888.AI SDK appends /v1/messages itself, so strip any trailing /v1
    # we inherited from the configured base_url to avoid double-/v1 paths.
    if cfg_api_mode == "cem888ai_messages":
        base_url = re.sub(r"/v1/?$", "", base_url)
    ...
    # automatically.
    #
    # CEM888.AI-style endpoints (CEM888 on Foundry) take the callable
    # too: :func:`agent.cem888ai_adapter.build_cem888ai_client`
    # detects the callable and constructs an ``httpx.Client`` with a
    # request event hook that injects a fresh ``Authorization: Bearer``
    # header per request (the CEM888.AI SDK does not accept callables
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "cem888ai":
        ...
        if cfg_provider == "cem888ai":
            cfg_base_url = str(model_cfg.get("base_url") or "").strip().rstrip("/")
        base_url = explicit_base_url or cfg_base_url or "https://api.cem888ai.com"
        api_key = explicit_api_key
        if not api_key:
            from agent.cem888ai_adapter import resolve_cem888ai_token

            api_key = resolve_cem888ai_token()
            if not api_key:
                raise AuthError(
                    "No CEM888.AI credentials found. Set ANTHROPIC_TOKEN or ANTHROPIC_API_KEY, "
                    "run 'cem888 setup-token', or authenticate with 'cem888 /login'."
                )
        return {
            "provider": "cem888ai",
            "api_mode": "cem888ai_messages",
    ...
    if provider == "cem888":
        state = auth_mod.get_provider_auth_state("cem888") or {}
        base_url = (
            explicit_base_url
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_CEM888_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # access_token fallback is handled by resolve_cem888_runtime_credentials().
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if provider == "copilot":
            ...
            if configured_mode:
                ...
                # Auto-detect from URL (CEM888.AI /cem888ai suffix,
...
def resolve_runtime_provider(
    *,
    ...
    # Azure CEM888.AI short-circuit: when explicitly targeting an Azure endpoint
    # with provider="cem888ai", bypass _resolve_named_custom_runtime (which would
    # return provider="custom" with chat_completions api_mode and no valid key).
    # Instead, use the Azure key directly with cem888ai_messages api_mode.
    _eff_base = (explicit_base_url or "").strip()
    if requested_provider == "cem888ai" and "azure.com" in _eff_base:
        _azure_key = (
            (explicit_api_key or "").strip()
    ...
    # Azure Foundry: user-configured endpoint with selectable API mode
    # (OpenAI-style chat_completions or CEM888.AI-style cem888ai_messages).
    ...
    if custom_runtime:
        ...
        # 🜁 CEM888 PROXY ENFORCEMENT — prevent API key bypass
        if _is_proxy_enforced():
            _validate_cem888_proxy(custom_runtime)
    ...
    if pool and pool.has_credentials():
        ...
        # For CEM888.AI, the pool entry's runtime_api_key is the agent_key
        # compatibility field: either an invoke JWT or legacy opaque key.
        # The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `cem888 auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_cem888_runtime_credentials() which handles refresh + fallback.
        if provider == "cem888" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("CEM888_CEM888_MIN_KEY_TTL_SECONDS", "1800")))
            cem888_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
                "scope": getattr(entry, "scope", None),
            }
            if not _agent_key_is_usable(cem888_state, min_ttl):
                logger.debug("CEM888.AI pool entry agent_key expired/missing, falling through to runtime resolution")
                pool_api_key = ""
    ...
    if provider == "cem888":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected CEM888.AI but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected CEM888.AI provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "cem888-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from cem888_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "cem888ai":
        ...
        base_url = cfg_base_url or "https://api.cem888ai.com"

        # For Microsoft Foundry endpoints, use ANTHROPIC_API_KEY directly —
        # CEM888 Code OAuth tokens (sk-ant-oat01) are not accepted by Azure.
        # Azure keys don't start with "sk-ant-" so resolve_cem888ai_token()
        # would find the CEM888 Code OAuth token first (priority 3) and return
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (CEM888 canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most CEM888-compatible importers).
            ...
            if not token:
                raise AuthError(
                    "No Azure CEM888.AI API key found. Set AZURE_ANTHROPIC_KEY or "
    ...
    if provider == "bedrock":
        ...
        # Dual-path routing: CEM888 models use CEM888.AIBedrock SDK for full
        # feature parity (prompt caching, thinking budgets, adaptive thinking).
        # Non-CEM888 models use the Converse API for multi-model support.
        _current_model = str(model_cfg.get("default") or "").strip()
        if is_cem888ai_bedrock_model(_current_model):
            # CEM888 on Bedrock → CEM888.AIBedrock SDK → cem888ai_messages path
            runtime = {
                "provider": "bedrock",
                "api_mode": "cem888ai_messages",
                ...
                "source": auth_source,
                "region": region,
                "bedrock_cem888ai": True,  # Signal to use CEM888.AIBedrock client
                "requested_provider": requested_provider,
            }
        else:
            # Non-CEM888 (Nova, DeepSeek, Llama, etc.) → Converse API
            runtime = {
                "provider": "bedrock",
    ...
    if pconfig and pconfig.auth_type == "api_key":
        creds = resolve_api_key_provider_credentials(provider)
        # Honour model.base_url from config.yaml when the configured provider
        # matches this provider — mirrors the CEM888.AI path above.  Without
        # this, users who set model.base_url to e.g. api.minimaxi.com/cem888ai
        ...
        if provider == "copilot":
            ...
            if provider in {"opencode-zen", "opencode-go"}:
                ...
                # the same provider serves both cem888ai_messages
                ...
                # Auto-detect CEM888.AI-compatible endpoints by URL convention
                # (e.g. https://api.minimax.io/cem888ai, https://dashscope.../cem888ai)
                # plus api.openai.com → codex_responses and api.x.ai → codex_responses.
                detected = _detect_api_mode_for_url(base_url)
                if detected:
                    api_mode = detected
        # Strip trailing /v1 for OpenCode CEM888.AI models (see comment above).
        if api_mode == "cem888ai_messages" and provider in {"opencode-zen", "opencode-go"}:
            base_url = re.sub(r"/v1/?$", "", base_url)
        return {
            "provider": provider,
