...
``<root>`` is the **shared CEM888 root** (the parent of any active
...
``cem888 -p <profile>`` joins the same board as the dispatcher that
...
the profile's CEM888 home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``CEM888_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``CEM888_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``cem888 kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.cem888``. In Docker / custom
deployments where ``CEM888_HOME`` points outside ``~/.cem888`` (e.g.
``/opt/cem888``), ``<root>`` is ``CEM888_HOME``. Legacy env-var
overrides still work:

* ``CEM888_KANBAN_DB`` — pin the database file path directly.
* ``CEM888_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``CEM888_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``CEM888_KANBAN_DB``,
``CEM888_KANBAN_WORKSPACES_ROOT``, and ``CEM888_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/cem888-kanban-v1-spec.pdf`` for the full
...
# ``CEM888_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``CEM888_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("CEM888_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``CEM888_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("CEM888_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``cem888-agent``
...
def kanban_home() -> Path:
    """Return the shared CEM888 root that anchors the kanban board.

    Resolution order:

    1. ``CEM888_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_cem888_root()``, which already returns ``<root>``
       when ``CEM888_HOME`` is ``<root>/profiles/<name>``, and returns
       ``CEM888_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``CEM888_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("CEM888_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from cem888_constants import get_default_cem888_root
    return get_default_cem888_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``cem888 kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``CEM888_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``cem888 kanban boards
    ...
    """
    env = os.environ.get("CEM888_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``cem888 kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``CEM888_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("CEM888_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``CEM888_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("CEM888_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``cem888 kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``CEM888_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``CEM888_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("CEM888_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources CEM888 treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``CEM888_KANBAN_DB`` env → ``CEM888_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See cem888_state._WAL_INCOMPAT_MARKERS for detection logic.
                from cem888_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``cem888 kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``CEM888_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from cem888_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``cem888 --skills ...`` alongside the built-in
    ...
    # `cem888 --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``cem888 kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `cem888 kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``cem888 kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``cem888 kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``cem888 kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``cem888 kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``CEM888_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold CEM888' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside CEM888-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``cem888 kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a CEM888 Code terminal like ``orion-cc``) rather than a CEM888
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``cem888 kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real CEM888 profile.
    ...
    if not rows:
        return False
    try:
        from cem888_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real CEM888 profile.
        # `_default_spawn` invokes ``cem888 -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive CEM888 Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a CEM888
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `cem888 kanban tail` — without
...
def _module_cem888_argv() -> list[str]:
    """Return the interpreter-bound CEM888 CLI invocation."""
    # ``cem888_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``cem888`` package — there is no
    # ``cem888`` package to import.
    return [sys.executable, "-m", "cem888_cli.main"]


def _absolute_cem888_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved CEM888 shim."""
...
def _cem888_path_argv(path: str) -> list[str]:
    """Return argv for a resolved CEM888 executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_cem888_argv()
    return [_absolute_cem888_path(path)]


def _resolve_cem888_argv() -> list[str]:
    """Resolve the ``cem888`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$CEM888_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("cem888")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\cem888.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m cem888_cli.main`` — fallback for setups where
       CEM888 is launched from a venv and the ``cem888`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_cem888_bin`` for the same reason. Kept
    local (not imported from gateway) because ``cem888_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("CEM888_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _cem888_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _cem888_path_argv(resolved_env_bin)
        return _module_cem888_argv()

    cem888_bin = _safe_which_no_cwd("cem888") if _IS_WINDOWS else shutil.which("cem888")
    if cem888_bin:
        return _cem888_path_argv(cem888_bin)
    return _module_cem888_argv()


def _kanban_worker_skill_available(cem888_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``cem888 -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset CEM888_HOME means the worker falls back to the default root
    # home (``~/.cem888``), which ships the bundled skill.
    base = _Path(cem888_home) if cem888_home else (_Path.home() / ".cem888")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``cem888 -p <profile> chat -q ...`` subprocess.
    ...
    ``CEM888_KANBAN_DB`` / ``CEM888_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject CEM888_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `cem888 -p <name>` the
    # _apply_profile_override() runs *before* cem888_constants is imported.
    # If CEM888_HOME is absent from the child's env, get_cem888_home() falls
    # back to Path.home() / ".cem888" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from cem888_cli.profiles import resolve_profile_env
    try:
        env["CEM888_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via CEM888_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # CEM888_HOME never had profiles created.
        pass
    if task.tenant:
        env["CEM888_TENANT"] = task.tenant
    env["CEM888_KANBAN_TASK"] = task.id
    env["CEM888_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["CEM888_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["CEM888_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["CEM888_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`cem888 -p <name>`
    # rewrites CEM888_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_cem888_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["CEM888_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["CEM888_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["CEM888_KANBAN_BOARD"] = resolved_board
    # CEM888_PROFILE is the author the kanban_comment tool defaults to.
    # `cem888 -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["CEM888_PROFILE"] = profile_arg

    cmd = [
        *_resolve_cem888_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped CEM888_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("CEM888_HOME")):
    ...
    # `cem888 kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`cem888` executable not found on PATH. "
            "Install CEM888 Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``cem888 kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    # the user to wire anything into SOUL.md or the memory store. Bounded to the
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled CEM888_PROFILE values like "cem888-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default CEM888 root exists

    Reads profile paths directly so this module has no import dependency on
    ``cem888_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from cem888_constants import get_default_cem888_root
        default_root = get_default_cem888_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``cem888 kanban assignees`` for the terminal.
