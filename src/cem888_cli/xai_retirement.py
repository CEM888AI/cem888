...
Pure logic: walks a CEM888 config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `cem888 doctor` and a future `cem888 migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a CEM888 config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a CEM888 config and return retirement issues.
