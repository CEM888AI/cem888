...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or cem888 -c.",
    ...
    "Ctrl+Z suspends CEM888 to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "cem888 -c resumes your most recent CLI session. cem888 -c \"project name\" resumes by title.",
    "cem888 -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "cem888 -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "cem888 chat -t web,terminal enables only specific toolsets for a focused session.",
    "cem888 chat -s github-pr-workflow preloads a skill at launch.",
    "cem888 chat -q \"query\" runs a single non-interactive query and exits.",
    "cem888 chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "cem888 chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "cem888 --yolo bypasses all dangerous command approval prompts for the entire session.",
    "cem888 chat --source telegram tags the session for filtering in cem888 sessions list.",
    "cem888 -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "cem888 doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "cem888 dump outputs a compact setup summary — great for bug reports.",
    "cem888 config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "cem888 config edit opens config.yaml in your default editor.",
    "cem888 config check scans for missing or stale configuration options.",
    "cem888 sessions browse opens an interactive session picker with search.",
    "cem888 sessions stats shows session counts by platform and database size.",
    "cem888 sessions prune --older-than 30 cleans up old sessions.",
    "cem888 skills search react --source skills-sh searches the skills.sh public directory.",
    "cem888 skills check scans installed hub skills for upstream updates.",
    "cem888 skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "cem888 skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "cem888 mcp add github --command npx adds MCP servers from the command line.",
    "cem888 mcp serve runs CEM888 itself as an MCP server for other agents.",
    "cem888 auth add lets you add multiple API keys for credential pool rotation.",
    "cem888 completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "cem888 logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "cem888 backup creates a zip backup of your entire CEM888 home directory.",
    "cem888 profile create coder creates an isolated profile that becomes its own command.",
    "cem888 profile create work --clone copies your current config and keys to a new profile.",
    "cem888 update syncs new bundled skills to ALL profiles automatically.",
    "cem888 gateway install sets up CEM888 as a system service (systemd/launchd).",
    "cem888 memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "cem888 webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: cem888 tools disables unused tools, cem888 skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "cem888 models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as cem888 -w).",
    ...
    "CEM888_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call CEM888 tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'cem888 profile create coder' creates the 'coder' command.",
    "cem888 profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, CEM888 prints a resume command with session ID and stats.",
    "cem888 sessions export backup.jsonl exports all sessions for backup or analysis.",
    "cem888 -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "cem888 skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .cem888/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: cem888 cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "CEM888 runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "cem888 gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to CEM888 via a local macOS server.",
    ...
    "Context files (.cem888.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.cem888/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "CEM888 loads project context from .cem888.md, AGENTS.md, CEM888.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "cem888 mcp opens an interactive picker of CEM888.AI-approved MCPs you can install in one keystroke.",
    "cem888 mcp catalog lists CEM888.AI-approved MCP servers shipped with the repo.",
    "cem888 mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — cem888 tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.cem888/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "cem888 chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "cem888 plugins install owner/repo installs plugins directly from GitHub.",
    ...
    # --- Miscellaneous ---
    "Prompt caching (CEM888.AI) reduces costs by reusing cached system prompt prefixes.",
    ...
    "cem888 acp runs CEM888 as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "CEM888_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "cem888 auth add nous or cem888 auth add openai-codex sets up OAuth-based providers.",
    ...
    "cem888 status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.cem888/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make CEM888 your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.cem888/SOUL.md to customize.",
    ...
    "Before a gateway session reset, CEM888 auto-flushes important facts to memory in the background.",
    ...
    "delegate_task with acp_command: 'cem888' spawns CEM888 Code as a child agent from any platform.",
    ...
    "The gateway caches AIAgent instances per session — destroying this cache breaks CEM888.AI prompt caching.",
    "Any website can expose skills via /.well-known/skills/index.json — the skills hub discovers them automatically.",
    "The skills audit log at ~/.cem888/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at CEM888_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "CEM888_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CEM888_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.cem888/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'cem888 profile create ls' would be rejected.",
    "cem888 profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "cem888 claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "CEM888_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via cem888 tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.cem888/.env.",
    "Skills from trusted repos (CEM888.AI) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.cem888/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — CEM888 auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of CEM888 config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while CEM888 is working.',
    ...
    '/reload reloads ~/.cem888/.env into the running session — pick up new API keys without restarting.',
    ...
    "CEM888_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.cem888/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.cem888/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'cem888 curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "cem888 curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'cem888 curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'cem888 auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your CEM888.AI subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'CEM888_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CEM888_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'cem888 dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'cem888 dashboard --tui embeds the full CEM888 TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.cem888/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.cem888/dashboard-plugins/ — no npm build required.',
    ...
    'CEM888_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'CEM888_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'CEM888_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CEM888_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.cem888/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.cem888/skills/ so drop-in skills appear without restarting the session.',
    ...
    'cem888 -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'cem888 chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'cem888 chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'cem888 chat --ignore-user-config skips ~/.cem888/config.yaml — reproducible bug reports and CI runs.',
    "cem888 chat --source tool tags programmatic chats so they don't clutter cem888 sessions list.",
    'cem888 dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'cem888 sessions rename <ID> "new title" renames any past session; cem888 sessions delete <ID> removes one.',
    'cem888 import restores a session export or profile archive produced by sessions export or profile export.',
    'cem888 fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'cem888 pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'cem888 setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'cem888 status --deep runs the full health sweep across every component; plain cem888 status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CEM888_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CEM888_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.cem888/plugins/ — trust-gated by design.',
    "CEM888_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CEM888_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CEM888_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CEM888_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CEM888_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CEM888_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CEM888_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CEM888_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CEM888_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CEM888_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CEM888_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes CEM888 block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from cem888 sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.cem888/dashboard-plugins/.',
]
