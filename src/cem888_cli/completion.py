"""Shell completion script generation for cem888 CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_cem888_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# CEM888 Agent bash completion
# Add to ~/.bashrc:
#   eval "$(cem888 completion bash)"

_cem888_profiles() {{
    local profiles_dir="$HOME/.cem888/profiles"
...
_cem888_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_cem888_profiles)" -- "$cur"))
        return
    fi
...
complete -F _cem888_completion cem888
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef cem888
# CEM888 Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(cem888 completion zsh)"

_cem888_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.cem888/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.cem888/profiles 2>/dev/null)}}")
    fi
...
_cem888() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_cem888_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'cem888 command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# CEM888 Agent fish completion",
        "# Add to your config:",
        "#   cem888 completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __cem888_profiles",
        "    echo default",
        "    if test -d $HOME/.cem888/profiles",
        "        ls $HOME/.cem888/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c cem888 -f -s p -l profile"
        " -d 'Profile name' -xa '(__cem888_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__cem888_profiles)' -d 'Profile name'"
                )
