"""cem888 webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    cem888 webhook subscribe <name> [options]
    cem888 webhook list
    cem888 webhook remove <name>
    cem888 webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.cem888/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
    _dhh = display_cem888_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     cem888 gateway setup
  ...
  Then start the gateway: cem888 gateway run
"""
...
def webhook_command(args):
    """Entry point for 'cem888 webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: cem888 webhook {subscribe|list|remove|test}")
        print("Run 'cem888 webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (cem888 gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: cem888 webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from cem888 webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (cem888 gateway run)")
