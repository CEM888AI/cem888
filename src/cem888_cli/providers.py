"""
Single source of truth for provider identity in CEM888 Agent.
...
2. **CEM888 overlays** — transport type, auth patterns, aggregator flags,
...
# -- CEM888 overlay ----------------------------------------------------------
# CEM888-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class CEM888Overlay:
    """CEM888-specific provider metadata layered on top of models.dev."""

    transport: str = "openai_chat"        # openai_chat | cem888ai_messages | codex_responses
...
CEM888_OVERLAYS: Dict[str, CEM888Overlay] = {
    "openrouter": CEM888Overlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "cem888": CEM888Overlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://api.cem888ai.com/v1",
    ),
    "openai-codex": CEM888Overlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": CEM888Overlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": CEM888Overlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": CEM888Overlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="CEM888_QWEN_BASE_URL",
    ),
    "google-gemini-cli": CEM888Overlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": CEM888Overlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": CEM888Overlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": CEM888Overlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "cem888ai": CEM888Overlay(
        transport="cem888ai_messages",
        extra_env_vars=("ANTHROPIC_TOKEN", "CEM888_OAUTH_TOKEN"),
    ),
    "zai": CEM888Overlay(
        transport="openai_chat",
    ...
    "kimi-for-coding": CEM888Overlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": CEM888Overlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": CEM888Overlay(
        transport="cem888ai_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": CEM888Overlay(
        transport="cem888ai_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/cem888ai",
    ),
    "minimax-cn": CEM888Overlay(
        transport="cem888ai_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": CEM888Overlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": CEM888Overlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": CEM888Overlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": CEM888Overlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": CEM888Overlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": CEM888Overlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": CEM888Overlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": CEM888Overlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": CEM888Overlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": CEM888Overlay(
        transport="openai_chat",
    ...
    "ollama-cloud": CEM888Overlay(
        transport="openai_chat",
        base_url_override="https://ollama.com/v1",
        base_url_env_var="OLLAMA_BASE_URL",
    ),
    # Azure Foundry: supports both OpenAI-style and CEM888.AI-style endpoints.
    # The transport is determined at runtime from config.yaml model.api_mode.
    "azure-foundry": CEM888Overlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": CEM888Overlay(
...
class ProviderDef:
    ...
    id: str
    name: str
    transport: str                        # openai_chat | cem888ai_messages | codex_responses
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "cem888", "user-config"
...
TRANSPORT_TO_API_MODE: Dict[str, str] = {
    ...
    "cem888ai_messages": "cem888ai_messages",
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. CEM888 overlays (for providers not in models.dev: cem888, openai-codex, etc.)
      2. models.dev catalog + CEM888 overlay
    ...
    overlay = CEM888_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + cem888 extra
    ...
    if overlay is not None:
        # CEM888-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    if pdef is not None:
        ...
        # (e.g. kimi /coding endpoint needs cem888ai_messages even on 'custom')
        if base_url:
            url_lower = base_url.rstrip("/").lower()
            if "api.kimi.com/coding" in url_lower:
                return "cem888ai_messages"
            if url_lower.endswith("/cem888ai") or "api.cem888ai.com" in url_lower:
                return "cem888ai_messages"
            if "api.openai.com" in url_lower:
                return "codex_responses"
        return TRANSPORT_TO_API_MODE.get(pdef.transport, "chat_completions")

    # Direct provider checks for providers not in CEM888_OVERLAYS
    if provider == "bedrock":
    ...
    if base_url:
        ...
        if url_lower.endswith("/cem888ai") or hostname == "api.cem888ai.com":
            return "cem888ai_messages"
