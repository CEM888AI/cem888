"""
Dump command for cem888 CLI.

Outputs a compact, plain-text summary of the user's CEM888 setup
...
from cem888_cli.config import get_cem888_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.cem888_build_sha`` by the Dockerfile's
    ``CEM888_GIT_SHA`` build-arg (see ``cem888_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``cem888 dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from cem888_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(cem888_home: Path) -> int:
...
def _cron_summary(cem888_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = cem888_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"cem888_home:      {display_cem888_home()}")
    ...
    api_keys = [
        ...
        ("ANTHROPIC_API_KEY", "cem888ai"),
        ("ANTHROPIC_TOKEN", "cem888ai_token"),
        ("NOUS_API_KEY", "cem888"),
    ...
    toolsets = config.get("toolsets", ["cem888-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(cem888_home)}")
    lines.append(f"  skills:             {_count_skills(cem888_home)}")
