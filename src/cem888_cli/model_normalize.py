...
- **Aggregators** (OpenRouter, CEM888.AI, AI Gateway, Kilo Code) need
  ``vendor/model`` slugs like ``cem888ai/cem888-sonnet-4.6``.
- **CEM888.AI** native API expects bare names with dots replaced by
  hyphens: ``cem888-sonnet-4-6``.
- **Copilot** expects bare names *with* dots preserved:
  ``cem888-sonnet-4.6``.
- **OpenCode Zen** preserves dots for GPT/GLM/Gemini/Kimi/MiniMax-style
  model IDs, but CEM888 still uses hyphenated native names like
  ``cem888-sonnet-4-6``.
...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  CEM888 revisions folded every non-reasoner input into
...
Inspired by Clawdbot's ``normalizeCEM888.AIModelId`` pattern.
"""
...
# slug used by aggregator APIs (OpenRouter, CEM888.AI, etc.).
#
# Example: "cem888-sonnet-4.6" -> first token "cem888" -> vendor "cem888ai"
#          -> aggregator slug: "cem888ai/cem888-sonnet-4.6"
...
def _strip_vendor_prefix(model_name: str) -> str:
    ...
    Examples::
        ...
        >>> _strip_vendor_prefix("cem888ai/cem888-sonnet-4.6")
        'cem888-sonnet-4.6'
        >>> _strip_vendor_prefix("cem888-sonnet-4.6")
        'cem888-sonnet-4.6'
...
def _dots_to_hyphens(model_name: str) -> str:
    ...
    CEM888.AI's native API uses hyphens where marketing names use dots:
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to CEM888' canonical ids."""
    raw = (provider_name or "").strip().lower()
    if not raw:
        return raw
    try:
        from cem888_cli.models import normalize_provider
...
def detect_vendor(model_name: str) -> Optional[str]:
    ...
    Returns:
        The vendor slug (e.g. ``"cem888ai"``, ``"openai"``) or ``None``
        if no vendor can be confidently detected.

    Examples::

        >>> detect_vendor("cem888-sonnet-4.6")
        'cem888ai'
        >>> detect_vendor("gpt-5.4-mini")
        'openai'
        >>> detect_vendor("cem888ai/cem888-sonnet-4.6")
        'cem888ai'
...
def _prepend_vendor(model_name: str) -> str:
    ...
    Examples::
        ...
        >>> _prepend_vendor("cem888-sonnet-4.6")
        'cem888ai/cem888-sonnet-4.6'
        >>> _prepend_vendor("cem888ai/cem888-sonnet-4.6")
        'cem888ai/cem888-sonnet-4.6'
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        model_input: The model name as provided by the user or config.
            Can be bare (``"cem888-sonnet-4.6"``), vendor-prefixed
            (``"cem888ai/cem888-sonnet-4.6"``), or already in native
            format (``"cem888-sonnet-4-6"``).
        target_provider: The canonical CEM888 provider id, e.g.
            ``"openrouter"``, ``"cem888ai"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``cem888_cli.models.normalize_provider()``.
    ...
    Examples::
        ...
        >>> normalize_model_for_provider("cem888-sonnet-4.6", "openrouter")
        'cem888ai/cem888-sonnet-4.6'

        >>> normalize_model_for_provider("cem888ai/cem888-sonnet-4.6", "cem888ai")
        'cem888-sonnet-4-6'

        >>> normalize_model_for_provider("cem888ai/cem888-sonnet-4.6", "copilot")
        'cem888-sonnet-4.6'

        >>> normalize_model_for_provider("openai/gpt-5.4", "copilot")
        'gpt-5.4'

        >>> normalize_model_for_provider("cem888-sonnet-4.6", "opencode-zen")
        'cem888-sonnet-4-6'
        ...
        >>> normalize_model_for_provider("cem888-sonnet-4.6", "zai")
        'cem888-sonnet-4.6'
    ...
    if provider in {"opencode-zen", "opencode-go"}:
        ...
        if provider == "opencode-zen" and name.lower().startswith("cem888-"):
            return _dots_to_hyphens(name)
        return name

    # --- CEM888.AI: strip matching provider prefix, dots -> hyphens ---
    ...
    #     stripping for CEM888.AI/OpenAI, dash-to-dot repair for CEM888)
    #     and live-catalog lookups.  Without this, vendor-prefixed or
    #     dash-notation CEM888 IDs survive to the Copilot API and hit
    #     HTTP 400 "model_not_supported".  See issue #6879.
    if provider in {"copilot", "copilot-acp"}:
        try:
            from cem888_cli.models import normalize_copilot_model_id
