"""Helpers for CEM888.AI subscription managed-tool capabilities."""
...
from cem888_cli.config import get_env_value, load_config
from cem888_cli.cem888_account import CEM888PortalAccountInfo, get_portal_account_info
...
class CEM888FeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_cem888: bool
...
@dataclass(frozen=True)
class CEM888SubscriptionFeatures:
    subscribed: bool
    cem888_auth_present: bool
    provider_is_cem888: bool
    features: Dict[str, CEM888FeatureState]
    account_info: Optional[CEM888PortalAccountInfo] = None

    @property
    def web(self) -> CEM888FeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> CEM888FeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> CEM888FeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> CEM888FeatureState:
        return self.features["browser"]

    @property
    def modal(self) -> CEM888FeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[CEM888FeatureState]:
...
def get_cem888_subscription_features(
    config: Optional[Dict[str, object]] = None,
    *,
    force_fresh: bool = False,
) -> CEM888SubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_cem888 = str(model_cfg.get("provider") or "").strip().lower() == "cem888"
    ...
    cem888_auth_present = bool(account_info and account_info.logged_in)
    subscribed = provider_is_cem888 or cem888_auth_present
    ...
    # Tool Gateway via `cem888 model`, so direct credentials should NOT
    ...
    managed_web_available = managed_tools_flag and cem888_auth_present and is_managed_tool_gateway_ready("firecrawl")
    managed_image_available = managed_tools_flag and cem888_auth_present and is_managed_tool_gateway_ready("fal-queue")
    managed_tts_available = managed_tools_flag and cem888_auth_present and is_managed_tool_gateway_ready("openai-audio")
    managed_browser_available = managed_tools_flag and cem888_auth_present and is_managed_tool_gateway_ready("browser-use")
    managed_modal_available = managed_tools_flag and cem888_auth_present and is_managed_tool_gateway_ready("modal")
    ...
    features = {
        "web": CEM888FeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_cem888=web_managed,
        ...
        "image_gen": CEM888FeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_cem888=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("CEM888.AI Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "tts": CEM888FeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_cem888=tts_managed,
        ...
        "browser": CEM888FeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_cem888=browser_managed,
        ...
        "modal": CEM888FeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_cem888=modal_managed,
    ...
    return CEM888SubscriptionFeatures(
        subscribed=subscribed,
        cem888_auth_present=cem888_auth_present,
        provider_is_cem888=provider_is_cem888,
        features=features,
...
def apply_cem888_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_cem888:
        return set()
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid CEM888.AI subscriber or
    is not using CEM888.AI as their provider.
    """
    if force_fresh:
        managed_enabled = managed_cem888_tools_enabled(force_fresh=True)
    else:
        managed_enabled = managed_cem888_tools_enabled()
    ...
    # Quick provider check without the heavy get_cem888_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "cem888":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_cem888 which fires implicitly when
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from cem888_cli.setup import prompt_choice
    except Exception:
        return set()
    ...
    desc_parts: list[str] = [
        ...
        "  text-to-speech, and browser automation through your CEM888.AI subscription.",
    ...
    try:
        idx = prompt_choice(
            "Your CEM888.AI subscription includes the Tool Gateway.",
            choices,
            default_idx,
    ...
    if changed:
        ...
        for key in sorted(newly_switched):
            ...
            print(f"  ✓ {label}: enabled via CEM888.AI subscription")
