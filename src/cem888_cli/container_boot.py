...
``$CEM888_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-cem888-setup
(the stage2 hook) has chowned the volume and seeded $CEM888_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$CEM888_HOME``,
    not under ``profiles/``). The dispatcher in ``cem888_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``cem888 gateway start`` (no ``-p``) targets. Without it,
    bare ``cem888 gateway start`` inside the container would land on
    ...
    ``$CEM888_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        cem888_home: The container's CEM888_HOME (typically /opt/data).
            Profiles live under ``<cem888_home>/profiles/<name>/``;
            the default profile lives at ``<cem888_home>`` itself.
    ...
    # ``cem888 gateway start`` (no ``-p``) has somewhere to land;
    # auto-up only when the prior state was "running" (same rule as
    # named profiles).
    default_prior_state = _read_prior_state(cem888_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(cem888_home)
    ...
    profiles_root = cem888_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `cem888 profile create` (config.yaml
            # is not — that comes later via `cem888 setup`). Use it as the
            ...
            # via ``cem888 -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(cem888_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `cem888 -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit cem888
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the cem888 user) won't hit EACCES. See
...
def _write_reconcile_log(
    cem888_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $CEM888_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    cem888_home = Path(os.environ.get("CEM888_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        cem888_home=cem888_home, scandir=scandir,
    )
    for a in actions:
        print(
