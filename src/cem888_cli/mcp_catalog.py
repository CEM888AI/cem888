"""MCP catalog — curated, CEM888.AI-approved MCP servers shipped with the repo.
...
entries via ``cem888 mcp catalog`` or the interactive ``cem888 mcp picker``,
and install them with ``cem888 mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into cem888-agent. Presence in the
  ``optional-mcps/`` directory = CEM888.AI approval. No community tier, no trust
  signals beyond "it's in the catalog".
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``cem888 mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.cem888/.env`` (the
...
from cem888_constants import get_cem888_home, get_optional_mcps_dir
...
from cem888_cli.cli_output import prompt as _prompt_input, prompt_yes_no
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this CEM888 install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this CEM888 understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their CEM888 is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this CEM888
        understands. Update CEM888 to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_cem888_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.cem888/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.cem888/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from cem888_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``cem888 mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `cem888 mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `cem888 mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `cem888 mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `cem888 mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `cem888 auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`cem888 auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new CEM888 session to load its tools.",
        Colors.GREEN,
    ))
