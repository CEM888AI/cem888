"""Implementation of the ``cem888 bundles`` CLI subcommand.

Mirrors the structure of ``cem888_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `cem888 bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  cem888 bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``cem888 bundles`` argparse tree.

    Called from ``cem888_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `cem888 bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``cem888 bundles <subcommand>`` to the right handler."""
