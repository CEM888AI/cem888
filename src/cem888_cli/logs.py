"""``cem888 logs`` — view and filter CEM888 log files.
...
Usage examples::
    ...
    cem888 logs                    # last 50 lines of agent.log
    cem888 logs -f                 # follow agent.log in real time
    cem888 logs errors             # last 50 lines of errors.log
    cem888 logs gateway -n 100    # last 100 lines of gateway.log
    cem888 logs --level WARNING    # only WARNING+ lines
    cem888 logs --session abc123   # filter by session ID substring
    cem888 logs --component tools  # only tool-related lines
    cem888 logs --since 1h         # lines from the last hour
    cem888 logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_cem888_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when CEM888 runs — try 'cem888 chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_cem888_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_cem888_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_cem888_home()}/logs/")
        return

    print(f"Log files in {display_cem888_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'cem888 chat' to generate logs)")
