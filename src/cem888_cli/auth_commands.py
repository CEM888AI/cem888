...
_OAUTH_CAPABLE_PROVIDERS = {"cem888ai", "cem888", "openai-codex", "xai-oauth", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    # (shell-exported vars), gh_cli (copilot), cem888_code, qwen-cli,
    ...
    if provider == "cem888ai":
        from agent import cem888ai_adapter as cem888ai_mod

        creds = cem888ai_mod.run_cem888_oauth_login_pure()
        if not creds:
            raise SystemExit("CEM888.AI OAuth login did not return credentials.")
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:cem888_pkce",
    ...
    if provider == "cem888":
        # Codex-style auto-import: if a shared CEM888.AI credential lives at
        # <cem888-root>/shared/cem888_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `cem888 --profile <name>
        # auth add cem888 --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_cem888_state()
        if shared:
            try:
                path = auth_mod._cem888_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing CEM888.AI OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_cem888_state(
                ...
                if rehydrated is not None:
                    ...
                    entry = auth_mod.persist_cem888_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._cem888_device_code_login(
        ...
        # Honor `--label <name>` so cem888 matches other providers' UX.  The
        # helper embeds this into providers.cem888 so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("cem888").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_cem888_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `cem888 auth
    ...
    raise SystemExit(f"`cem888 auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source CEM888 reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `cem888 auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `cem888 auth` is called bare."""
