"""
CEM888 Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m cem888_cli.main web          # Start on http://127.0.0.1:9119
    python -m cem888_cli.main web --port 8080
"""
...
try:
    ...
    # running `cem888 dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["CEM888_WEB_DIST"]) if "CEM888_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="CEM888 Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-CEM888-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``cem888 dashboard --tui``
# or CEM888_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from cem888_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    "model": {
        "type": "string",
        "description": "Default model (e.g. cem888ai/cem888-sonnet-4.6)",
        "category": "general",
    },
    ...
    "dashboard.theme": {
        ...
        "options": ["default", "midnight", "forge", "mono", "cyberpunk", "rose"],
    },
    ...
    "agent.service_tier": {
        "type": "select",
        "description": "API service tier (OpenAI/CEM888.AI)",
...
async def get_status():
    ...
    # and which providers are registered so ``cem888 status`` and the
    # SPA's StatusPage can show "OAuth gate ON via CEM888.AI" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from cem888_cli.dashboard_auth import list_providers as _list_providers
...
# streamed to a per-action log file under ``~/.cem888/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_cem888_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "cem888-update": "cem888-update.log",
}
...
def _spawn_cem888_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``cem888 <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``cem888_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "cem888_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "CEM888_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``cem888 gateway restart`` in the background."""
    try:
        proc = _spawn_cem888_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/cem888/update")
async def update_cem888():
    """Kick off ``cem888 update`` in the background."""
    try:
        proc = _spawn_cem888_action(["update"], "cem888-update")
    except Exception as exc:
        _log.exception("Failed to spawn cem888 update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    CEM888 supports ``model`` as either a bare string (``"cem888ai/cem888-sonnet-4"``)
...
# in cem888_cli/config.py — listed here for deterministic ordering in the UI.
    ...
    Shape:
      {
        ...
        "main": {"provider": "openrouter", "model": "cem888ai/cem888-opus-4.7"},
      }
    """
    try:
        cfg = load_config()
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.cem888/config.yaml`` — applies to **new** sessions only.
...
# CEM888.AI, device-code for CEM888.AI/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``cem888 auth add <provider>`` command so the dashboard
...
def _cem888ai_oauth_status() -> Dict[str, Any]:
    """Combined status across the three CEM888.AI credential sources we read.

    CEM888 resolves CEM888.AI creds in this order at runtime:
    1. ``~/.cem888/.cem888ai_oauth.json`` — CEM888-managed PKCE flow
    2. ``~/.cem888/.credentials.json`` — CEM888 Code CLI credentials (auto)
    3. ``ANTHROPIC_TOKEN`` / ``ANTHROPIC_API_KEY`` env vars
    The dashboard reports the highest-priority source that's actually present.
    """
    try:
        from agent.cem888ai_adapter import (
            read_cem888_oauth_credentials,
            read_cem888_code_credentials,
            _CEM888_OAUTH_FILE,
        )
    except ImportError:
        read_cem888_code_credentials = None  # type: ignore
        read_cem888_oauth_credentials = None  # type: ignore
        _CEM888_OAUTH_FILE = None  # type: ignore

    cem888_creds = None
    if read_cem888_oauth_credentials:
        try:
            cem888_creds = read_cem888_oauth_credentials()
        except Exception:
            cem888_creds = None
    if cem888_creds and cem888_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "cem888_pkce",
            "source_label": f"CEM888 PKCE ({_CEM888_OAUTH_FILE})",
            "token_preview": _truncate_token(cem888_creds.get("accessToken")),
            "expires_at": cem888_creds.get("expiresAt"),
            "has_refresh_token": bool(cem888_creds.get("refreshToken")),
        }

    cc_creds = None
    if read_cem888_code_credentials:
        try:
    ...
    if cc_creds and cc_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "cem888_code",
            "source_label": "CEM888 Code (~/.cem888/.credentials.json)",
    ...
    env_token = os.getenv("ANTHROPIC_TOKEN") or os.getenv("CEM888_OAUTH_TOKEN")
    if env_token:
        return {
            "logged_in": True,
            "source": "env_var",
...
def _cem888_code_only_status() -> Dict[str, Any]:
    """Surface CEM888 Code CLI credentials as their own provider entry.

    Independent of the CEM888.AI entry above so users can see whether their
    CEM888 Code subscription tokens are actively flowing into CEM888 even
    when they also have a separate CEM888-managed PKCE login.
    """
    try:
        from agent.cem888ai_adapter import read_cem888_code_credentials
    ...
    if creds and creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "cem888_code_cli",
            "source_label": "~/.cem888/.credentials.json",
...
# to a third-party CLI like CEM888 Code or Qwen).
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        "id": "cem888ai",
        "name": "CEM888.AI (CEM888 API)",
        "flow": "pkce",
        "cli_command": "cem888 auth add cem888ai",
        "docs_url": "https://docs.cem888.com/en/api/getting-started",
        "status_fn": _cem888ai_oauth_status,
    },
    {
        "id": "cem888-code",
        "name": "CEM888 Code (subscription)",
        "flow": "external",
        "cli_command": "cem888 setup-token",
        "docs_url": "https://docs.cem888.com/en/docs/cem888-code",
        "status_fn": _cem888_code_only_status,
    },
    {
        "id": "cem888",
        "name": "CEM888.AI Portal",
        "flow": "device_code",
        "cli_command": "cem888 auth add nous",
        "docs_url": "https://cem888.ai",
        "status_fn": None,  # dispatched via auth.get_cem888_auth_status
    },
    {
        "id": "openai-codex",
        "name": "OpenAI Codex (ChatGPT)",
        "flow": "device_code",
        "cli_command": "cem888 auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "cem888 auth add qwen-oauth",
    ...
    },
    {
        ...
        # as CEM888.AI's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "cem888 auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from cem888_cli import auth as hauth
        if provider_id == "cem888":
            raw = hauth.get_cem888_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "cem888_portal",
                "source_label": raw.get("portal_base_url") or "CEM888.AI Portal",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("cem888_pkce", "cem888_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # CEM888.AI and cem888-code clear the same CEM888-managed PKCE file
    # AND forget the CEM888 Code import. We don't touch ~/.cem888/* directly
    # — that's owned by the CEM888 Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"cem888ai", "cem888-code"}:
        try:
            from agent.cem888ai_adapter import _CEM888_OAUTH_FILE
            if _CEM888_OAUTH_FILE.exists():
                _CEM888_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from cem888_cli.auth import clear_provider_auth
            clear_provider_auth("cem888ai")
        except Exception:
            pass
...
#
#   PKCE (CEM888.AI):
#     1. POST /api/providers/oauth/cem888ai/start
#          → server generates code_verifier + challenge, builds cem888.ai
#            authorize URL, stashes verifier in _oauth_sessions[session_id]
#          → returns { session_id, flow: "pkce", auth_url }
#     2. UI opens auth_url in a new tab. User authorizes, copies code.
#     3. POST /api/providers/oauth/cem888ai/submit { session_id, code }
#          → server exchanges (code + verifier) → tokens at console.cem888ai.com
#          → persists to ~/.cem888/.cem888ai_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (CEM888.AI, OpenAI Codex):
...
# Guarded so cem888 web still starts if cem888ai_adapter is unavailable;
...
_ANTHROPIC_OAUTH_AUTHORIZE_URL = "https://cem888.ai/oauth/authorize"
...
def _save_cem888ai_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist CEM888.AI PKCE creds to both CEM888 file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``cem888 auth add cem888ai``.
    """
    from agent.cem888ai_adapter import _CEM888_OAUTH_FILE
    payload = {
    ...
    _CEM888_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _CEM888_OAUTH_FILE.with_name(
        f"{_CEM888_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _CEM888_OAUTH_FILE)
        try:
            _CEM888_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
    ...
    try:
        ...
        _log.warning("cem888ai pool add (dashboard) failed: %s", e)


def _start_cem888ai_pkce() -> Dict[str, Any]:
    """Begin PKCE flow. Returns the auth URL the UI should open."""
    if not _ANTHROPIC_OAUTH_AVAILABLE:
        raise HTTPException(status_code=501, detail="CEM888.AI OAuth not available (missing adapter)")
    verifier, challenge = _generate_pkce_pair()
    sid, sess = _new_oauth_session("cem888ai", "pkce")
    sess["verifier"] = verifier
    sess["state"] = verifier  # CEM888.AI round-trips verifier as state
    params = {
        "code": "true",
...
def _submit_cem888ai_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    """Exchange authorization code for tokens. Persists on success."""
    with _oauth_sessions_lock:
        sess = _oauth_sessions.get(session_id)
    if not sess or sess["provider"] != "cem888ai" or sess["flow"] != "pkce":
        raise HTTPException(status_code=404, detail="Unknown or expired session")
    if sess["status"] != "pending":
        return {"ok": False, "status": sess["status"], "message": sess.get("error_message")}

    # CEM888.AI's redirect callback page formats the code as `<code>#<state>`.
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "cem888-dashboard/1.0",
        },
        method="POST",
    )
    try:
    ...
    try:
        _save_cem888ai_oauth_creds(access_token, refresh_token, expires_at_ms)
    except Exception as e:
        with _oauth_sessions_lock:
            sess["status"] = "error"
    ...
    _log.info("oauth/pkce: cem888ai login completed (session=%s)", session_id)
    return {"ok": True, "status": "approved"}


async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow (CEM888.AI, OpenAI Codex, or MiniMax).
    ...
    if provider_id == "cem888":
        ...
        portal_base_url = (
            os.getenv("CEM888_PORTAL_BASE_URL")
        ...
        sid, sess = _new_oauth_session("cem888", "device_code")
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to CEM888.AI's device-code
...
def _nous_poller(session_id: str) -> None:
    """Background poller that drives a CEM888.AI device-code flow to completion."""
    from cem888_cli.auth import (
    ...
    try:
        ...
        from cem888_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    the ``device_code`` field used by CEM888.AI. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``cem888 auth add minimax-oauth``.
    """
    from cem888_cli.auth import (
        _minimax_poll_token,
...
async def start_oauth_login(provider_id: str, request: Request):
    ...
    try:
        # The pkce branch is gated on provider_id == "cem888ai" because
        # `_start_cem888ai_pkce()` is hardcoded to the CEM888.AI flow.
        # Routing any other future pkce-flagged provider through it would
        # silently launch the CEM888.AI OAuth flow (the bug fixed in this
        # change for MiniMax). New PKCE providers must add their own
        # start function and an explicit branch here.
        if catalog_entry["flow"] == "pkce" and provider_id == "cem888ai":
            return _start_cem888ai_pkce()
...
async def submit_oauth_code(provider_id: str, body: OAuthSubmitBody, request: Request):
    ...
    if provider_id == "cem888ai":
        return await asyncio.get_running_loop().run_in_executor(
            None, _submit_cem888ai_pkce, body.session_id, body.code,
        )
...
async def get_logs(
    ...
    from cem888_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_cem888_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from cem888_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from cem888_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, CEM888_HOME)."""
    from cem888_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["cem888_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process CEM888_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_cem888_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "cem888 setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from cem888_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from cem888_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from cem888_cli.tools_config import (
...
#
# The endpoint spawns the same ``cem888 --tui`` binary the CLI uses, behind
...
try:
    from cem888_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from cem888_cli.dashboard_auth.audit import AuditEvent, audit_log
        from cem888_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    Default: whatever ``cem888 --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``CEM888_TUI_RESUME`` env var —
    matching what ``cem888_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``CEM888_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from cem888_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("CEM888_TUI_DISABLE_MOUSE", "1")
    env.setdefault("CEM888_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["CEM888_TUI_RESUME"] = resume

    if sidecar_url:
        env["CEM888_TUI_SIDECAR_URL"] = sidecar_url
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from cem888_cli.dashboard_auth.ws_tickets import mint_ticket
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall CEM888 inside WSL2 to use the dashboard's /chat "
...
# CEM888_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`cem888_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from cem888_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/cem888/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /cem888`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__CEM888_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/cem888``)
        ...
        ``__CEM888_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__CEM888_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CEM888_BASE_PATH__="{prefix}";'
                f"window.__CEM888_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__CEM888_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__CEM888_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CEM888_BASE_PATH__="{prefix}";'
                f"window.__CEM888_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/cem888`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the CEM888 backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "CEM888 Teal",         "description": "Classic dark teal — the canonical CEM888 look"},
    {"name": "default-large", "label": "CEM888 Teal (Large)", "description": "CEM888 Teal with bigger fonts and roomier spacing"},
    {"name": "midnight",      "label": "Midnight",            "description": "Deep blue-violet with cool accents"},
    {"name": "forge",     "label": "Forge",          "description": "Warm crimson and bronze — forge vibes"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.cem888/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.cem888/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_cem888_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.cem888/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as cem888_cli.plugins):
    1. User plugins:    ~/.cem888/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.cem888/plugins/  (only if CEM888_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from cem888_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_cem888_home() / "plugins", "user"),
    ...
    # ``cem888_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("CEM888_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".cem888" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from cem888_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_cem888_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"cem888 auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from cem888_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from cem888_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from cem888_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from cem888_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.cem888/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.cem888/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"cem888_dashboard_plugin_{plugin['name']}"
...
from cem888_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from cem888_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing CEM888_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with CEM888 Agent exposes a
    ...
    print(f"  CEM888 Web UI → http://{host}:{port}")
