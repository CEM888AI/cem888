"""Profile distributions — shareable, packaged CEM888 profiles via git.

A distribution is a CEM888 profile published as a git repository (or
...
* ``cem888 profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``cem888 skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``cem888 profile``, not a parallel tree):

    cem888 profile install <source> [--name N] [--alias] [--force] [--yes]
    cem888 profile update  <name>  [--force-config] [--yes]
    cem888 profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "cem888-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            cem888_requires=str(data.get("cem888_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.cem888_requires:
            out["cem888_requires"] = self.cem888_requires
        if self.author:
...
def check_cem888_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires CEM888 {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this CEM888 distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``cem888 profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a CEM888 profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a CEM888 distribution."
        )

    # Version check up-front so we fail fast
    check_cem888_requires(manifest.cem888_requires, cem888_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.cem888).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="cem888_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `cem888 profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`cem888 profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="cem888_dist_update_") as tmp:
        plan = plan_install(
