"""
CEM888 Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.cem888/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove CEM888 PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing cem888-agent or cem888 PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# CEM888 Agent" comment and following line
                if '# CEM888 Agent' in line or '# cem888-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('cem888' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing cem888
                if 'cem888' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the cem888 wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "cem888",
        Path("/usr/local/bin/cem888"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains cem888_cli reference)
                content = wrapper.read_text()
                if 'cem888_cli' in content or 'cem888-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``cem888 gateway run`` processes
    ...
    try:
        from cem888_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from cem888_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CEM888_HOME`` and ``CEM888_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\cem888\git\cmd``, ``%LOCALAPPDATA%\cem888\git\bin``,
#      ``%LOCALAPPDATA%\cem888\git\usr\bin``, ``%LOCALAPPDATA%\cem888\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\cem888\git\`` and Node to
#      ``%LOCALAPPDATA%\cem888\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``cem888 dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\cem888\gateway-service\`` and sometimes
...
def _cem888_path_markers(cem888_home: Path) -> list[str]:
    """Path-entry substrings that identify CEM888-owned User-PATH entries."""
    root = str(cem888_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare cem888-agent install dir.
    markers = [root + "\\cem888-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CEM888_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "cem888".  We don't want to catch
    # unrelated entries like "ccem888-foo" or "ephermeral", so we look for
    # backslash-cem888 as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(cem888_home: Path) -> list[str]:
    """Strip CEM888-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _cem888_path_markers(cem888_home)
            kept: list[str] = []
...
def remove_cem888_env_vars_windows() -> list[str]:
    """Delete CEM888_HOME and CEM888_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CEM888_HOME", "CEM888_GIT_BASH_PATH"):
                try:
...
def _is_default_cem888_home(cem888_home: Path) -> bool:
    """Return True when ``cem888_home`` points at the default (non-profile) root."""
    try:
        from cem888_constants import get_default_cem888_root
        return cem888_home.resolve() == get_default_cem888_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CEM888_HOME directory.

    We shell out to ``cem888 -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CEM888_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m cem888_cli.main` so we don't depend on a `cem888`
    #    wrapper that may be half-removed mid-uninstall.
    cem888_invocation = [_sys.executable, "-m", "cem888_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                cem888_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's CEM888_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.cem888/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.cem888/ for future reinstall
    """
    project_root = get_project_root()
    cem888_home = get_cem888_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CEM888_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_cem888_home(cem888_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ CEM888 Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {cem888_home / 'config.yaml'}")
    print(f"  Secrets: {cem888_home / '.env'}")
    print(f"  Data:    {cem888_home / 'cron/'}, {cem888_home / 'sessions/'}, {cem888_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CEM888_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL CEM888 data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the CEM888 code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in cem888_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\cem888\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(cem888_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No CEM888-owned PATH entries in User environment")

        log_info("Removing CEM888_HOME / CEM888_GIT_BASH_PATH User env vars...")
        removed_env = remove_cem888_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No CEM888-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing cem888 command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.cem888/, just remove the cem888-agent subdir
            if cem888_home in project_root.parents or project_root.parent == cem888_home:
    ...
    #     under CEM888_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(cem888_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(cem888_home)
    ...
    # 5. Optionally remove ~/.cem888/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CEM888_HOME dirs live under
        ...
        try:
            if cem888_home.exists():
                shutil.rmtree(cem888_home)
                log_success(f"Removed {cem888_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {cem888_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {cem888_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://cem888.ai/install/windows.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://cem888.ai/install/linux.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using CEM888 Agent! ⚕")
    print()
