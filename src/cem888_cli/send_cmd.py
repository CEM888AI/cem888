"""CLI subcommand: ``cem888 send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"cem888 send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"cem888 send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"cem888 send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"cem888 send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"cem888 send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `cem888 gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.cem888/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_cem888_env() -> None:
    """Populate ``os.environ`` from ``~/.cem888/.env`` AND bridge top-level
    ...
    startup that ``cem888 send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.cem888/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.cem888/config.yaml`` into
       ...
       via ``cem888 config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``cem888 send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.cem888/.env and ~/.cem888/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "cem888 send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  cem888 send --to telegram \"hello\"\n"
            "  cem888 send --to discord:#ops --file report.md\n"
            "  cem888 send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "cem888 send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `cem888 send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform CEM888 "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.cem888/.env + ~/.cem888/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  cem888 send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | cem888 send --to telegram:-1001234567890\n"
            "  cem888 send --to discord:#ops --file /tmp/report.md\n"
            "  cem888 send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  cem888 send --list                  # all platforms\n"
            "  cem888 send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `cem888 send --list telegram`.",
    )
