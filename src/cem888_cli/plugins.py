"""
CEM888 Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with cem888-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.cem888/plugins/<name>/``
3. **Project plugins** – ``./.cem888/plugins/<name>/`` (opt-in via
   ``CEM888_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``cem888_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``CEM888_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("CEM888_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``CEM888_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.cem888/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("CEM888_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When CEM888_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other CEM888 loggers.
    """
    ...
    logger.debug(
        "CEM888_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "cem888_agent.plugins"

_NS_PARENT = "cem888_plugins"
...
class PluginManifest:
    ...
    #              in ~/.cem888/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``cem888 plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``cem888 honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``cem888 <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`cem888_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from cem888_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``cem888 model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: cem888_cli.main imports plugins indirectly
        from cem888_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.cem888/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to cem888_cli/. Two layouts are
        ...
        # 2. User plugins (~/.cem888/plugins/)
        user_dir = get_cem888_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.cem888/plugins/)
        if _env_enabled("CEM888_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".cem888" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set CEM888_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with cem888 and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform CEM888 ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `cem888 plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``cem888_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``cem888_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
# =============================================================================
# CEM-176: runtime-minted tool-result evidence authority
# =============================================================================
# The tool executor calls ``attest_tool_result`` at the trusted post-execution
# boundary -- on the runtime-owned object returned by ``registry.dispatch``,
# before any plugin, model or caller can observe or mutate it -- and hands the
# returned token to the post_tool_call / transform_tool_result hooks.
#
# ``is_runtime_attested`` / ``get_attested_evidence_id`` are the only way a
# verifier may establish that an evidence id came from a real tool execution.
# A verifier that persists a caller-supplied object and then consults its own
# persistence has learned nothing: authority is minted HERE, in a registry the
# verifier cannot write to, and never by the verifier's own storage.

_RUNTIME_TOOL_ATTESTATIONS = {}
_RUNTIME_TOOL_ATTESTATION_LOCK = threading.RLock()
ATTESTATION_ORIGIN_RUNTIME = "runtime_post_exec"
_ATTESTATION_MAX_ENTRIES = 4096
...
def attest_tool_result(
    ...
    *,
    ...
    """Runtime-only: mint evidence authority for one completed tool execution.

    Returns an opaque attestation token, or None on any failure. Callers must
    treat None as "no authority exists" -- never as "assume verified".
    """
    try:
        import uuid as _uuid

        token = "att:" + _uuid.uuid4().hex
        with _RUNTIME_TOOL_ATTESTATION_LOCK:
            _RUNTIME_TOOL_ATTESTATIONS[token] = {
                "origin": ATTESTATION_ORIGIN_RUNTIME,
            ...
            if len(_RUNTIME_TOOL_ATTESTATIONS) > _ATTESTATION_MAX_ENTRIES:
                for _k in list(_RUNTIME_TOOL_ATTESTATIONS)[: _ATTESTATION_MAX_ENTRIES // 4]:
                    _RUNTIME_TOOL_ATTESTATIONS.pop(_k, None)
        return token
    except Exception:
        return None


def bind_attested_evidence(attestation: str, evidence_id: str) -> bool:
    """Associate a persisted evidence id with a runtime attestation.

    Only the runtime mints attestations, so binding confers no authority by
    itself: the id is honoured later only if this succeeded for a token that
    really came from the tool-execution boundary.
    """
    try:
        if not attestation or not evidence_id:
            return False
        with _RUNTIME_TOOL_ATTESTATION_LOCK:
            entry = _RUNTIME_TOOL_ATTESTATIONS.get(str(attestation))
            if not entry or entry.get("origin") != ATTESTATION_ORIGIN_RUNTIME:
                return False
...
def get_attested_evidence_id(attestation: str, tool_call_id: str = "") -> Optional[str]:
    """Return the attested evidence id for a token, or None if not authoritative."""
    try:
        if not attestation:
            return None
        with _RUNTIME_TOOL_ATTESTATION_LOCK:
            ...
            if not entry or entry.get("origin") != ATTESTATION_ORIGIN_RUNTIME:
                return None
            bound = entry.get("evidence_id")
            entry_tc = entry.get("tool_call_id") or ""
        if not bound:
            return None
        if tool_call_id and entry_tc and str(tool_call_id) != entry_tc:
            return None
        return str(bound)
    except Exception:
        return None
...
def is_runtime_attested(attestation: str) -> bool:
    """True only for a token minted by this process at the execution boundary."""
    try:
        if not attestation:
            return False
        with _RUNTIME_TOOL_ATTESTATION_LOCK:
            entry = _RUNTIME_TOOL_ATTESTATIONS.get(str(attestation))
        return bool(entry) and entry.get("origin") == ATTESTATION_ORIGIN_RUNTIME
    except Exception:
        return False
...
def get_pre_api_request_block_message(**kwargs: Any) -> Optional[str]:
    """Return the first structured pre-inference veto from a plugin.

    ``pre_api_request`` is the final seam before provider dispatch.  It uses
    the same narrow ``action=block`` wire shape as ``pre_tool_call`` so a
    lifecycle controller can stop inference without relying on model prose.
    """
    for result in invoke_hook("pre_api_request", **kwargs):
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="cem888-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``cem888 tools`` TUI so plugin-provided toolsets appear
