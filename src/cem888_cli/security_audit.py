"""On-demand supply-chain audit for CEM888 Agent installs.

Scans three surfaces a CEM888 user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The CEM888 venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.cem888/plugins``
...
def _discover_plugins(cem888_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.cem888/plugins``.
    ...
    """
    plugins_dir = cem888_home / "plugins"
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from cem888_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def run_audit(
    *,
    ...
    home = cem888_home or Path(get_cem888_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, cem888_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(cem888_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `cem888 security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, cem888_home=home
    )
    if total == 0:
