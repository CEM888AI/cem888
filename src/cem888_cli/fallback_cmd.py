"""
cem888 fallback — manage the fallback provider chain.
...
Subcommands:
  cem888 fallback [list]   Show the current fallback chain (default when no subcommand)
  cem888 fallback add      Pick provider + model via the same picker as `cem888 model`,
                           then append the selection to the chain
  cem888 fallback remove   Pick an entry to delete from the chain
  cem888 fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.cem888/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from cem888_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `cem888 model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  cem888 fallback add")
        print()
        return
    ...
    print("  Docs: https://cem888.ai/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `cem888 model`, then append the selection to the chain."""
    from cem888_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `cem888 model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `cem888 fallback list` to view, or `cem888 fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``cem888 fallback [subcommand]``."""
