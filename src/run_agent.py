...
Usage:
    ...
    agent = AIAgent(base_url="http://localhost:30000/v1", model="cem888-opus-4-20250514")
...
from cem888_cli.timeouts import (
...
_loaded_env_paths = load_cem888_dotenv(cem888_home=_cem888_home, project_env=_project_env)
...
from agent.prompt_builder import (
    ...
    CEM888_AGENT_HELP_GUIDANCE,
    KANBAN_GUIDANCE,
    build_cem888_subscription_prompt,
)
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"CEM888Agent/{_CEM888_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[cem888-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=self.platform or os.environ.get("CEM888_SESSION_SOURCE", "cli"),
                model=self.model,
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("CEM888_SESSION_SOURCE", "cli"),
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least CEM888' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from cem888_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``cem888 chat -q``. In that mode,
    ...
    def _is_provider_stream_parse_error(self, error: BaseException) -> bool:
        ...
        Some CEM888.AI-compatible streaming providers can send a malformed
        event-stream frame.  The CEM888.AI SDK surfaces that as a plain
        ...
        """
        if getattr(self, "api_mode", None) != "cem888ai_messages":
            return False
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``CEM888_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``CEM888_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("CEM888_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``CEM888_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("CEM888_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        cem888-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See cem888-agent#21444 for symptom history."
        )
    ...
    def _cem888ai_prompt_cache_policy(
        self,
        *,
        ...
        """Forwarder — see ``agent.agent_runtime_helpers.cem888ai_prompt_cache_policy``."""
        from agent.agent_runtime_helpers import cem888ai_prompt_cache_policy
        return cem888ai_prompt_cache_policy(self, provider=provider, base_url=base_url, api_mode=api_mode, model=model)
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # CEM888.AI serves GPT-5.x models via its OpenAI-compatible chat
        # completions endpoint; its /v1/responses endpoint returns 404.
        if normalized_provider == "cem888":
            return False
    ...
    def _is_entitlement_failure(
        ...
        Extend here for new providers as we discover them (CEM888.AI's
        CEM888 Max OAuth entitlement errors look distinct enough today that
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        # JSON body errors from OpenAI/CEM888.AI SDKs
    ...
    def _redact_message_content(content):
        ...
        Handles both plain-string content and the OpenAI/CEM888.AI multimodal
        ...
        Respects ``CEM888_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.cem888/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects CEM888_REDACT_SECRETS via
    ...
    def _get_tool_call_name_static(tc) -> str:
        ...
        message to carry the matching function name. OpenAI/CEM888.AI/ollama
    ...
    def _is_thinking_only_assistant(msg: Dict[str, Any]) -> bool:
        ...
        CEM888.AI, OpenRouter CEM888.AI, third-party CEM888.AI-compatible
        gateways), the resulting message has only thinking blocks — which
        CEM888.AI rejects with HTTP 400 "The final block in an assistant
        message cannot be `thinking`."

        Symmetric with CEM888 Code's ``filterOrphanedThinkingOnlyMessages``
    ...
    def _create_request_openai_client(self, *, reason: str, api_kwargs: Optional[dict] = None) -> Any:
        ...
        # Shared/primary clients and CEM888.AI / Bedrock paths are
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``cem888 auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_cem888_client_credentials(
        self,
        *,
        force: bool = True,
        inference_auth_mode: str | None = None,
    ) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "cem888":
            return False
        ...
        try:
            ...
            logger.debug("CEM888.AI credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # CEM888.AI requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)

        if not self._replace_primary_openai_client(reason="cem888_credential_refresh"):
            return False
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from cem888_cli.copilot_auth import resolve_copilot_token
    ...
    def _try_refresh_cem888ai_client_credentials(self) -> bool:
        if self.api_mode != "cem888ai_messages" or not hasattr(self, "_cem888ai_api_key"):
            return False
        # Only refresh credentials for the native CEM888.AI provider.
        # Other cem888ai_messages providers (MiniMax, Alibaba, etc.) use their own keys.
        if self.provider != "cem888ai":
            return False
        # Azure endpoints use static API keys — OAuth token rotation doesn't apply.
        # Refreshing would pick up ~/.cem888/.credentials.json OAuth token and break auth.
        _base = getattr(self, "_cem888ai_base_url", "") or ""
        if "azure.com" in _base:
            return False

        try:
            from agent.cem888ai_adapter import resolve_cem888ai_token, build_cem888ai_client

            new_token = resolve_cem888ai_token()
        except Exception as exc:
            logger.debug("CEM888.AI credential refresh failed: %s", exc)
            return False
        ...
        if not isinstance(new_token, str) or not new_token.strip():
            return False
        new_token = new_token.strip()
        if new_token == self._cem888ai_api_key:
            return False

        try:
            self._cem888ai_client.close()
        except Exception:
            pass

        try:
            self._cem888ai_client = build_cem888ai_client(
                new_token,
                getattr(self, "_cem888ai_base_url", None),
                timeout=get_provider_request_timeout(self.provider, self.model),
            )
        except Exception as exc:
            logger.warning("Failed to rebuild CEM888.AI client after credential refresh: %s", exc)
            return False

        self._cem888ai_api_key = new_token
        # Update OAuth flag — token type may have changed (API key ↔ OAuth).
        # Only treat as OAuth on native CEM888.AI; third-party endpoints using
        # the CEM888.AI protocol must not trip OAuth paths (#1739 & third-party
        # identity-injection guard).
        from agent.cem888ai_adapter import _is_oauth_token
        self._is_cem888ai_oauth = _is_oauth_token(new_token) if self.provider == "cem888ai" else False
        return True
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = {"User-Agent": "cem888-code/0.1.0"}
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "cem888ai_messages":
            from agent.cem888ai_adapter import build_cem888ai_client, _is_oauth_token
            ...
            self._cem888ai_api_key = runtime_key
            self._cem888ai_base_url = runtime_base
            self._cem888ai_client = build_cem888ai_client(
                runtime_key, runtime_base,
                timeout=get_provider_request_timeout(self.provider, self.model),
            )
            self._is_cem888ai_oauth = _is_oauth_token(runtime_key) if self.provider == "cem888ai" else False
    ...
    def _cem888ai_messages_create(self, api_kwargs: dict):
        if self.api_mode == "cem888ai_messages":
            self._try_refresh_cem888ai_client_credentials()
        return self._cem888ai_client.messages.create(**api_kwargs)

    def _rebuild_cem888ai_client(self) -> None:
        """Rebuild the CEM888.AI client after an interrupt or stale call.

        Handles both direct CEM888.AI and Bedrock-hosted CEM888.AI models
        correctly — rebuilding with the Bedrock SDK when provider is bedrock,
        rather than always falling back to build_cem888ai_client() which
        requires a direct CEM888.AI API key.
        ...
        if getattr(self, "provider", None) == "bedrock":
            ...
            self._cem888ai_client = build_cem888ai_bedrock_client(region)
        else:
            from agent.cem888ai_adapter import build_cem888ai_client
            self._cem888ai_client = build_cem888ai_client(
                self._cem888ai_api_key,
                getattr(self, "_cem888ai_base_url", None),
                timeout=get_provider_request_timeout(self.provider, self.model),
    ...
    def _materialize_data_url_for_vision(image_url: str) -> tuple[str, Optional[Path]]:
        ...
        tmp = tempfile.NamedTemporaryFile(prefix="cem888ai_image_", suffix=suffix, delete=False)
        try:
            with tmp:
    ...
    def _describe_image_for_cem888ai_fallback(self, image_url: str, role: str) -> str:
        cache_key = hashlib.sha256(str(image_url or "").encode("utf-8")).hexdigest()
        cached = self._cem888ai_image_fallback_cache.get(cache_key)
        if cached:
            return cached
        ...
        self._cem888ai_image_fallback_cache[cache_key] = note
        return note
    ...
    def _preprocess_cem888ai_content(self, content: Any, role: str) -> Any:
        ...
        for part in content:
            ...
            if ptype in {"image_url", "input_image"}:
                ...
                if image_url:
                    image_notes.append(self._describe_image_for_cem888ai_fallback(image_url, role))
                else:
        ...
        if suffix:
            return suffix
        return "[A multimodal message was converted to text for CEM888.AI compatibility.]"
    ...
    def _prepare_cem888ai_messages_for_api(self, api_messages: list) -> list:
        ...
        # The CEM888.AI adapter (agent/cem888ai_adapter.py:_convert_content_part_to_cem888ai)
        # already translates OpenAI-style image_url/input_image parts into
        # native CEM888.AI ``{"type": "image", "source": ...}`` blocks. When
        ...
        # Non-vision CEM888.AI model (rare today, but keep the fallback for
        # compat): replace each image part with a vision_analyze text note.
        transformed = copy.deepcopy(api_messages)
        for msg in transformed:
            if not isinstance(msg, dict):
                continue
            msg["content"] = self._preprocess_cem888ai_content(
                msg.get("content"),
    ...
    def _prepare_messages_for_non_vision_model(self, api_messages: list) -> list:
        ...
        for msg in transformed:
            if not isinstance(msg, dict):
                continue
            # Reuse the CEM888.AI text-fallback preprocessor — the behaviour is
    ...
    def _cem888ai_preserve_dots(self) -> bool:
        """True when using an cem888ai-compatible endpoint that preserves dots in model names.
        Alibaba/DashScope keeps dots (e.g. qwen3.5-plus).
        MiniMax keeps dots (e.g. MiniMax-M2.7).
        Xiaomi MiMo keeps dots (e.g. mimo-v2.5, mimo-v2.5-pro).
        OpenCode Go/Zen keeps dots for non-CEM888 models (e.g. minimax-m2.5-free).
        ZAI/Zhipu keeps dots (e.g. glm-4.7, glm-5.1).
        AWS Bedrock uses dotted inference-profile IDs
        (e.g. ``global.cem888ai.cem888-opus-4-7``,
        ``us.cem888ai.cem888-sonnet-4-5-20250929-v1:0``) and rejects
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct CEM888.AI Portal.
        """
        ...
        if (
            ...
            try:
                from cem888_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from cem888_cli.models import lmstudio_model_reasoning_options
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        # Respond First Interceptor: Stream immediate conversational acknowledgement if no text has been streamed yet
        if tool_calls and not getattr(self, "_current_streamed_assistant_text", ""):
            try:
                first_tool_call = tool_calls[0]
                tool_name = first_tool_call.function.name
                tool_acknowledgements = {
                    "search_files": "Let me search our files for you real quick...",
                    "mcp_tavily_tavily_search": "Let me run a quick web search on that...",
                    "mcp_tavily_tavily_research": "Let me dive deep and research this for you real quick...",
                    "read_file": "Let me open up and read that file for you...",
                    "write_file": "Let me write those changes to the file...",
                    "patch": "Applying those edits to the file now...",
                    "mcp_desktop_commander_read_file": "Reading the contents of that local file...",
                    "mcp_desktop_commander_start_search": "Starting a search across the workspace...",
                    "terminal": "Executing that terminal command now...",
                    "mcp_desktop_commander_start_process": "Spawning that terminal process for you..."
                }
                msg = tool_acknowledgements.get(tool_name, f"Executing {tool_name} to check on that real quick...")
                self._fire_stream_delta(msg)
            except Exception:
                pass
...
def main(
    ...
    Args:
        ...
        model (str): Model name to use (OpenRouter format: provider/model). Defaults to cem888ai/cem888-sonnet-4.6.
